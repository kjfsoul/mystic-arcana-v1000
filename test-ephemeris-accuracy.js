...
const {
  SwissEphemerisShim,
} = require("./src/lib/astrology/SwissEphemerisShim.ts");
...
async function runEphemerisValidation() {
  ...
  for (const testCase of TEST_CASES) {
    ...
    if (sunPos) {
      ...
      const accuracy = Math.abs(
        sunPos.longitude - testCase.expected.sun.longitude,
      );

      console.log(`   ☉ Sun: ${sunPos.longitude.toFixed(1)}° (${sunSign})`);
      console.log(
        `   Expected: ${testCase.expected.sun.longitude}° (${testCase.expected.sun.sign})`,
      );
      console.log(`   Accuracy: ±${accuracy.toFixed(1)}°`);

      if (accuracy < 5.0) {
        // Within 5 degrees is acceptable for fallback calculations
    ...
    if (moonPos) {
      ...
      const accuracy = Math.abs(
        moonPos.longitude - testCase.expected.moon.longitude,
      );

      console.log(`   ☽ Moon: ${moonPos.longitude.toFixed(1)}° (${moonSign})`);
      console.log(
        `   Expected: ${testCase.expected.moon.longitude}° (${testCase.expected.moon.sign})`,
      );
      console.log(`   Accuracy: ±${accuracy.toFixed(1)}°`);

      if (accuracy < 10.0) {
        // Moon is more complex, 10 degrees acceptable for fallback
  ...
  if (mercuryToday && mercuryTomorrow) {
    ...
    console.log(
      `   Mercury tomorrow: ${mercuryTomorrow.longitude.toFixed(2)}°`,
    );
    ...
    if (isRetrograde) {
      ...
      console.log(
        `   ⚠️ Mercury retrograde detection inconclusive (may be direct during test)`,
      );
  ...
  if (passedTests >= totalTests * 0.8) {
    console.log(
      `   🎉 EXCELLENT - Ephemeris integration is working correctly!`,
    );
