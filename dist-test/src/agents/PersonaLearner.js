...
export class PersonaLearnerAgent {
        ...
        const memoryNote = {
            content: JSON.stringify({
                ...
                learning_insights: {
                    ...
                    interpretation_quality: 0, // No quality in ConversationSession
                    personalization_applied: reading.card_interpretations.some(interp => interp.confidence_score > 0.8)
                }
            }),
            keywords,
    ...
    async logToAMem(memoryNote, userId, sessionId) {
        try {
            ...
            if (response.status === 201) {
                ...
                throw new Error(`Supermemory MCP logging failed with status: ${response.status}`);
            }
        }
        catch (error) {
    ...
    async processLearningPatterns(userId) {
        ...
        if (!profile)
            return;
        const recentEvents = this.learningQueue.filter(event => event.userId === userId &&
    ...
    calculateReadingQuality(reading) {
        ...
        const avgConfidence = reading.card_interpretations.reduce((sum, interp) => sum + interp.confidence_score, 0) / reading.card_interpretations.length;
    ...
    async retrieveUserMemories(userId) {
        try {
            ...
            if (response.status === 200) {
                ...
                throw new Error(`Supermemory MCP retrieval failed with status: ${response.status}`);
            }
        }
        catch (error) {
            console.error('PersonaLearner: Failed to retrieve user memories from Supermemory MCP:', error);
            return [];
        }
    }
    /**
    ...
    getStats() {
        const lastActivity = this.learningQueue.length > 0
            ...
            : null;
        const interactionEvents = this.learningQueue.filter(e => ['conversation_turn', 'user_response', 'card_revealed', 'question_answered'].includes(e.eventType)).length;
     ...
     */
    async logConversationTurn(userId, sessionId, conversationState, turnNumber, sophiaDialogue, userResponse, revealedCard) {
        if (!userId)
     ...
     */
    async logCardReveal(userId, sessionId, cardName, cardIndex, engagementMetrics) {
        if (!userId)
            return;
        try {
            const learningEvent = {
     ...
     */
    async logQuestionResponse(userId, sessionId, question, response, questionContext) {
        if (!userId)
            return;
        try {
            const learningEvent = {
