...
import { EnhancedTarotCard } from './EnhancedTarotCard.js';
export const EnhancedTarotSpreadLayouts = ({ spreadType, cards, onCardClick, isRevealing = false, showBioluminescence = true, customPositions = [], isMobile = false }) => {
    ...
    const getSpreadPositions = (spreadType) => {
    ...
    const renderConnectionLines = () => {
        if (!showBioluminescence || !isRevealing)
            return null;
        return connectionLines.map((connection, index) => {
            ...
            if (!fromPos || !toPos)
                return null;
            const lineLength = Math.sqrt(Math.pow(toPos.x - fromPos.x, 2) + Math.pow(toPos.y - fromPos.y, 2));
            const angle = Math.atan2(toPos.y - fromPos.y, toPos.x - fromPos.x) * (180 / Math.PI);
            return (React.createElement(motion.div, { key: `connection-${index}`, className: "absolute pointer-events-none", style: {
                    left: '50%',
                    top: '50%',
                    width: lineLength,
                    height: '2px',
              ...
              rgba(0, 255, 255, ${connection.strength * 0.3}) 100%)`,
                ...
                }, initial: { scaleX: 0, opacity: 0 }, animate: {
                    scaleX: 1,
    ...
    const renderEnergyField = () => {
        if (!showBioluminescence || !isRevealing)
            return null;
        return (React.createElement(motion.div, { className: "absolute inset-0 pointer-events-none", style: {
    ...
    return (React.createElement("div", { className: "relative w-full min-h-screen flex items-center justify-center overflow-hidden" },
        renderEnergyField(),
        renderConnectionLines(),
        React.createElement(motion.div, { className: "relative", initial: { opacity: 0, scale: 0.8 }, animate: { opacity: 1, scale: 1 }, transition: { duration: 0.6 }, style: {
            ...
            if (!position)
                return null;
            return (React.createElement(motion.div, { key: card.id, className: "absolute", style: {
                ...
                }, custom: index, initial: "hidden", animate: isRevealing ? "visible" : "hidden", variants: cardVariants },
                React.createElement(motion.div, { className: "absolute -top-8 left-1/2 transform -translate-x-1/2 text-center pointer-events-none", initial: { opacity: 0, y: 10 }, animate: { opacity: isRevealing ? 1 : 0, y: 0 }, transition: { delay: index * 0.4 + 0.5, duration: 0.5 } },
                    React.createElement("div", { className: "text-sm font-medium text-cyan-300 mb-1", style: {
                    ...
                    position.description && (React.createElement("div", { className: "text-xs text-purple-300 opacity-80 max-w-24 leading-tight" }, position.description))),
                React.createElement(EnhancedTarotCard, { card: card, isFlipped: isRevealing && revealedCards.has(index), onFlip: () => handleCardReveal(card, index), size: position.size || 'medium', showBioluminescence: showBioluminescence, delay: index * 0.2 }),
                React.createElement(motion.div, { className: "absolute -bottom-6 left-1/2 transform -translate-x-1/2 bg-purple-600/80 text-white text-xs px-2 py-1 rounded-full font-bold backdrop-blur-sm", initial: { opacity: 0, scale: 0 }, animate: { opacity: isRevealing ? 0.8 : 0, scale: 1 }, transition: { delay: index * 0.4 + 0.8, duration: 0.3 } }, index + 1)));
        })),
        isMobile && spreadType === 'celtic-cross' && (React.createElement(motion.div, { className: "absolute bottom-4 left-4 right-4 bg-black/80 backdrop-blur-md rounded-lg p-4 text-xs", initial: { opacity: 0, y: 50 }, animate: { opacity: isRevealing ? 1 : 0, y: 0 }, transition: { delay: 1, duration: 0.5 } },
            React.createElement("h4", { className: "text-cyan-300 font-semibold mb-2 text-center" }, "Position Guide"),
            React.createElement("div", { className: "grid grid-cols-2 gap-2" }, positions.slice(0, cards.length).map((position, index) => (React.createElement("div", { key: index, className: "flex items-center space-x-2" },
                React.createElement("span", { className: "bg-purple-600 text-white w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold" }, index + 1),
                React.createElement("span", { className: "text-purple-300" }, position.label))))))),
        React.createElement(motion.div, { className: "absolute top-4 left-4 bg-black/60 backdrop-blur-sm rounded-lg p-3 text-sm", initial: { opacity: 0, x: -50 }, animate: { opacity: 0.9, x: 0 }, transition: { delay: 0.5, duration: 0.5 } },
            React.createElement("div", { className: "text-cyan-300 font-semibold capitalize" },
                spreadType.replace('-', ' '),
                " Spread"),
            React.createElement("div", { className: "text-purple-300 text-xs" },
                cards.length,
                " Cards"))));
};
