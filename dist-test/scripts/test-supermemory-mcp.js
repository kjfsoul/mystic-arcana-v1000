import { PersonaLearnerAgent } from "../src/agents/PersonaLearner.js";
...
async function testSupermemoryMcpE2E() {
  ...
  console.log(
    `Simulating reading completion for user ${testUserId} and session ${testSessionId}...`,
  );
  const mockConversationSession = {
    ...
    cards: [
      ...
      {
        id: "high-priestess-card",
        name: "The High Priestess",
        arcana: "major",
        suit: undefined,
        meaning: {
          upright: "Intuition",
          reversed: "Hidden agendas",
          keywords: ["mystery", "subconscious"],
        },
        isReversed: false,
        frontImage: "",
        backImage: "",
    ...
    cardInterpretations: [
      {
        base_interpretation: "Beginnings",
        personalized_guidance: "Embrace the new.",
        spiritual_wisdom: "",
        practical_advice: "",
        reader_notes: "",
        confidence_score: 1,
      ...
      {
        base_interpretation: "Manifestation",
        personalized_guidance: "Create your reality.",
        spiritual_wisdom: "",
        practical_advice: "",
        reader_notes: "",
        confidence_score: 1,
      ...
      {
        base_interpretation: "Intuition",
        personalized_guidance: "Trust your inner voice.",
        spiritual_wisdom: "",
        practical_advice: "",
        reader_notes: "",
        confidence_score: 1,
  ...
  try {
    ...
    const retrievedMemories =
      await personaLearner.retrieveUserMemories(testUserId);
    ...
    if (retrievedMemories.length > 0) {
      const foundEntry = retrievedMemories.find((mem) => {
        try {
          ...
          return (
            contentData.reading_summary?.id === testSessionId ||
            contentData.interaction_data?.session_id === testSessionId
          );
        } catch (e) {
          return false;
        }
      ...
      if (foundEntry) {
        console.log(
          "✅ Test Passed: Memory successfully persisted and retrieved from Supermemory MCP.",
        );
        console.log("Retrieved Entry:", foundEntry);
      } else {
        console.error(
          "❌ Test Failed: Logged memory not found in retrieved entries.",
        );
    ...
    console.error(
      "❌ Test Failed: An error occurred during the E2E test:",
      error,
    );
    process.exit(1);
  }
