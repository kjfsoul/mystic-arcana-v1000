  ...
  async testAllServers(configPath = ".mcp.json") {
    ...
    for (const [serverName, serverConfig] of Object.entries(servers)) {
      ...
      if (envValidation.missing.length > 0) {
        console.log(
          `⚠️  Missing environment variables for ${serverName}: ${envValidation.missing.join(", ")}`,
        );
        results.broken.push({
          name: serverName,
      ...
      if (envValidation.present.length > 0) {
        console.log(
          `✅ Environment variables set for ${serverName}: ${envValidation.present.join(", ")}`,
        );
      }
      ...
      if (result.success) {
        ...
        if (result.capabilities) {
          console.log(
            `   Capabilities: ${Object.keys(result.capabilities).join(", ")}`,
          );
        }
        if (result.serverInfo) {
          console.log(
            `   Server: ${result.serverInfo.name || "Unknown"} v${result.serverInfo.version || "Unknown"}`,
          );
        }
