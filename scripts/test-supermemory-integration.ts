...
// Test configuration
const SUPERMEMORY_MCP_URL =
  process.env.SUPERMEMORY_MCP_URL ||
  "https://supermemory-mcp.your-cloud-provider.com";
...
class SupermemoryIntegrationTest {
  ...
  async runTestSuite(): Promise<TestSuiteResults> {
    ...
    try {
      ...
      if (!endpointReachable) {
        throw new Error(
          `Cloud endpoint ${this.supermemoryUrl} is not reachable`,
        );
      }
  ...
  private async verifyCloudEndpoint(): Promise<boolean> {
    try {
      ...
      try {
        ...
        console.log(
          "⚠️  Health endpoint unavailable, trying status endpoint...",
        );
  ...
  private async runIntegrationTests(): Promise<void> {
    const tests = [
      ...
      {
        name: "PersonaLearner Integration",
        method: this.testPersonaLearnerIntegration.bind(this),
      },
      {
        name: "End-to-End Reading Flow",
        method: this.testEndToEndReadingFlow.bind(this),
      },
      { name: "Error Handling", method: this.testErrorHandling.bind(this) },
      {
        name: "Concurrent Operations",
        method: this.testConcurrentOperations.bind(this),
      },
    ];
  ...
  private async testMemoryRecording(): Promise<any> {
    ...
    const response = await axios.post(
      `${this.supermemoryUrl}/record`,
      testData,
    );

    if (response.status !== 201 && response.status !== 200) {
      throw new Error(
        `Memory recording failed with status: ${response.status}`,
      );
    }
  ...
  private async testMemoryRetrieval(): Promise<any> {
    ...
    const response = await axios.get(
      `${this.supermemoryUrl}/journey/${testUserId}`,
    );

    if (response.status !== 200) {
      throw new Error(
        `Memory retrieval failed with status: ${response.status}`,
      );
    }

    const journey = response.data.journey || [];
    const foundMemory = journey.find(
      (entry: any) =>
  ...
  private async testPersonaLearnerIntegration(): Promise<any> {
    ...
    const testMemory = memories.find(
      (m) =>
  ...
  private async testEndToEndReadingFlow(): Promise<any> {
    ...
    const interactions = [
      {
        ...
        data: { turn: 1, dialogue: "Welcome to your reading" },
      },
      {
        type: "card_reveal",
        data: { card: "The Magician", engagement: "high" },
      },
      { type: "user_response", data: { response: "I feel ready for change" } },
      {
        type: "question_answer",
        data: { question: "What are your goals?", answer: "Personal growth" },
      },
    ];
    ...
    if (memories.length < interactions.length) {
      throw new Error(
        `Expected at least ${interactions.length} memories, got ${memories.length}`,
      );
    }

    // Test engagement analysis
    const analysis =
      await this.personaLearner.getEngagementAnalysis(testUserId);
   ...
   */
  private async logTestInteraction(
    userId: string,
    interaction: any,
  ): Promise<void> {
    const payload = {
  ...
  private printTestSummary(results: TestSuiteResults): void {
    ...
    console.log(
      `🔗 Endpoint Reachable: ${results.endpointReachable ? "✅" : "❌"}`,
    );
