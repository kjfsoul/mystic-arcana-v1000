...
async function testAstronomicalCalculations() {
  ...
  try {
    ...
    if (sun) {
      ...
      console.log(`  Sun in Pisces (Mar 14): ${sunInPisces ? '✅' : '❌'} (${sun.sign} ${sun.degree}°)`);
    }
    
    // Check if planets have different positions (not all the same)
    const uniqueLongitudes = new Set(chart.planets.map(p => Math.floor(p.longitude)));
    const varietyCheck = uniqueLongitudes.size > 5;
    console.log(`  Planetary variety: ${varietyCheck ? '✅' : '❌'} (${uniqueLongitudes.size} unique positions)`);
    
    // Check if retrograde status varies
    const retrogradeCount = chart.planets.filter(p => p.isRetrograde).length;
    const retrogradeCheck = retrogradeCount > 0 && retrogradeCount < chart.planets.length;
    console.log(`  Retrograde variation: ${retrogradeCheck ? '✅' : '❌'} (${retrogradeCount}/${chart.planets.length} retrograde)`);
    ...
    if (sun?.sign === 'Pisces' && varietyCheck && houseCheck) {
      console.log('✅ All astronomical calculations appear to be working correctly!');
