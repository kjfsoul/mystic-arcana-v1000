...
const PING_ENDPOINT = `${API_BASE_URL}/api/health` || `${API_BASE_URL}/api/astrology/birth-chart`;
...
class EinsteinCachingBenchmark {
  ...
  private async logToAMem(event: any): Promise<void> {
    try {
      ...
      const logPath = '/Users/kfitz/mystic-arcana-v1000/A-mem/crew_operations.log';
   ...
   */
  private async callBirthChartAPI(requestNumber: number): Promise<CacheTestResult> {
    ...
    try {
      ...
      try {
        ...
        console.warn(`⚠️  Failed to parse response for request ${requestNumber}`);
      }

      // Extract cache status from headers or response
      const cacheStatus = this.extractCacheStatus(response, data);
      const apiMethod = response.headers.get('X-Calculation-Method') || data.metadata?.method;
   ...
   */
  private extractCacheStatus(response: Response, data: any): 'HIT' | 'MISS' | 'ERROR' | 'UNKNOWN' {
    // Check headers first
  ...
  private async clearCache(): Promise<boolean> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/astrology/cache/clear`, {
        method: 'POST',
      ...
      console.warn('⚠️  Cache clear failed, continuing test anyway:', error.message);
      return false;
    }
  }
  ...
  private async checkRedisConnectivity(): Promise<boolean> {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/astrology/cache/stats`, {
        method: 'GET',
      ...
      if (response.ok) {
        ...
        console.warn('⚠️  Cache stats endpoint unavailable - Redis may not be connected');
        return false;
      }
    } catch (error) {
  ...
  private async primeCache(): Promise<boolean> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.warn('⚠️  Cache priming failed - continuing with benchmark anyway');
        return false;
      }
    } catch (error) {
      ...
      const tryEndpoint = (index: number) => {
        ...
        curlProcess.on('close', (code) => {
          ...
          if (httpStatus >= 200 && httpStatus < 500) {
            console.log(`✅ Server is available via curl (HTTP ${httpStatus} at ${endpoints[index]})`);
            resolve(true);
          } else {
            console.log(`⚠️  Endpoint ${endpoints[index]} returned HTTP ${httpStatus}, trying next...`);
            tryEndpoint(index + 1);
          }
        });

        curlProcess.on('error', (error) => {
          console.log(`⚠️  curl error for ${endpoints[index]}: ${error.message}`);
          tryEndpoint(index + 1);
        });
      };
  ...
  private async testAPIAvailability(): Promise<boolean> {
    ...
    if (!serverAvailable) {
      ...
      console.log('   (Set API_PORT environment variable to use different port)');
  ...
  async runBenchmark(): Promise<CacheBenchmarkSummary> {
    ...
    console.log(`Birth: ${new Date(EINSTEIN_BIRTH_PAYLOAD.birthDate).toLocaleString()}`);
    console.log(`Location: ${EINSTEIN_BIRTH_PAYLOAD.location.city}, ${EINSTEIN_BIRTH_PAYLOAD.location.country}`);
    console.log(`Coordinates: ${EINSTEIN_BIRTH_PAYLOAD.location.lat}°N, ${EINSTEIN_BIRTH_PAYLOAD.location.lon}°E`);
    ...
    console.log(`Expected Results: 5-10x speedup after first request due to caching\n`);
    ...
    if (!apiAvailable) {
      console.error('❌ API is not available. Please start the development server first.');
    ...
    if (!redisConnected) {
      console.warn('⚠️  Redis not available - cache functionality may be limited');
      console.log('   To enable full caching: Start Redis server or check connection');
    }
    ...
    for (let i = 1; i <= TEST_ITERATIONS; i++) {
      ...
      console.log(`   ${status} ${cacheIcon} ${result.responseTime}ms | ${result.cacheStatus} | ${(result.responseSize / 1024).toFixed(1)}KB`);
  ...
  private generateSummary(): CacheBenchmarkSummary {
    ...
    const hitTimes = this.results.filter(r => r.cacheStatus === 'HIT').map(r => r.responseTime);
    const missTimes = this.results.filter(r => r.cacheStatus === 'MISS').map(r => r.responseTime);
    ...
    return {
      ...
      cacheHits,
      cacheMisses,
      cacheEfficiency: this.results.length > 0 ? (cacheHits / this.results.length) * 100 : 0,
      averageResponseTime,
      fastestResponse,
      slowestResponse,
  ...
  displayResults(summary: CacheBenchmarkSummary): void {
    ...
    const hitResults = summary.results.filter(r => r.cacheStatus === 'HIT' && r.success);
    const missResults = summary.results.filter(r => r.cacheStatus === 'MISS' && r.success);
    ...
    if (hitResults.length > 0 && missResults.length > 0) {
      ...
      // Performance rating
      if (speedupFactor > 3) {
        console.log('🟢 EXCELLENT: Cache provides significant performance improvement');
      ...
      console.log('   This may indicate cache is not working or was cleared between requests');
    } else {
      console.log('🟡 INSUFFICIENT DATA: Need both cache hits and misses for comparison');
    }
    ...
    summary.results.forEach((result, index) => {
      ...
      console.log(`${index + 1}. ${status} ${icon} Request ${result.requestNumber}`);
      console.log(`   Time: ${result.responseTime}ms | Cache: ${result.cacheStatus} | Size: ${(result.responseSize / 1024).toFixed(1)}KB`);
      if (result.apiMethod) {
