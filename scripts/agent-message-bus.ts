...
class AgentMessageBus {
  ...
  private initializeMessageStore() {
    if (!existsSync(this.messagePath)) {
      writeFileSync(
        this.messagePath,
        JSON.stringify(
          { messages: [], last_cleaned: new Date().toISOString() },
          null,
          2,
        ),
      );
  ...
  sendMessage(message: Omit<AgentMessage, "id" | "timestamp">): string {
    ...
    try {
      ...
      this.log(
        `Message sent: ${message.from} → ${message.to} [${message.type}:${message.priority}]`,
      );
      return messageId;
    } catch (error) {
      this.log(
  ...
  getMessages(agentId: string, markAsRead = true): AgentMessage[] {
    try {
      ...
      const messages = data.messages.filter(
        (msg: AgentMessage) => msg.to === agentId || msg.to === "broadcast",
      );
      ...
      this.log(
        `Error getting messages for ${agentId}: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      return [];
    }
  }
  ...
  requestCoordination(coordination: CoordinationRequest): string {
    ...
    try {
      ...
      const coordinationRecord = {
        ...
        deadline:
          coordination.deadline || new Date(Date.now() + 300000).toISOString(), // 5 min default
      };
      ...
      this.log(
        `Coordination initiated: ${coordinationId} for ${coordination.task_type} (${coordination.required_agents.join(", ")})`,
      );
      return coordinationId;
    } catch (error) {
      this.log(
        `Error requesting coordination: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      throw error;
    }
  }
  ...
  respondToCoordination(
    ...
    try {
      const data = JSON.parse(readFileSync(this.coordinationPath, "utf-8"));
      const coordination = data.active_coordinations.find(
        (c: any) => c.id === coordinationId,
      );
      ...
      const allResponded = coordination.required_agents.every(
        (agent: string) => coordination.responses[agent],
      );
      ...
      if (allResponded) {
        ...
        data.active_coordinations = data.active_coordinations.filter(
          (c: any) => c.id !== coordinationId,
        );
        ...
        this.log(
          `Coordination completed: ${coordinationId} - all agents responded`,
        );
      }
      ...
      this.log(
        `Error responding to coordination: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      throw error;
    }
  }
  ...
  getCoordinationStatus(coordinationId: string) {
    try {
      ...
      let coordination = data.active_coordinations.find(
        (c: any) => c.id === coordinationId,
      );
      if (!coordination) {
        coordination = data.completed_coordinations.find(
          (c: any) => c.id === coordinationId,
        );
      }
      ...
      this.log(
        `Error getting coordination status: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      return null;
    }
  }
  ...
  cleanup() {
    try {
      ...
      const expired = data.active_coordinations.filter(
        (c: any) => c.deadline < now,
      );
  ...
  getStats() {
    try {
      ...
      const coordData = JSON.parse(
        readFileSync(this.coordinationPath, "utf-8"),
      );
  ...
  switch (command) {
    ...
    case "broadcast": {
      ...
      if (!from || !type || !message) {
        console.error(
          "Usage: agent-message-bus.ts broadcast <from> <type> <message>",
        );
        process.exit(1);
      }
