...
import {
  ContentIngestorAgent,
  StructuredContent,
  ContentSource,
} from "@/src/agents/content-ingestor";
...
class AstrologyKnowledgeIngestor {
  ...
  private async ingestPlanetaryKnowledge(): Promise<void> {
    ...
    for (const planetName of planets) {
      const planet: PlanetKnowledge =
        await this.buildPlanetKnowledge(planetName);
  ...
  private async ingestAspectKnowledge(): Promise<void> {
    ...
    for (const aspectInfo of aspects) {
      const aspect: AspectKnowledge = await this.buildAspectKnowledge(
        aspectInfo.name,
        aspectInfo.degrees,
      );
  ...
  private async ingestTechniqueKnowledge(): Promise<void> {
    ...
    for (const techniqueName of techniques) {
      const technique: TechniqueKnowledge =
        await this.buildTechniqueKnowledge(techniqueName);
      this.knowledgePool.techniques[
        techniqueName.toLowerCase().replace(/\\s+/g, "_")
      ] = technique;
    }
   ...
   */
  private async buildPlanetKnowledge(
    planetName: string,
  ): Promise<PlanetKnowledge> {
  ...
  private async buildPlanetInSigns(
    planetName: string,
  ...
  private async buildPlanetInHouses(
    planetName: string,
  ): Promise<Record<string, string>> {
    return {
  ...
  private async buildPlanetAspects(
    planetName: string,
  ): Promise<Record<string, string>> {
    return {
  ...
  private async buildSignWithPlanets(
    signName: string,
  ): Promise<Record<string, string>> {
    return {}; // Simplified for brevity
  }

  private async buildSignInHouses(
    signName: string,
  ): Promise<Record<string, string>> {
    return {}; // Simplified for brevity
  }
  ...
  private async buildHouseWithPlanets(
    houseNum: number,
  ): Promise<Record<string, string>> {
    return {}; // Simplified for brevity
  }

  private async buildHouseWithSigns(
    houseNum: number,
  ): Promise<Record<string, string>> {
    return {}; // Simplified for brevity
  }

  private async buildAspectKnowledge(
    aspectName: string,
    degrees: number,
  ): Promise<AspectKnowledge> {
  ...
  private async buildTechniqueKnowledge(
    techniqueName: string,
  ): Promise<TechniqueKnowledge> {
