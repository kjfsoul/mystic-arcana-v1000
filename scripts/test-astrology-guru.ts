...
import { AstrologyGuruAgent, ChartAnalysisRequest } from '@/src/agents/astrology-guru';
...
async function testAstrologyGuruIntegration() {
  ...
  try {
    ...
    console.log('Planetary Influences:', Object.keys(analysis.planetaryInfluences).length, 'planets analyzed');
    console.log('Aspect Analysis:', analysis.aspectAnalysis.length, 'major aspects found');
    console.log('House Analysis:', Object.keys(analysis.houseAnalysis).length, 'houses interpreted');
    console.log('Predictive Insights:', analysis.predictiveInsights?.length || 0, 'insights generated');
  ...
  try {
    ...
    console.log('Friendship Rating:', synastryResult.friendship?.rating || 'N/A');
    console.log('Teamwork Rating:', synastryResult.teamwork?.rating || 'N/A');
    console.log('Detailed Analysis:', synastryResult.detailedAnalysis?.substring(0, 100) + '...');
    console.log('Relationship Advice:', synastryResult.relationshipAdvice?.length || 0, 'pieces of advice');
    console.log('Astrological Insights:', synastryResult.astrologicalInsights?.length || 0, 'insights');
  ...
  try {
    const synthesizedKnowledge = await agent.synthesizeKnowledge(
      'Venus in Scorpio', 
      ['Traditional Astrology', 'Modern Interpretation', 'Psychological Astrology']
    );
