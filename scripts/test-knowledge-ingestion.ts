...
class KnowledgeGraphIngestionTest {
  ...
  private async testClearKnowledgeGraphTables(): Promise<void> {
    ...
    try {
      ...
      const clearOperations = [
        {
          table: "kg_interpretation_lineage",
          description: "interpretation lineage records",
        },
        {
          table: "kg_interpretation_synthesis",
          description: "interpretation syntheses",
        },
        {
          table: "kg_concept_relationships",
  ...
  private async testIngestionPipeline(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `  ✅ Pipeline completed: ${successfulExtractions}/${this.TEST_URLS.length} successful`,
      );
    } catch (error) {
      this.addTestResult({
        testName: "Ingestion Pipeline Execution",
  ...
  private async testDataIntegrity(): Promise<void> {
    ...
    try {
      ...
      const { data: interpretations, error: interpretationsError } =
        await this.supabase.from("kg_interpretations").select("*");

      if (interpretationsError) {
        throw new Error(
          `Failed to query interpretations: ${interpretationsError.message}`,
        );
      }
      ...
      this.addTestResult({
        ...
        details: {
          ...
          contexts: [
            ...new Set(interpretations?.map((i) => i.context_type) || []),
          ],
        },
      });
      ...
      for (const expectedCard of this.EXPECTED_CARDS) {
        ...
        this.addTestResult({
          ...
          passed: !!concept,
          message: concept
            ? `${expectedCard} found in database`
            : `${expectedCard} missing from database`,
    ...
    try {
      ...
      for (const concept of conceptsWithInterpretations || []) {
        ...
        for (const interpretation of interpretations) {
          ...
          if (
            interpretation.primary_meaning &&
      ...
      this.addTestResult({
        ...
        passed:
          validRelationships === totalRelationships && totalRelationships > 0,
      ...
      this.addTestResult({
        ...
        message: biddyTarotSource
          ? "Biddy Tarot source found"
          : "Biddy Tarot source missing",
        details: { biddyTarotSource },
      });

      if (biddyTarotSource) {
        console.log(
          `  ✅ Source: Biddy Tarot found with ID ${biddyTarotSource.id}`,
        );
      }
    } catch (error) {
      this.addTestResult({
  ...
  private async testDataQuality(): Promise<void> {
    ...
    try {
      ...
      if (conceptsError) {
        throw new Error(
          `Failed to query concept quality: ${conceptsError.message}`,
        );
      }
      ...
      for (const concept of concepts || []) {
        ...
        console.log(
          `  📊 ${concept.name}: Quality ${qualityScore.toFixed(1)}/10, Keywords: ${hasKeywords}, Archetype: ${hasArchetype}`,
        );
      }
      ...
      this.addTestResult({
        ...
        details: {
          ...
          minQualityScore,
          qualityRate:
            totalConcepts > 0 ? (qualityPassCount / totalConcepts) * 100 : 0,
        },
      });
      ...
      if (interpretationsError) {
        throw new Error(
          `Failed to query interpretation quality: ${interpretationsError.message}`,
        );
      }
      ...
      this.addTestResult({
        ...
        passed:
          meaningfulInterpretations.length === interpretationCount &&
        ...
        details: {
          ...
          qualityRate:
            interpretationCount > 0
              ? (meaningfulInterpretations.length / interpretationCount) * 100
              : 0,
        },
      });

      console.log(
        `  ✅ Quality: ${qualityPassCount}/${totalConcepts} concepts pass standards`,
      );
      console.log(
        `  ✅ Meanings: ${meaningfulInterpretations.length}/${interpretationCount} interpretations are meaningful`,
      );
    } catch (error) {
      this.addTestResult({
  ...
  private async testTablesAreEmpty(): Promise<void> {
    ...
    for (const table of tablesToCheck) {
      ...
      if (error) {
        throw new Error(
          `Failed to check if ${table} is empty: ${error.message}`,
        );
      }
  ...
  private logResults(): void {
    ...
    console.log(
      `📈 Success Rate: ${this.results.totalTests > 0 ? ((this.results.passedTests / this.results.totalTests) * 100).toFixed(1) : 0}%`,
    );
    ...
    // Overall result
    const overallPassed =
      this.results.failedTests === 0 && this.results.passedTests > 0;
    console.log(
    ...
    if (overallPassed) {
      console.log(
        "✨ Knowledge Graph ingestion pipeline is working correctly!",
      );
