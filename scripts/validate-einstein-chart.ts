...
class EinsteinChartValidator {
  ...
  private async testJulianDayCalculation(): Promise<void> {
    await this.runTest("Julian Day Calculation", async () => {
      ...
      if (accuracy < 0.999) {
        ...
        throw new Error(
          `Julian Day accuracy too low: ${(accuracy * 100).toFixed(3)}%`,
        );
      }
    ...
    await this.runTest("Planetary Position Calculations", async () => {
      ...
      for (const planet of planets) {
        ...
        if (EXPECTED_POSITIONS[planet as keyof typeof EXPECTED_POSITIONS]) {
          const expected =
            EXPECTED_POSITIONS[planet as keyof typeof EXPECTED_POSITIONS];
          const accuracy =
            1 - Math.abs(position.longitude - expected.longitude) / 360;
  ...
  private async testAstronomicalAccuracy(): Promise<void> {
    await this.runTest("Astronomical Accuracy Validation", async () => {
      ...
      if (sunSign !== "Pisces") {
        throw new Error(
          `Sun sign mismatch: calculated ${sunSign}, expected Pisces`,
        );
      }
  ...
  private async testBirthChartAPI(): Promise<void> {
    await this.runTest("Birth Chart API Endpoint", async () => {
      const response = await fetch(
        `${this.serverUrl}/api/astrology/birth-chart`,
        {
          method: "POST",
  ...
  private async testAstrologyCalculateAPI(): Promise<void> {
    await this.runTest("Astrology Calculate API", async () => {
      const response = await fetch(
        `${this.serverUrl}/api/astrology/calculate`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ birthData: EINSTEIN_BIRTH_DATA }),
  ...
  private async testCompatibilityAPI(): Promise<void> {
    await this.runTest("Compatibility API", async () => {
      ...
      const response = await fetch(
        `${this.serverUrl}/api/astrology/compatibility`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  private async testHoroscopeAPI(): Promise<void> {
    await this.runTest("Horoscope API", async () => {
      const response = await fetch(
        `${this.serverUrl}/api/astrology/horoscope`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  private displayResults(): void {
    ...
    keyTests.forEach((testName) => {
      ...
      if (result && result.success && result.data) {
        ...
        if (testName === "Astronomical Accuracy Validation") {
          console.log(
            `   Sun: ${result.data.sun.degree.toFixed(1)}° ${result.data.sun.sign}`,
          );
          console.log(
            `   Moon: ${result.data.moon.degree.toFixed(1)}° ${result.data.moon.sign}`,
          );
          console.log(
            `   Sun Sign Correct: ${result.data.validation.sunSignCorrect ? "✅" : "❌"}`,
          );
        }
        ...
        if (testName === "Planetary Position Calculations") {
          Object.entries(result.data).forEach(
            ([planet, data]: [string, any]) => {
              console.log(
                `   ${planet}: ${data.degree.toFixed(1)}° ${data.sign}`,
              );
            },
          );
        }
    ...
    if (successRate >= 90) {
      console.log(
        "🎉 EXCELLENT: Astrology engine is performing exceptionally well!",
      );
    } else if (successRate >= 80) {
      console.log(
        "👍 GOOD: Astrology engine is functioning properly with minor issues.",
      );
    } else if (successRate >= 70) {
      console.log(
        "⚠️  ACCEPTABLE: Astrology engine has some issues that should be addressed.",
      );
    } else {
      console.log(
        "🚨 CRITICAL: Astrology engine requires immediate attention.",
      );
    }
    ...
    console.log(
      "   Astrological Profile: Perfect for a theoretical physicist!",
    );
    console.log("");
