...
const SPREAD_CONFIGURATIONS = {
  'single': [
    { name: 'Your Guidance', description: 'The universe\'s message for you', index: 0 }
  ],
  'three-card': [
  ...
  'celtic-cross': [
    ...
    { name: 'Hopes & Fears', description: 'Inner desires and worries', index: 8 },
...
function generatePersonalizationHooks(card: TarotCard, spreadType: string, position: SpreadPosition): any[] {
  const hooks = [];
...
function generateContextualInterpretation(card: TarotCard, spreadType: string, position: SpreadPosition): string {
  const baseMeaning = card.meanings.light[0] || card.fortune_telling[0] || 'This card brings guidance to your path';
...
async function loadTarotData(): Promise<TarotImageData> {
  ...
  const filePath = path.join(projectRoot, 'public/data/catalogs/tarot-images.json');
...
async function ingestTarotInterpretations(tarotData: TarotImageData): Promise<number> {
    ...
    for (const card of tarotData.cards) {
      for (const position of positions) {
        const interpretation = {
          ...
          arcana_type: card.arcana.toLowerCase().includes('major') ? 'major' : 'minor',
          suit: card.suit?.toLowerCase() === 'trump' ? null : card.suit?.toLowerCase(),
          ...
          base_meaning: generateContextualInterpretation(card, spreadType, position),
          ...
          // Personalization
          personalization_hooks: generatePersonalizationHooks(card, spreadType, position),
          conditional_logic: [
          ...
          content_tags: [
            ...
            ...(card.keywords?.map(k => `keyword_${k.toLowerCase().replace(/\s+/g, '_')}`) || [])
          ],
        ...
        if (interpretations.length >= batchSize) {
          ...
          console.log(`    Inserted ${interpretations.length} interpretations (total: ${totalInserted})`);
  ...
  const sampleInsights = [
    {
      ...
      element_description: 'Mercury stations retrograde in Leo, encouraging inner dialogue and creative reflection',
      ...
      archetypal_energy: 'The Messenger in retreat, turning inward to refine communication and creative expression',
      base_meaning: 'This transit invites deep reflection on how you express your authentic self. Communication may require more care and attention, while creative projects benefit from revision and refinement.',
      contextual_influences: 'Leo\'s influence adds a creative and dramatic flair to this retrograde, making it particularly powerful for artists, performers, and anyone seeking to express their true voice.',
      natal_chart_considerations: {
        'leo_placements': 'Those with strong Leo placements may feel this retrograde more intensely, particularly around self-expression and creativity',
        'mercury_aspects': 'Individuals with prominent Mercury aspects should pay extra attention to communication during this period',
        'fifth_house': 'Fifth house stelliums or rulers may experience shifts in creative projects and self-expression'
      },
      ...
      spiritual_wisdom: 'This retrograde teaches us that true expression comes from deep self-knowledge. Use this time to connect with your authentic voice.',
      practical_advice: 'Review and revise creative projects rather than starting new ones. Practice mindful communication and double-check important messages.',
      emotional_guidance: 'Be patient with yourself if self-expression feels challenging. This is a time for inner work that will enhance your outer creative flow.',
      timing_energy: 'July 18 - August 11, 2025. Peak introspection around the station dates.',
      db_entry_id: 'astro_mercury_retrograde_leo_2025',
      keywords: ['mercury_retrograde', 'leo', 'communication', 'creativity', 'self_expression', 'revision'],
    ...
    },
    {
      ...
      element_description: 'Harmonious aspect between Moon and Pluto promoting emotional transformation',
      primary_planet: 'Moon',
      secondary_planet: 'Pluto',
      aspect_type: 'trine',
      aspect_degree: 120.0,
      orb_range: 3.0,
      archetypal_energy: 'The Divine Feminine in harmony with the Transformer, facilitating deep emotional healing',
      base_meaning: 'This aspect supports profound emotional insights and healing. You may find yourself able to access and transform deep-seated emotional patterns with grace and wisdom.',
      contextual_influences: 'The trine aspect ensures that this transformation feels supportive rather than disruptive, allowing for gentle but powerful emotional evolution.',
      natal_chart_considerations: {
        'water_signs': 'Water sign placements will particularly benefit from this aspect\'s emotional healing potential',
        'eighth_house': 'Eighth house placements may experience profound psychological insights',
        'moon_aspects': 'Those with challenging natal Moon aspects may find relief and healing during this transit'
      },
      personalization_conditions: [
      ...
      spiritual_wisdom: 'True healing happens when we embrace both our light and shadow with equal compassion. This aspect supports that integration.',
      practical_advice: 'Engage in deep emotional work, therapy, or healing practices. Trust your intuitive insights about emotional patterns.',
      emotional_guidance: 'Allow yourself to feel deeply without fear. This aspect provides the strength to transform pain into wisdom.',
      timing_energy: 'Peak influence within 24-48 hours of exact aspect. Effects may be felt for several days.',
      db_entry_id: 'astro_moon_trine_pluto_general',
      keywords: ['moon', 'pluto', 'trine', 'emotional_healing', 'transformation', 'intuition'],
...
async function ingestDailyCosmic(): Promise<number> {
  ...
  const dailyInsight = {
    ...
    database_focus: 'Tarot Card Combination: Ace of Cups + Ten of Swords – Synthesized Meaning in Healing Position',
    ...
    sun_sign: 'Leo',
    active_aspects: [
      { aspect: 'Moon trine Pluto', description: 'Deep emotional transformation' },
      { aspect: 'Chiron retrograde', description: 'Spiritual healing and introspection' }
    ],
    special_events: ['Chiron station retrograde'],
    numerology_value: 9,
    structured_interpretation: {
      element_identification: 'Ace of Cups + Ten of Swords in Healing position of Self-Care spread',
      base_meaning: 'The pairing of emotional renewal (Ace of Cups) with the completion of mental suffering (Ten of Swords) creates a powerful healing synthesis',
      contextual_nuances: 'In the Healing position, these cards invite acknowledgment of past wounds to receive the gift of emotional rebirth',
      ...
      actionable_reflection: 'What tender new beginning waits for me once I honor and release this old wound?',
      keywords: ['emotional_rebirth', 'healing', 'release', 'renewal', 'self_care']
    },
    ...
    blog_post_title: 'From Ruin to Renewal: Embracing Emotional Rebirth with the Ace of Cups + Ten of Swords',
    seo_keywords: ['emotional rebirth', 'tarot healing ritual', 'Ace of Cups meaning', 'Ten of Swords transformation', 'self-care tarot spread'],
    social_content: {
      instagram: {
        caption: 'When the Ten of Swords meets the Ace of Cups in your healing space, remember: every ending carries the seed of a beautiful beginning 🌱✨',
        hashtags: ['#TarotHealing', '#EmotionalRebirth', '#SelfCareSpread']
      },
      tiktok: {
        caption: 'POV: You thought it was over, but the Ace of Cups says "let\'s begin again" 💧',
...
async function ingestCardCombinations(tarotData: TarotImageData): Promise<number> {
  ...
  const popularCards = tarotData.cards.filter(card => 
    ['The Fool', 'The Magician', 'The High Priestess', 'The Empress', 'The Emperor', 
     ...
     'Ace of Cups', 'Ace of Wands', 'Ace of Swords', 'Ace of Pentacles',
     'Ten of Cups', 'Ten of Wands', 'Ten of Swords', 'Ten of Pentacles'].includes(card.name)
  );
  
  // Create meaningful combinations
  const meaningfulPairs = [
    ['The Fool', 'The Magician', 'New beginnings meet focused intention - a powerful combination for manifesting dreams into reality'],
    ['The High Priestess', 'The Hermit', 'Inner wisdom and solitary reflection combine to deepen spiritual understanding'],
    ['The Lovers', 'Ten of Cups', 'Heart-centered choices lead to emotional fulfillment and lasting happiness'],
    ['Ace of Cups', 'Ten of Swords', 'Emotional renewal emerges from the completion of mental suffering'],
    ['The Empress', 'Ace of Wands', 'Creative fertility meets passionate inspiration for artistic manifestation'],
    ['The Emperor', 'Ace of Pentacles', 'Structured leadership creates solid material foundations'],
    ['Strength', 'The Chariot', 'Inner courage combines with determined action for unstoppable progress'],
    ['The Hermit', 'Ace of Swords', 'Solitary reflection brings mental clarity and breakthrough insights']
  ];
    ...
    if (primaryCard && secondaryCard) {
      ...
      for (const spreadType of ['three-card', 'celtic-cross', 'relationship']) {
        const combination = {
          ...
          potential_conflicts: 'These cards generally work in harmony, though their different energies may require conscious integration.',
          harmony_aspects: 'Both cards support personal development and conscious evolution along the spiritual path.',
  ...
  for (let i = 0; i < combinations.length; i += batchSize) {
    ...
    console.log(`  Inserted ${batch.length} combinations (total: ${totalInserted})`);
  }
  ...
  const patterns = [
    {
      ...
      pattern_description: 'Users with Sun, Moon, or Rising in Leo receive enhanced creative and self-expression guidance',
      trigger_conditions: {
        'natal_sun': 'Leo',
        'natal_moon': 'Leo',
        'natal_rising': 'Leo',
      ...
      guidance_modifications: 'Emphasize creative courage and authentic self-expression. Encourage bold action and creative risk-taking.',
      recommended_actions: [
      ...
      suggested_features: ['Creative Expression Spread', 'Leo New Moon Ritual', 'Confidence Building Meditation'],
      meditation_tracks: ['Solar Confidence', 'Creative Fire', 'Authentic Self'],
      journal_prompts: [
    ...
    },
    {
      ...
      pattern_description: 'Users who frequently draw The Tower receive specialized transformation and resilience guidance',
      trigger_conditions: {
        'card_frequency': { 'The Tower': '>= 3' },
        'time_period': '30_days'
      },
      card_requirements: {
        'transformation_cards': ['The Tower', 'Death', 'The Hanged Man', 'Judgment']
      },
      interpretation_adjustments: {
      ...
      guidance_modifications: 'Provide extra support for major life changes. Emphasize resilience, growth through challenge, and the rebuilding process.',
      recommended_actions: [
      ...
      suggested_features: ['Resilience Building Spread', 'Phoenix Rising Meditation', 'Transformation Journal'],
      meditation_tracks: ['Stability in Change', 'Phoenix Rising', 'Inner Strength'],
      journal_prompts: [
    ...
    },
    {
      ...
      pattern_description: 'Users frequently journaling about relationships receive enhanced relationship-focused interpretations',
      trigger_conditions: {
        'journal_keywords': ['love', 'relationship', 'partner', 'dating', 'marriage', 'connection'],
        'frequency': '>= 5',
        'time_period': '14_days'
      },
      card_requirements: {
        'relationship_cards': ['The Lovers', 'Two of Cups', 'Ten of Cups', 'The Empress', 'The Emperor']
      },
      interpretation_adjustments: {
      ...
      guidance_modifications: 'Frame interpretations through the lens of relationships and emotional connections. Emphasize communication, understanding, and heart-centered choices.',
      recommended_actions: [
      ...
      suggested_features: ['Relationship Dynamics Spread', 'Heart Chakra Meditation', 'Love Journal'],
      meditation_tracks: ['Heart Opening', 'Compassionate Love', 'Relationship Harmony'],
      journal_prompts: [
...
async function main() {
  ...
  try {
    ...
    const { data, error } = await supabase.from('tarot_interpretations').select('count', { count: 'exact', head: true });
    if (error) {
