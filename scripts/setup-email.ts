...
async function setupEmailSystem() {
  ...
  console.log('2. App-specific password generated from Google Account settings');
  console.log('3. Go to: Google Account > Security > 2-Step Verification > App passwords\n');
  
  const emailUser = await question('Enter your email address (for sending notifications): ');
  const emailPass = await question('Enter your Gmail app password (16 characters): ');
  
  console.log('\n📅 Schedule Configuration\n');
  const dailyTime = await question('Daily report time (default 09:00): ') || '09:00';
  const urgentInterval = await question('Urgent check interval in minutes (default 15): ') || '15';
  
  console.log('\n📬 Recipient Configuration\n');
  const recipient = await question('Recipient email (default kjfsoul@gmail.com): ') || 'kjfsoul@gmail.com';
...
`;
  ...
  if (existingEnv) {
    ...
    const cleanedEnv = existingEnv
      .split('\n')
      .filter(line => !line.startsWith('EMAIL_') && !line.startsWith('NOTIFICATION_') && !line.startsWith('DAILY_REPORT_') && !line.startsWith('URGENT_CHECK_'))
      .join('\n');
  ...
  try {
    ...
    console.log(`❌ Email test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    console.log('Check your credentials and try again with: npm run email:test');
  }
