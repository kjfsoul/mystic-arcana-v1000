...
class CrewSystemTester {
  ...
  async runAllTests(): Promise<void> {
    ...
    await this.testCrew(
      "Complete Crew Tarot Deck Generation",
    ...
    try {
      ...
      if (result.success) {
        ...
        result.results.forEach((taskResult) => {
          ...
          console.log(
            `      ${status} ${taskResult.taskId} (${taskResult.duration}ms)`,
          );
        });
  ...
  private displayResults(): void {
    ...
    const deckGenResult = this.results.find(
      (r) =>
    ...
    if (deckGenResult?.data) {
      ...
      const cardGenTask = operation.results.find(
        (r: any) => r.taskId === "generate_cards",
      );
      ...
      const validationTask = operation.results.find(
        (r: any) => r.taskId === "validate_deck",
      );
      if (validationTask?.data?.validationResults) {
        const validation = validationTask.data.validationResults;
        console.log(
          `Validation: ${validation.issues.length === 0 ? "PASSED" : "FAILED"}`,
        );
