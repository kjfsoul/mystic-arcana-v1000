...
async function checkJourneyEntriesTable() {
  try {
    ...
    if (error) {
      if (error.message.includes('relation "public.journey_entries" does not exist')) {
        console.log('Table does not exist. It needs to be created via Supabase Dashboard.');
        console.log('Please go to your Supabase Dashboard SQL Editor and run the migration manually.');
        return false;
      } else {
