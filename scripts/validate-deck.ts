...
async function validateDeck() {
  ...
  try {
    ...
    const requiredFields = ['name', 'meaning_upright', 'meaning_reversed', 'image_url', 'keywords'];
    
    cards.forEach((card: CardValidation) => {
      const missing: string[] = [];
      
      if (!card.name || card.name.trim().length === 0) missing.push('name');
      if (!card.meaning_upright || card.meaning_upright.trim().length === 0) missing.push('meaning_upright');
      if (!card.meaning_reversed || card.meaning_reversed.trim().length === 0) missing.push('meaning_reversed');
      if (!card.image_url || card.image_url.trim().length === 0) missing.push('image_url');
      if (!card.keywords || !Array.isArray(card.keywords) || card.keywords.length === 0) missing.push('keywords');

      if (missing.length > 0) {
        incompleteCards++;
        issues.push(`❌ ${card.name} (${card.arcana_type}): Missing ${missing.join(', ')}`);
      }
    });
    ...
    if (incompleteCards === 0) {
      ...
      console.log(`❌ Data Completeness: ${incompleteCards} cards missing data`);
    }
    ...
    const spotCheckCards = [
      ...
      { name: 'Ace of Cups', expectedNumber: 1, expectedArcana: 'minor', expectedSuit: 'cups' },
      { name: 'King of Wands', expectedNumber: 14, expectedArcana: 'minor', expectedSuit: 'wands' },
    ];
    ...
    spotCheckCards.forEach(check => {
      ...
      if (!card) {
        ...
        console.log(`   Number: ${card.card_number} (expected: ${check.expectedNumber})`);
        console.log(`   Arcana: ${card.arcana_type} (expected: ${check.expectedArcana})`);
        if (check.expectedSuit) {
          console.log(`   Suit: ${card.suit} (expected: ${check.expectedSuit})`);
        }
        console.log(`   Keywords: [${card.keywords?.join(', ') || 'none'}]`);
        console.log(`   Image: ${card.image_url}`);
        
        if (card.card_number !== check.expectedNumber) {
          issues.push(`❌ ${check.name}: Wrong number ${card.card_number}, expected ${check.expectedNumber}`);
        }
        if (card.arcana_type !== check.expectedArcana) {
          issues.push(`❌ ${check.name}: Wrong arcana ${card.arcana_type}, expected ${check.expectedArcana}`);
        }
        if (check.expectedSuit && card.suit !== check.expectedSuit) {
          issues.push(`❌ ${check.name}: Wrong suit ${card.suit}, expected ${check.expectedSuit}`);
        }
      }
      console.log('');
    });
    ...
    if (imagePaths.length === uniquePaths.size) {
      ...
      const duplicates = imagePaths.filter((path, index) => imagePaths.indexOf(path) !== index);
      issues.push(`❌ Duplicate image paths found: ${duplicates.join(', ')}`);
    }

    // Sample image paths
    const majorArcanaPath = cards.find(c => c.arcana_type === 'major')?.image_url;
    const minorArcanaPath = cards.find(c => c.arcana_type === 'minor')?.image_url;
