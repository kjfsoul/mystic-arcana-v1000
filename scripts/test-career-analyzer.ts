...
async function testCareerAnalyzer(): Promise<void> {
  ...
  try {
    ...
    const { analyzeCareer, getRealCareerPlacements } = await import('../src/lib/astrology/CareerAnalyzer.js');
    console.log('✅ CareerAnalyzer imports successful');
    
    // Test SwissEphemerisShim import
    const { SwissEphemerisShim } = await import('../src/lib/astrology/SwissEphemerisShim.js');
    ...
    try {
      ...
      console.log(`   Recommended paths: ${analysis.recommendedPaths.length} items`);
      
    } catch (error) {
      if (error instanceof Error) {
        if (error.message.includes('calculateChart') || error.message.includes('not a function')) {
          console.log('❌ FAILED: Still has calculateChart error - fix not applied correctly');
          throw error;
        } else {
          console.log('⚠️  Career analysis failed (expected if Swiss Ephemeris unavailable)');
          console.log(`   Error: ${error.message}`);
          console.log('✅ But no "calculateChart" errors detected - fix successful!');
        }
      }
    }
    ...
    try {
      ...
      console.log(`   Planetary positions: ${Object.keys(placements).length} planets`);
      console.log(`   Midheaven: ${placements.midheaven}°`);
      
    } catch (error) {
      if (error instanceof Error) {
        if (error.message.includes('calculateChart') || error.message.includes('not a function')) {
          console.log('❌ FAILED: Still has calculateChart error in getRealCareerPlacements');
          throw error;
        } else {
          console.log('⚠️  Real placements failed (expected if Swiss Ephemeris unavailable)');
          console.log(`   Error: ${error.message}`);
          console.log('✅ But no "calculateChart" errors detected - fix successful!');
        }
      }
...
async function testBirthChartComponent(): Promise<void> {
  ...
  try {
    ...
    const componentPath = path.resolve(process.cwd(), 'src/components/astrology/InteractiveBirthChart.tsx');
    ...
    if (componentContent.includes('AstronomicalCalculator.calculateChart')) {
      console.log('❌ FAILED: Still contains old AstronomicalCalculator.calculateChart call');
      throw new Error('Component still has incorrect method call');
    } else {
      console.log('✅ No incorrect AstronomicalCalculator.calculateChart calls found');
    }
...
function generateTestReport(): void {
  ...
  console.log('\n✅ FIXES APPLIED:');
  console.log('  • Updated CareerAnalyzer.ts imports to use SwissEphemerisShim');
  ...
  console.log('\n🔧 TECHNICAL CHANGES:');
  console.log('  • CareerAnalyzer: getRealCareerPlacements() now uses SwissEphemerisShim.calculateFullChart()');
  console.log('  • CareerAnalyzer: getHousePositions() now uses SwissEphemerisShim.calculateFullChart()');
  console.log('  • InteractiveBirthChart: Chart calculation now uses SwissEphemerisShim.calculateFullChart()');
  console.log('  • All files: Removed non-existent AstronomicalCalculator.calculateChart references');
  
  console.log('\n🎯 EXPECTED RESULT:');
  console.log('  • Career report should load without "is not a function" errors');
  console.log('  • Birth chart component should render without calculation errors');
  console.log('  • Real astronomical calculations when Swiss Ephemeris available');
