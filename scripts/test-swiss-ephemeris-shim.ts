...
async function testSwissEphemerisShim() {
  ...
  try {
    ...
    console.log(
      `  Initialization status: ${initialized ? "✅ Swiss Ephemeris loaded" : "⚠️ Using fallback calculations"}`,
    );
    ...
    for (const planet of planetTests) {
      ...
      if (result) {
        ...
        console.log(
          `  ${planet}: ${result.longitude.toFixed(2)}° ${sign} ${degree}°${retrograde}`,
        );
        ...
        if (planet === "Moon" && result.speed > 10 && result.speed < 15) {
          console.log(
            `    ✅ Moon speed realistic: ${result.speed.toFixed(2)}°/day`,
          );
        }
      } else {
    ...
    const houses = SwissEphemerisShim.calculateHouses(
      jd,
    ...
    console.log(
      `  Ascendant (1st house): ${houses[0]?.toFixed(2)}° ${SwissEphemerisShim.getZodiacSign(houses[0])}`,
    );
    console.log(
      `  Midheaven (10th house): ${houses[9]?.toFixed(2)}° ${SwissEphemerisShim.getZodiacSign(houses[9])}`,
    );

    console.log("\n🎯 Testing Full Chart Calculation...");
    const fullChart =
      await SwissEphemerisShim.calculateFullChart(testBirthData);
    ...
    const validationChecks = {
      ...
      sunInPisces:
        fullChart.planets.find((p) => p.name === "Sun")?.sign === "Pisces",
      varietyCheck:
        new Set(fullChart.planets.map((p) => Math.floor(p.longitude / 30)))
          .size >= 4,
      retrogradePresent: fullChart.planets.some((p) => p.isRetrograde),
      speedsRealistic: fullChart.planets.every(
        (p) => p.speed >= 0 && p.speed < 20,
      ),
    };
    ...
    if (passedChecks >= 5) {
      ...
      console.log(
        "✅ Compatibility layer successfully provides astronomical calculations.",
      );
    } else if (passedChecks >= 3) {
      console.log(
        "⚠️ PARTIAL: Shim is partially working but needs improvement.",
      );
    } else {
    ...
    const calcChart =
      await AstronomicalCalculator.calculateChart(testBirthData);
    console.log(
      `  Integration test: ${calcChart.planets.length > 0 ? "✅ Working" : "❌ Failed"}`,
    );
  } catch (error) {
