...
class TestSophiaAgent {
    ...
    try {
      ...
      if (context.userId) {
        try {
          ...
          console.log(`TestSophia: Retrieved ${userMemories.length} memories for user ${context.userId}`);
        } catch (memoryError) {
          console.warn('TestSophia: Could not retrieve user memories, proceeding with base reading:', memoryError);
        }
      }
  ...
  private generateProgressionPattern(currentCard: TarotCard, userMemories: any[]): string | null {
...
class SophiaPersonalizationTester {
  ...
  private async testMemorySetup(): Promise<void> {
    ...
    try {
      ...
      const mockMemoryNote: MemoryNote = {
        content: JSON.stringify({
          reading_summary: {
            ...
            narrative: 'A reading about sudden change and upheaval in your life',
            guidance: 'Embrace the transformation that comes with unexpected change',
            spiritual_insight: 'The Tower represents necessary destruction for rebuilding'
          },
          interaction_data: {
        ...
        tags: ['user_interaction', 'reading_completed', 'sophia_session', 'card_the_tower'],
        timestamp: '2025-01-15T10:30:00Z'
      };
      ...
      const mockMemoryNote2: MemoryNote = {
        ...
        category: 'tarot_reading',
        tags: ['user_interaction', 'reading_completed', 'sophia_session', 'card_the_hermit'],
        timestamp: '2025-01-20T14:15:00Z'
      };
  ...
  private async testMemoryRetrieval(): Promise<void> {
    ...
    try {
      const memories = await this.personaLearner.retrieveUserMemories(this.testUserId);
  ...
  private async testSynthesisLogic(): Promise<void> {
    ...
    try {
      ...
      const mockMemories = await this.personaLearner.retrieveUserMemories(this.testUserId);
  ...
  private async testCompleteReading(): Promise<void> {
    ...
    try {
      ...
      const reading = await this.sophia.getReading([testCard], 'single', readingContext);
      
      const passed = reading && reading.narrative && reading.card_interpretations.length > 0;
  ...
  private async testTowerToAceSwordsPattern(): Promise<void> {
    ...
    try {
      ...
      const reading = await this.sophia.getReading([aceOfSwords], 'single', readingContext);
      
      // Check if the interpretation references The Tower from memory
      const interpretationText = reading.card_interpretations[0]?.personalized_guidance || '';
      const narrativeText = reading.narrative || '';
      const guidanceText = reading.overall_guidance || '';
      
      const fullText = (interpretationText + ' ' + narrativeText + ' ' + guidanceText).toLowerCase();
      ...
      const containsMemoryReference = fullText.includes('previous') || 
                                     fullText.includes('before') || 
                                     fullText.includes('journey together') ||
                                     fullText.includes('our work');
      const containsSynthesis = fullText.includes('cut through') ||
                               fullText.includes('old ways') ||
                               fullText.includes('build anew') ||
                               fullText.includes('clarity');

      const passed = containsTowerReference && (containsMemoryReference || containsSynthesis);
      ...
      if (!passed) {
        ...
        console.log('Full interpretation text:', fullText.substring(0, 500) + '...');
      }
  ...
  private async testThemeContinuity(): Promise<void> {
    ...
    try {
      ...
      const fullText = (reading.narrative + ' ' + reading.overall_guidance + ' ' + 
                       reading.card_interpretations[0]?.personalized_guidance).toLowerCase();
      
      // Check for theme continuity (transformation themes from Tower reading)
      const hasThemeContinuity = fullText.includes('transformation') ||
  ...
  private async testMemoryIntegration(): Promise<void> {
    ...
    try {
      ...
      const reading = await this.sophia.getReading([newCard], 'single', readingContext);
      
      const fullText = (reading.narrative + ' ' + reading.overall_guidance + ' ' + 
                       reading.card_interpretations[0]?.personalized_guidance).toLowerCase();
      
      // Should reference user's journey or growth based on memories
      const hasPersonalization = fullText.includes('journey') ||
