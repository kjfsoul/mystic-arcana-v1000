...
async function testHoroscopeGeneration() {
  ...
  try {
    ...
    console.log(`   - Knowledge Base: ${status.knowledgeBase?.signsLoaded || 0} signs loaded`);
    ...
    horoscopeData.horoscopes.slice(0, 3).forEach((horoscope, index) => {
      ...
      console.log(`   ⭐ Ratings: Overall ${horoscope.rating.overall}/5, Love ${horoscope.rating.love}/5, Career ${horoscope.rating.career}/5`);
    ...
    console.log('===========================');
    const ariesSample = horoscopeData.horoscopes.find(h => h.sign === 'Aries');
    if (ariesSample) {
    ...
    console.log('\n🔍 Validation Results:');
    console.log('======================');
    const allValid = horoscopeData.horoscopes.every(h => 
      h.sign && h.horoscope && h.keywords && h.rating && 
      typeof h.rating.overall === 'number' && h.rating.overall >= 1 && h.rating.overall <= 5
    );
    
    console.log(`✅ Structure Valid: ${allValid ? 'YES' : 'NO'}`);
    console.log(`✅ All 12 Signs: ${horoscopeData.horoscopes.length === 12 ? 'YES' : 'NO'}`);
    console.log(`✅ Swiss Ephemeris: ${horoscopeData.metadata.moonSign !== 'Unknown' ? 'WORKING' : 'FALLBACK'}`);
...
testHoroscopeGeneration()
  .then((result) => {
    if (result.success) {
      ...
      console.log(`📊 Generated ${result.count} horoscopes with ${result.valid ? 'valid' : 'invalid'} structure`);
      ...
      const sampleOutput = {
        ...
        horoscope: result.data.horoscopes[0]?.horoscope || 'Sample horoscope text',
        keywords: result.data.horoscopes[0]?.keywords || ['action', 'energy', 'leadership'],
        luckyNumbers: result.data.horoscopes[0]?.luckyNumbers || [1, 8, 15, 22, 29],
        colors: result.data.horoscopes[0]?.colors || ['Red', 'Orange', 'Gold'],
        careerInsight: result.data.horoscopes[0]?.careerInsight || 'Career sample',
        ...
        rating: result.data.horoscopes[0]?.rating || { overall: 4.0, love: 4.0, career: 4.0, health: 4.0 }
      };
