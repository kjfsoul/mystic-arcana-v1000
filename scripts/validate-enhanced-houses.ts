...
async function callBirthChartAPI(
  birthData: BirthData,
): Promise<BirthChartResponse> {
  try {
    const response = await fetch(
      "http://localhost:3002/api/astrology/birth-chart",
      {
        method: "POST",
        headers: {
...
function validateHouseBreakdown(houseBreakdown: string[]): boolean {
  ...
  console.log(
    `✅ Valid house breakdown items: ${validItems}/${houseBreakdown.length}`,
  );
...
function validateSignSummary(signSummary: string): boolean {
  ...
  const requiredElements = [/birth chart/i, /house system/i, /calculated/i];
...
async function runEnhancedHouseValidation(): Promise<void> {
  ...
  console.log(
    "Subject: Albert Einstein (March 14, 1879, 11:30 AM, Ulm, Germany)\n",
  );
  ...
  try {
    ...
    if (testsPassed === totalTests) {
      console.log(
        "🎉 ALL VALIDATIONS PASSED - Enhanced house system is working correctly!",
      );
    } else if (testsPassed >= totalTests * 0.75) {
      console.log(
        "⚠️  MOST VALIDATIONS PASSED - System is functional with minor issues",
      );
    } else {
    ...
    console.log(
      `Placidus Integration: ${data.placidusMethod ? "Active" : "Not detected"}`,
    );
  } catch (error) {
