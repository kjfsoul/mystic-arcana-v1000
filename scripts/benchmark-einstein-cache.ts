...
class AstrologyBenchmark {
  ...
  async callBirthChartAPI(birthData: BirthData, testName: string): Promise<BenchmarkResult> {
    const startTime = Date.now();
    
    try {
      const response = await fetch(`${API_BASE_URL}/api/astrology/birth-chart`, {
        method: 'POST',
        headers: {
      ...
      const cacheStatus = response.headers.get('X-Cache-Status') as 'HIT' | 'MISS' | 'ERROR' || 
  ...
  async runBenchmark(): Promise<BenchmarkSummary> {
    ...
    console.log(`Birth: ${new Date(EINSTEIN_BIRTH_DATA.date).toLocaleString()}`);
    ...
    const coldResult = await this.callBirthChartAPI(EINSTEIN_BIRTH_DATA, 'Cold Cache (First Request)');
    this.results.push(coldResult);
    console.log(`   Duration: ${coldResult.duration_ms}ms | Status: ${coldResult.cache_status} | Success: ${coldResult.success}`);
    ...
    const warmResult = await this.callBirthChartAPI(EINSTEIN_BIRTH_DATA, 'Warm Cache (Cached Request)');
    this.results.push(warmResult);
    console.log(`   Duration: ${warmResult.duration_ms}ms | Status: ${warmResult.cache_status} | Success: ${warmResult.success}`);

    // Test 3-6: Multiple cache hits to measure consistency
    console.log('3️⃣  Testing cache consistency (4 additional requests)...');
    for (let i = 0; i < 4; i++) {
      const result = await this.callBirthChartAPI(EINSTEIN_BIRTH_DATA, `Cache Consistency Test ${i + 1}`);
      this.results.push(result);
      console.log(`   Request ${i + 1}: ${result.duration_ms}ms | ${result.cache_status}`);
    ...
    const modifiedData = { ...EINSTEIN_BIRTH_DATA, name: 'Albert Einstein (Modified)' };
    const modifiedResult = await this.callBirthChartAPI(modifiedData, 'Modified Data (Different Cache Key)');
    this.results.push(modifiedResult);
    console.log(`   Duration: ${modifiedResult.duration_ms}ms | Status: ${modifiedResult.cache_status} | Success: ${modifiedResult.success}`);
  ...
  generateSummary(): BenchmarkSummary {
    ...
    const cacheHits = this.results.filter(r => r.cache_status === 'HIT').length;
    const cacheMisses = this.results.filter(r => r.cache_status === 'MISS').length;
    ...
    const hitDurations = this.results.filter(r => r.cache_status === 'HIT').map(r => r.duration_ms);
    const missDurations = this.results.filter(r => r.cache_status === 'MISS').map(r => r.duration_ms);
    
    const avgHitTime = hitDurations.length > 0 ? hitDurations.reduce((a, b) => a + b, 0) / hitDurations.length : 0;
    const avgMissTime = missDurations.length > 0 ? missDurations.reduce((a, b) => a + b, 0) / missDurations.length : 0;
    ...
    return {
      ...
      cache_efficiency: this.results.length > 0 ? (cacheHits / this.results.length) * 100 : 0,
  ...
  displaySummary(summary: BenchmarkSummary): void {
    ...
    console.log(`Total Time Saved by Caching: ${summary.total_time_saved_ms}ms`);

    console.log('\n🎯 PERFORMANCE ANALYSIS:');
    console.log('========================');
    
    const hitResults = summary.results.filter(r => r.cache_status === 'HIT' && r.success);
    const missResults = summary.results.filter(r => r.cache_status === 'MISS' && r.success);
    
    if (hitResults.length > 0 && missResults.length > 0) {
      const avgHitTime = hitResults.reduce((a, b) => a + b.duration_ms, 0) / hitResults.length;
      const avgMissTime = missResults.reduce((a, b) => a + b.duration_ms, 0) / missResults.length;
    ...
    summary.results.forEach((result, index) => {
      ...
      const cacheIcon = result.cache_status === 'HIT' ? '🚀' : result.cache_status === 'MISS' ? '⏱️' : '❌';
      console.log(`${index + 1}. ${status} ${cacheIcon} ${result.operation}`);
      console.log(`   Duration: ${result.duration_ms}ms | Cache: ${result.cache_status} | Size: ${(result.response_size_bytes / 1024).toFixed(1)}KB`);
    });
    ...
    if (summary.cache_efficiency > 80) {
      ...
      console.log('❌ Cache efficiency is poor (<60%) - investigate cache configuration');
    }
    ...
    if (summary.fastest_response_ms < 100) {
      ...
      console.log('❌ Cache response times are slow (>500ms) - investigate network/database performance');
    }
  }
