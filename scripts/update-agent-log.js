...
class AgentLogUpdater {
  ...
  saveLog(data) {
    try {
      ...
      const backupPath = path.join(
        this.backupDir,
        `mcp-agent-status-${timestamp}.json`,
      );
  ...
  updateMcpServer(serverName, status, error = null) {
    ...
    const workingServers = Object.values(log.mcpServers.servers).filter(
      (s) => s.status === "WORKING",
    ).length;
    ...
    log.mcpServers.status =
      workingServers === totalServers
        ? "HEALTHY"
        : workingServers > 0
          ? "DEGRADED"
          : "BROKEN";
  ...
  updateAgent(agentId, status, health = null, issues = []) {
    ...
    log.registeredAgents.agents[agentKey].lastActivity =
    ...
    const activeAgents = Object.values(log.registeredAgents.agents).filter(
      (a) => a.status === "ACTIVE",
    ).length;
    const completedAgents = Object.values(log.registeredAgents.agents).filter(
    ...
    log.registeredAgents.inactiveAgents =
      totalAgents - activeAgents - completedAgents;
  ...
  resolveSystemIssue(issueId, resolution) {
    ...
    log.systemHealth.criticalIssues = log.systemHealth.criticalIssues.filter(
      (issue) => {
        if (issue.id === issueId) {
    ...
    // Update overall health
    if (
      log.systemHealth.criticalIssues.length === 0 &&
      log.systemHealth.warnings.length === 0
    ) {
      log.systemHealth.overall = "HEALTHY";
  ...
  getStatusSummary() {
    ...
    return {
      ...
      pendingHandoffs: log.handoffs
        ? log.handoffs.filter((h) => h.status === "PENDING").length
        : 0,
    };
  }
}
...
if (require.main === module) {
  ...
  switch (action) {
    ...
    case "add-issue":
      ...
      if (
        updater.addSystemIssue(issueId, severity, description, impact, action)
      ) {
    ...
    default:
      ...
      console.log(
        "  update-mcp [server] [status] [error]     - Update MCP server status",
      );
      console.log(
        "  update-agent [id] [status] [health]      - Update agent status",
      );
      console.log(
        "  add-issue [id] [severity] [desc] [impact] [action] - Add system issue",
      );
      console.log(
        "  resolve-issue [id] [resolution]          - Resolve system issue",
      );
      console.log(
        "  handoff [from] [to] [reason] [task]      - Request agent handoff",
      );
      console.log(
        "  accept-handoff [agent] [handoffId]       - Accept handoff",
      );
      break;
  }
}
