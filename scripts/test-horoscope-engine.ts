...
class HoroscopeEngineTestSuite {
  ...
  async testPlanetaryPositions() {
    ...
    try {
      ...
      const validLongitudes = positions.every(
      ...
      const validSignAssignments = positions.every((p) =>
        validSigns.includes(p.sign),
      );
      this.addResult(
  ...
  async testDailyTransit() {
    ...
    try {
      ...
      this.addResult(
        ...
        !!dailyTransit.lunarPhase &&
          typeof dailyTransit.lunarPhase.illumination === "number",
      ...
      this.addResult(
        ...
        !!dailyTransit.cosmicWeather &&
          dailyTransit.cosmicWeather.focus.length > 0,
  ...
  async testPersonalizedHoroscope() {
    ...
    try {
      const horoscope =
        await this.transitEngine.generatePersonalizedHoroscope(sampleBirthData);
  ...
  async testTransitCalculation() {
    ...
    try {
      const transits =
        await this.transitEngine.calculateTransits(sampleBirthData);
      ...
      if (transits.length > 0) {
        ...
        const validAspectTypes = transits.every((t) =>
          validAspects.includes(t.aspect),
        );
        this.addResult(
          "Aspect Types",
          validAspectTypes,
  ...
  async testFallbackHandling() {
    ...
    try {
      ...
      const horoscope =
        await this.transitEngine.generatePersonalizedHoroscope(
          invalidBirthData,
        );
