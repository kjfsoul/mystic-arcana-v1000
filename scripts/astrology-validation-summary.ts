...
class AstrologyValidationSummary {
  async generateSummary(): Promise<ValidationSummary> {
    console.log('📊 Generating Comprehensive Astrology Engine Validation Summary');
    console.log('===============================================================');
  ...
  private async validateAstronomicalAccuracy(summary: ValidationSummary): Promise<void> {
    ...
    summary.astronomical.planetaryAccuracy = (accuracyScores.reduce((a, b) => a + b, 0) / accuracyScores.length) * 100;
    
    console.log(`   ✅ Julian Day: ${julianDay.toFixed(4)} (${(jdAccuracy * 100).toFixed(2)}% accurate)`);
    console.log(`   ✅ Sun: ${sunDegree.toFixed(1)}° ${sunSign} ${summary.astronomical.sunPosition.accurate ? '🎯' : '⚠️'}`);
    console.log(`   ✅ Moon: ${moonDegree.toFixed(1)}° ${moonSign}`);
  }

  private async validateSystemIntegrations(summary: ValidationSummary): Promise<void> {
    ...
    summary.systems.swissEphemeris = {
      ...
      status: swissInitialized ? 'Swiss Ephemeris Available' : 'Fallback Mode (Enhanced Calculations)'
    };
    ...
    console.log(`   ✅ API Endpoints: ${Object.values(summary.systems.apiEndpoints).filter(Boolean).length}/4 functional`);
    console.log(`   ✅ Components: ${Object.values(summary.systems.components).filter(Boolean).length}/3 functional`);
  }
  ...
  private calculateOverallScores(summary: ValidationSummary): void {
    ...
    const accuracyComponents = [
      ...
      (Object.values(summary.systems.apiEndpoints).filter(Boolean).length / 4) * 100 * 0.2, // 20% weight
  ...
  displaySummary(summary: ValidationSummary): void {
    ...
    console.log(`📅 Validated: ${new Date(summary.overall.timestamp).toLocaleString()}`);
    console.log('');
    
    // Astronomical Accuracy
    console.log('🔬 ASTRONOMICAL ACCURACY:');
    console.log(`   Julian Day Calculation: ${summary.astronomical.julianDayAccuracy.toFixed(2)}%`);
    console.log(`   Sun Position: ${summary.astronomical.sunPosition.degree.toFixed(1)}° ${summary.astronomical.sunPosition.sign} ${summary.astronomical.sunPosition.accurate ? '✅' : '⚠️'}`);
    console.log(`   Moon Position: ${summary.astronomical.moonPosition.degree.toFixed(1)}° ${summary.astronomical.moonPosition.sign}`);
    console.log(`   Overall Planetary Accuracy: ${summary.astronomical.planetaryAccuracy.toFixed(1)}%`);
    console.log('');
    ...
    console.log(`   Swiss Ephemeris: ${summary.systems.swissEphemeris.status}`);
    console.log(`   API Endpoints: ${Object.values(summary.systems.apiEndpoints).filter(Boolean).length}/4 functional`);
    console.log(`   UI Components: ${Object.values(summary.systems.components).filter(Boolean).length}/3 functional`);
    console.log('');
    
    // Performance Metrics
    console.log('⚡ PERFORMANCE METRICS:');
    console.log(`   Average Calculation Time: ${summary.performance.averageCalculationTime.toFixed(1)}ms`);
    console.log(`   Maximum Calculation Time: ${summary.performance.maxCalculationTime}ms`);
    console.log(`   Throughput: ${summary.performance.throughput}`);
    ...
    console.log(`   Real-time Calculations: ${features.realTimeCalculations ? '✅' : '❌'}`);
    console.log(`   Multi-planet Support: ${features.multiPlanetSupport ? '✅' : '❌'}`);
    console.log(`   Zodiac Accuracy: ${features.zodiacAccuracy ? '✅' : '❌'}`);
    console.log(`   House Calculations: ${features.houseCalculations ? '✅' : '⚠️  Simplified'}`);
    console.log(`   Transit Support: ${features.transitSupport ? '✅' : '❌'}`);
    console.log(`   Compatibility Analysis: ${features.compatibilityAnalysis ? '✅' : '❌'}`);
    console.log('');
    ...
    console.log(`   Sun Sign: ${summary.astronomical.sunPosition.sign} (${summary.astronomical.sunPosition.accurate ? 'Historically Accurate' : 'Needs Verification'})`);
    console.log(`   Moon Sign: ${summary.astronomical.moonPosition.sign}`);
    console.log(`   Astrological Profile: Perfect for theoretical physicist! 🧠⚡`);
    console.log('');
    ...
    console.log('   curl http://localhost:3000/api/astrology/birth-chart  # API test');
  }
}
