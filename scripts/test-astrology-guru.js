...
async function testAstrologyGuruIntegration() {
  ...
  try {
    ...
    console.log(`- Techniques: ${Object.keys(knowledgePool.techniques).length}`);
    ...
    if (aries && leoCompat && aries.compatibility && leoCompat.compatibility) {
      ...
      console.log('✅ Aries-Leo Compatibility:', isGoodMatch ? 'Excellent' : 'Challenging');
      console.log('- Aries Best Matches:', ariesBestMatches.join(', '));
      console.log('- Leo Best Matches:', leoCompat.compatibility.bestMatches?.join(', ') || 'Not specified');
    }

    console.log('\\n🎉 AstrologyGuru Knowledge Integration Test Complete!');
    console.log('✅ All knowledge systems are properly structured and accessible');
