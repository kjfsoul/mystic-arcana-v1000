...
class KnowledgePoolValidator {
  ...
  async validateMigrationSchema(): Promise<void> {
    ...
    try {
      const migrationPath = path.join(projectRoot, 'supabase/migrations/20250729_create_knowledge_pool.sql');
      ...
      const hasFunctions = migrationContent.includes('find_similar_interpretations') && 
  ...
  async validateIngestionScript(): Promise<void> {
    ...
    try {
      const scriptPath = path.join(projectRoot, 'scripts/ingest-knowledge-pool.ts');
      ...
      for (const func of requiredFunctions) {
        const hasFunction = scriptContent.includes(`async function ${func}`) || 
                           scriptContent.includes(`function ${func}`);
        this.addResult(
          `Function ${func}`,
          hasFunction,
          hasFunction ? 'Function implementation found' : 'Function implementation missing'
        );
      }
      ...
      const hasBatchProcessing = scriptContent.includes('batchSize') && 
                               scriptContent.includes('slice(');
      this.addResult(
        'Batch Processing',
        hasBatchProcessing,
        hasBatchProcessing ? 'Batch processing implemented' : 'Batch processing missing'
      );
      
      // Check for Supabase integration
      const hasSupabaseIntegration = scriptContent.includes('createClient') && 
                                   scriptContent.includes('from(') &&
                                   scriptContent.includes('insert(');
      this.addResult(
        'Supabase Integration',
        hasSupabaseIntegration,
        hasSupabaseIntegration ? 'Supabase integration found' : 'Supabase integration missing'
      );
  ...
  async validateSourceData(): Promise<void> {
    ...
    try {
      ...
      const tarotPath = path.join(projectRoot, 'public/data/catalogs/tarot-images.json');
      ...
      if (tarotData?.cards?.length > 0) {
        ...
        const hasRequiredFields = requiredCardFields.every(field => field in sampleCard);
        
        this.addResult(
          'Card Data Structure',
          hasRequiredFields,
          hasRequiredFields ? 'Cards have required fields' : 'Cards missing required fields',
          { sampleCard: Object.keys(sampleCard) }
        );
        
        // Check for Major and Minor Arcana
        const majorArcana = tarotData.cards.filter(c => c.arcana?.toLowerCase().includes('major'));
        const minorArcana = tarotData.cards.filter(c => c.arcana?.toLowerCase().includes('minor'));
      ...
      const enrichmentPath = path.join(projectRoot, 'docs/database_tarot/___TAROT AND ASTROLOGY DAILY DATABASE ENRICHMENT__.md');
      try {
        const enrichmentContent = await fs.readFile(enrichmentPath, 'utf-8');
        const hasStructuredFormat = enrichmentContent.includes('Database Focus') && 
                                   enrichmentContent.includes('Personalization Hook');
        
        this.addResult(
          'Enrichment Document',
          hasStructuredFormat,
          hasStructuredFormat ? 'Structured enrichment format found' : 'Enrichment format incomplete'
        );
      } catch {
        this.addResult(
          'Enrichment Document',
      ...
      const astroPath = path.join(projectRoot, 'docs/social_output/ASTROLOGICAL SOURCE BLOCK.md');
      try {
        const astroContent = await fs.readFile(astroPath, 'utf-8');
        const hasAstroData = astroContent.includes('Moon:') && astroContent.includes('Sun:');
        
        this.addResult(
          'Astrological Source',
          hasAstroData,
          hasAstroData ? 'Astrological data structure found' : 'Astrological data incomplete'
        );
      } catch {
        this.addResult(
          'Astrological Source',
  ...
  async validateDataIntegrity(): Promise<void> {
    ...
    try {
      ...
      const hasSpreadConfigs = scriptContent.includes('SPREAD_CONFIGURATIONS') &&
      ...
      this.addResult(
        ...
        hasSpreadConfigs,
        hasSpreadConfigs ? 'All spread types configured' : 'Spread configurations incomplete'
      );
      
      // Check personalization logic
      const hasPersonalization = scriptContent.includes('generatePersonalizationHooks') &&
      ...
      this.addResult(
        ...
        hasPersonalization,
        hasPersonalization ? 'Personalization hooks implemented' : 'Personalization logic missing'
      );
      
      // Check data transformation functions
      const hasTransformations = scriptContent.includes('generateContextualInterpretation') &&
      ...
      this.addResult(
        ...
        hasTransformations,
        hasTransformations ? 'Data transformation functions found' : 'Data transformation functions missing'
      );
      
      // Validate expected record counts
      const expectedTarotRecords = 78 * 5; // 78 cards × 5 spread types (approximate)
      const recordCountMention = scriptContent.includes('batchSize') && 
                                scriptContent.includes('totalInserted');
      
      this.addResult(
        'Record Count Tracking',
        recordCountMention,
        recordCountMention ? 'Record counting implemented' : 'Record counting missing'
      );
  ...
  printSummary(): void {
    ...
    console.log('• Card Combinations: Meaningful pairings for complex readings');
