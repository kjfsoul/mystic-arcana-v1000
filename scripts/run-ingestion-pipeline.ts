...
class IngestionPipeline {
      ...
      for (let i = 0; i < targetUrls.length; i++) {
        ...
        try {
          ...
          if (
            ...
            (await this.cardExistsInDatabase(url))
          ) {
          ...
          console.log(
            `  ✅ Extracted: ${extraction.cardName} (Quality: ${extraction.qualityScore.toFixed(1)}/10)`,
          );
          console.log(
            `  📝 Meanings: ${Object.keys(extraction.meanings).join(", ")}`,
          );
          console.log(
            `  🏷️ Keywords: ${extraction.keywords.slice(0, 5).join(", ")}${extraction.keywords.length > 5 ? "..." : ""}`,
          );
        ...
        if (i < targetUrls.length - 1) {
          console.log(
            `  ⏳ Waiting ${this.config.delayBetweenRequests}ms before next request...`,
          );
  ...
  private getTargetUrls(): string[] {
    ...
    if (this.config.sourceFilter) {
      const sourceKey = this.config.sourceFilter
        .toLowerCase()
        .replace(/[^a-z-]/g, "-");
      const source =
        this.TARGET_SOURCES[sourceKey as keyof typeof this.TARGET_SOURCES];

      if (!source) {
        throw new Error(
          `Unknown source: ${this.config.sourceFilter}. Available sources: ${Object.keys(this.TARGET_SOURCES).join(", ")}`,
        );
      }

      allUrls = source.urls;
      console.log(
        `📌 Filtering to source: ${source.name} (${allUrls.length} cards)`,
      );
    } else {
      // Use all sources
      allUrls = Object.values(this.TARGET_SOURCES).flatMap(
        (source) => source.urls,
      );
    ...
    if (this.config.cardFilter && this.config.cardFilter.length > 0) {
      const cardFilters = this.config.cardFilter.map((card) =>
        card.toLowerCase().replace(/[^a-z]/g, ""),
      );
      ...
      console.log(
        `📌 Filtering to specific cards: ${this.config.cardFilter.join(", ")} (${allUrls.length} URLs)`,
      );
    }
  ...
  private async cardExistsInDatabase(url: string): Promise<boolean> {
    try {
      ...
      if (error) {
        console.warn(
          `Warning: Could not check if card exists: ${error.message}`,
        );
        return false;
      }
  ...
  private logResults(results: IngestionResults): void {
    ...
    console.log(
      `⏱️ Processing Time: ${(results.processingTimeMs || 0) / 1000}s`,
    );
    console.log(
      `📈 Success Rate: ${results.totalAttempted > 0 ? ((results.totalSuccessful / results.totalAttempted) * 100).toFixed(1) : 0}%`,
    );
    ...
    if (results.extractions.length > 0) {
      ...
      results.extractions.forEach((extraction, index) => {
        console.log(
          `  ${index + 1}. ${extraction.cardName} (Quality: ${extraction.qualityScore.toFixed(1)}/10)`,
        );
      });
    }
  }
  ...
  private generateLabyrinthosUrls(): string[] {
    ...
    return majorArcana.map(
      (card) => `${baseUrl}/${card}-meaning-major-arcana-tarot-card-meanings`,
    );
  }
  ...
  async crawlAndExtractTarot(url: string): Promise<TarotCardExtraction> {
    ...
    try {
      ...
      throw new Error(
        `Extraction failed for ${url}: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  async saveToKnowledgeGraph(
    ...
    try {
      ...
      throw new Error(
        `Failed to save to Knowledge Graph: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private isTarotRelevant(word: string): boolean {
    ...
    return (
      tarotWords.includes(word) || (word.length > 3 && /^[a-z]+$/.test(word))
    );
  }
