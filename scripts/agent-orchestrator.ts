...
class AgentOrchestrator extends EventEmitter {
    ...
    try {
      ...
      for (const [agentId, agent] of this.agents) {
        if (registry.agents[agentId]) {
          ...
          registry.agents[agentId].last_active = agent.last_heartbeat || new Date().toISOString();
        }
      }
      ...
      this.log(`Error updating registry: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private getSystemStatus(): string {
    const activeCount = Array.from(this.agents.values()).filter(a => a.status === 'active').length;
    const failedCount = Array.from(this.agents.values()).filter(a => a.status === 'failed').length;
  ...
  async loadAgents() {
    try {
      ...
      this.log(`Error loading agents: ${error instanceof Error ? error.message : 'Unknown error'}`);
      throw error;
    }
  }
  ...
  async spawnAgent(agentId: string): Promise<boolean> {
    ...
    try {
      ...
      agentProcess.on('exit', (code) => {
        ...
        if (code !== 0 && !this.isShuttingDown && (agent.restart_count || 0) < 3) {
          agent.restart_count = (agent.restart_count || 0) + 1;
          this.log(`Restarting agent ${agentId} (attempt ${agent.restart_count})`);
          setTimeout(() => this.spawnAgent(agentId), 5000);
        }
      });

      this.log(`Agent ${agent.name} spawned successfully (PID: ${agentProcess.pid})`);
      return true;

    } catch (error) {
      this.log(`Failed to spawn agent ${agentId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      agent.status = 'failed';
      return false;
    }
  }
...
async function main() {
  ...
  try {
    ...
    console.error('❌ Failed to start agent orchestrator:', error instanceof Error ? error.message : 'Unknown error');
    process.exit(1);
  }
}
