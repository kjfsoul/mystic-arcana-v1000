...
class ProgressiveRevealTester {
  ...
  private async testInitialEngagementLevel(): Promise<void> {
    ...
    try {
      ...
      const result = await this.personaLearner.checkAndIncrementEngagementLevel(
        this.testUserId,
      );
  ...
  private async testEngagementMetricsCalculation(): Promise<void> {
    ...
    try {
      ...
      const analysis = await this.personaLearner.getEngagementAnalysis(
        this.testUserId,
      );
  ...
  private async testProgressiveLevelUp(): Promise<void> {
    ...
    try {
      ...
      for (const step of simulationSteps) {
        await this.createMockMemoryData(
          step.readings,
          step.turns,
          step.questions,
        );
        const result =
          await this.personaLearner.checkAndIncrementEngagementLevel(
            this.testUserId,
          );
      ...
      const validProgression = levelProgression.every(
        (p) => p.achieved >= p.expected,
      );
  ...
  private async testEngagementAnalysis(): Promise<void> {
    ...
    try {
      ...
      const hasValidProgress =
        analysis.progressToNext !== undefined &&
  ...
  private async testEdgeCases(): Promise<void> {
    ...
    try {
      ...
      totalCount++;
      const noMemoriesResult =
        await this.personaLearner.checkAndIncrementEngagementLevel(
          "user_with_no_memories",
        );
  ...
  private async createMockMemoryData(
    ...
    const userId = userIdSuffix
      ? `${this.testUserId}_${userIdSuffix}`
      : this.testUserId;
    ...
    console.log(
      `Created mock memory data: ${readings} readings, ${conversationTurns} turns, ${questions} questions for user ${userId}`,
    );
  }
