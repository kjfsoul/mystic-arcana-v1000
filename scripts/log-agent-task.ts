...
if (args.length < 4) {
  console.error('Usage: log-agent-task.ts <agent_id> <agent_name> <task_type> <task_description> [status] [error]');
  process.exit(1);
}

const [agentId, agentName, taskType, taskDescription, status = 'completed', error] = args;
