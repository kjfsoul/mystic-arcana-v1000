...
async function validateEphemerisAccuracy() {
  ...
  try {
      ...
      if (calculated) {
        ...
        if (reference.retrograde !== undefined) {
          const retrogradeMatch =
            calculated.isRetrograde === reference.retrograde;
          console.log(
            `          | Retrograde: ${calculated.isRetrograde ? "Yes" : "No"} | ${reference.retrograde ? "Yes" : "No"} |       | ${retrogradeMatch ? "✅" : "❌"}`,
          );
        ...
        console.log(
          `${planetName.padEnd(9)} | Not found  | ${reference.longitude.toFixed(1)}° ${reference.sign.slice(0, 3)} |       | ❌`,
        );
        totalTests++;
      }
    }
    ...
    if (passedTests === totalTests) {
      ...
      console.log(
        "⚠️ Some positions show minor deviations from reference data.",
      );
      console.log(
        "📝 Note: Small differences (<2°) are acceptable for general astrology.",
      );
      console.log(
        "📝 Our fallback calculations provide good approximations when Swiss Ephemeris is unavailable.",
      );
    }
    ...
    console.log(
      `  Sun in Pisces (March 14): ${sun?.sign === "Pisces" ? "✅" : "❌"}`,
    );
    ...
    console.log(
      `  Moon daily motion (~13°/day): ${moonSpeedRealistic ? "✅" : "❌"} (${moon?.speed.toFixed(2)}°/day)`,
    );

    // Retrograde detection
    const retrogradeCount = chart.planets.filter((p) => p.isRetrograde).length;
    console.log(
      `  Retrograde planets detected: ${retrogradeCount > 0 ? "✅" : "❌"} (${retrogradeCount} planets)`,
    );

    // House system
    console.log(
      `  House system functional: ${chart.houses.length === 12 ? "✅" : "❌"}`,
    );
  } catch (error) {
