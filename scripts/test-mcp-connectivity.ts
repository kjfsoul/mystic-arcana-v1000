...
async function testMCPConnectivity(serverName: string, serverConfig: MCPServer): Promise<MCPServer> {
  ...
  try {
    ...
    const responseTime = Date.now() - startTime;
    console.log(`[MCP] ❌ ${serverName} connection failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
...
async function main() {
  ...
  try {
    ...
    for (const [serverName, serverConfig] of Object.entries(registry.mcp_servers)) {
    ...
    console.log(`📊 Overall: ${onlineCount}/${Object.keys(registry.mcp_servers).length} servers operational`);
    ...
    const activityLogPath = join(process.cwd(), 'logs', 'agent-activity', `${new Date().toISOString().split('T')[0]}.log`);
    
    try {
      const fs = require('fs');
      fs.appendFileSync(activityLogPath, logEntry + '\n');
    } catch (logError) {
      console.warn('[MCP] Warning: Could not write to activity log:', logError instanceof Error ? logError.message : 'Unknown error');
    }
    ...
    if (onlineCount === 0) {
      ...
      console.log('3. Explore open-source alternatives at glama.ai/settings/mcp/servers');
    ...
    console.error('[MCP] ❌ Connectivity test failed:', error instanceof Error ? error.message : 'Unknown error');
    ...
    try {
      ...
      console.warn('[MCP] Warning: Could not write to error log:', logError instanceof Error ? logError.message : 'Unknown error');
    }
