...
import { ConversationState, ConversationTurn, ConversationOption, InteractiveQuestion, SophiaReading } from '../src/agents/sophia';
...
class TestSophiaAgent {
  async processReadingTurn(
    ...
    const mockTurn: ConversationTurn = {
      ...
      interactiveQuestion: newState === ConversationState.ASKING_QUESTION ? mockQuestion : undefined,
      revealedCard: cards && cards.length > 0 ? {
        card: cards[0],
        interpretation: `The ${cards[0].name} brings wisdom and guidance to your path.`
      } : undefined,
      finalReading: newState === ConversationState.COMPLETE ? {
        id: `test_reading_${Date.now()}`,
        narrative: 'Your cards reveal a beautiful journey ahead.',
        card_interpretations: cards ? cards.map(card => ({
...
class ConversationalFlowTester {
  ...
  private async testConversationStateMachine(): Promise<void> {
    ...
    try {
      ...
      const hasAllStates = expectedStates.every(state => states.includes(state as ConversationState));
  ...
  private async testInteractiveLearningHooks(): Promise<void> {
    ...
    try {
      ...
      this.results.push({
        ...
        passed: true,
        details: 'Successfully logged conversation turn, user response, and question response',
        execution_time: Date.now() - startTime
      });
  ...
  private async testResponsePatternAnalysis(): Promise<void> {
    ...
    try {
      ...
      const emotionalResponse = "I feel like this reading really speaks to my heart and soul";
      const analyticalResponse = "I think this interpretation makes logical sense because of the card positions";
      const conciseResponse = "Yes, I agree";
      const detailedResponse = "This reading provides a lot of insight into my current situation and I can see how the cards relate to what's happening in my life right now. The guidance feels very relevant and helpful.";
      ...
      this.results.push({
        ...
        passed: true,
        details: 'Successfully analyzed emotional and analytical response patterns',
        execution_time: Date.now() - startTime
      });
  ...
  private async testConversationCompletionFlow(): Promise<void> {
    ...
    try {
      ...
      while (currentState !== ConversationState.COMPLETE && turnCount < maxTurns) {
        const turn = await this.sophia.processReadingTurn(
  ...
  private async testConversationalErrorHandling(): Promise<void> {
    ...
    try {
      ...
      const passed = turn1 && turn1.sophiaDialogue && turn2 && turn2.sophiaDialogue;
