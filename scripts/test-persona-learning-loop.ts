...
class PersonaLearningTestSuite {
   ...
   */
  private async testAgentStructure(): Promise<{ passed: boolean; message: string; details?: any }> {
    try {
      // Test that agent classes can be imported and have expected methods
      const { SophiaAgent } = await import('../src/agents/sophia');
      const { PersonaLearnerAgent } = await import('../src/agents/PersonaLearner');
      
      // Check SophiaAgent structure
      const sophiaPrototype = SophiaAgent.prototype;
      const expectedSophiaMethods = ['getReading', 'healthCheck', 'getPersonality'];
      const missingSophiaMethods = expectedSophiaMethods.filter(method => !(method in sophiaPrototype));
      
      // Check PersonaLearnerAgent structure
      const personaPrototype = PersonaLearnerAgent.prototype;
      const expectedPersonaMethods = ['logInteraction', 'getPersonalizationRecommendations', 'healthCheck', 'getStats'];
      const missingPersonaMethods = expectedPersonaMethods.filter(method => !(method in personaPrototype));
      
      const missingMethods = [...missingSophiaMethods, ...missingPersonaMethods];
   ...
   */
  private async testKnowledgePoolConnectivity(): Promise<{ passed: boolean; message: string; details?: any }> {
    if (TEST_CONFIG.skipDbTests) {
      return {
        passed: true,
        message: 'Knowledge Pool connectivity test skipped (no database config)',
   ...
   */
  private async testSophiaReadingGeneration(): Promise<{ passed: boolean; message: string; details?: any }> {
    if (!this.sophiaAgent) {
      return {
        passed: true,
    ...
    try {
      ...
      const requiredFields = ['id', 'narrative', 'card_interpretations', 'overall_guidance', 'spiritual_insight', 'reader_signature'];
      ...
      const hasKnowledgePoolInfluence = reading.card_interpretations.some(interp => 
      ...
      return {
        ...
        details: {
          ...
          averageConfidence: reading.card_interpretations.reduce(
   ...
   */
  private async testPersonaLearnerLogging(): Promise<{ passed: boolean; message: string; details?: any }> {
    ...
    try {
      ...
      const recommendations = await this.personaLearner.getPersonalizationRecommendations(TEST_CONFIG.testUserId);
   ...
   */
  private async testAMemIntegration(): Promise<{ passed: boolean; message: string; details?: any }> {
    ...
    try {
      ...
      if (!isHealthy) {
        return {
          passed: false,
          message: 'PersonaLearner health check failed - a-mem system may be unavailable'
        };
      }
      ...
      return {
        passed: true,
        message: 'a-mem integration test completed (check logs for detailed results)',
        details: {
          readingId: reading.id,
   ...
   */
  private async testAgentOrchestrationPerformance(): Promise<{ passed: boolean; message: string; details?: any }> {
    if (!this.sophiaAgent || !this.personaLearner) {
      return {
        passed: true,
    ...
    try {
      ...
      const readingPromises = [1, 2, 3].map(async (i) => {
        ...
        await this.personaLearner.logInteraction(
          context.userId!,
          reading,
          { rating: 3 + i, session_notes: `Performance test ${i}` }
        );
   ...
   */
  private async testSophiaHealthCheck(): Promise<{ passed: boolean; message: string; details?: any }> {
    if (TEST_CONFIG.skipDbTests || !this.sophiaAgent) {
      return {
        passed: true,
        message: 'Sophia health check skipped (no database config or Next.js context)',
        details: { skipped: true, reason: TEST_CONFIG.skipDbTests ? 'no_db_config' : 'no_nextjs_context' }
      };
    }
  ...
  public async runAllTests(): Promise<void> {
    ...
    // Execute all tests
    await this.executeTest('Agent Class Structure Validation', () => this.testAgentStructure());
    await this.executeTest('Knowledge Pool Connectivity', () => this.testKnowledgePoolConnectivity());
    await this.executeTest('Sophia Reading Generation', () => this.testSophiaReadingGeneration());
    await this.executeTest('PersonaLearner Interaction Logging', () => this.testPersonaLearnerLogging());
    await this.executeTest('a-mem System Integration', () => this.testAMemIntegration());
    await this.executeTest('Agent Orchestration Performance', () => this.testAgentOrchestrationPerformance());
    await this.executeTest('Sophia Health Check', () => this.testSophiaHealthCheck());
  ...
  private printSummary(): void {
    ...
    console.log('\n🎯 KEY METRICS:');
    const knowledgePoolTest = this.results.find(r => r.testName.includes('Knowledge Pool'));
    if (knowledgePoolTest?.passed && knowledgePoolTest.details?.recordCount) {
      console.log(`   • Knowledge Pool Records: ${knowledgePoolTest.details.recordCount}`);
    }

    const sophiaTest = this.results.find(r => r.testName.includes('Sophia Reading'));
    if (sophiaTest?.passed && sophiaTest.details) {
      console.log(`   • Average Confidence Score: ${sophiaTest.details.averageConfidence?.toFixed(2) || 'N/A'}`);
      console.log(`   • Knowledge Pool Integration: ${sophiaTest.details.knowledgePoolInfluence ? 'Active' : 'Fallback'}`);
    }

    const personaTest = this.results.find(r => r.testName.includes('PersonaLearner'));
    if (personaTest?.passed && personaTest.details?.stats) {
      console.log(`   • Learning Events Logged: ${personaTest.details.stats.total_events}`);
      console.log(`   • Active User Profiles: ${personaTest.details.stats.total_users}`);
    }

    const perfTest = this.results.find(r => r.testName.includes('Performance'));
    if (perfTest?.passed && perfTest.details) {
      console.log(`   • Average Reading Generation Time: ${perfTest.details.averageTime?.toFixed(0)}ms`);
    }
    ...
    if (passRate === '100.0') {
      ...
      console.log('⚠️  PHASE 2 - PERSONA LEARNER ACTIVATION: 🟡 MOSTLY COMPLETED');
      console.log('   Core functionality working with minor issues to address.');
    } else {
      console.log('🚨 PHASE 2 - PERSONA LEARNER ACTIVATION: ❌ REQUIRES ATTENTION');
