...
class TarotAPITester {
  ...
  async testDrawEndpoint(): Promise<void> {
    ...
    const invalidCount = await this.makeRequest(
      "/api/tarot/draw",
      "POST",
      {
        count: 100,
    ...
    const invalidDeck = await this.makeRequest(
      "/api/tarot/draw",
      "POST",
      {
        count: 1,
  ...
  async testShuffleEndpoint(): Promise<void> {
    ...
    if (fisherYates.status === "PASS") {
      ...
      console.log(
        `   Entropy: ${fisherYates.data?.shuffleState?.entropy || 0}`,
      );
    } else {
    ...
    if (riffle.status === "PASS") {
      ...
      console.log(
        `   Top card: ${riffle.data?.preview?.topCard?.name || "N/A"}`,
      );
    } else {
  ...
  async testSaveReadingEndpoint(): Promise<void> {
    ...
    const saveReading = await this.makeRequest(
      "/api/tarot/save-reading",
      "POST",
      {
        userId: this.testUserId,
        ...
        interpretation:
          "Your journey shows progression from new beginnings to mastery.",
    ...
    const saveSingle = await this.makeRequest(
      "/api/tarot/save-reading",
      "POST",
      {
        userId: this.testUserId,
        ...
        interpretation:
          "The Star brings hope and renewal to your current situation.",
        isPublic: false,
    ...
    const invalidUser = await this.makeRequest(
      "/api/tarot/save-reading",
      "POST",
      {
        ...
        spreadType: "single",
        cards: [
          { id: "test", name: "Test", position: "Test", isReversed: false },
        ],
        interpretation: "Test",
      },
      404,
    ...
    const missingFields = await this.makeRequest(
      "/api/tarot/save-reading",
      "POST",
      {
        userId: this.testUserId,
    ...
    const getStats = await this.makeRequest(
      `/api/tarot/save-reading?userId=${this.testUserId}`,
      "GET",
    );
    ...
    if (getStats.status === "PASS") {
      ...
      console.log(
        `   Total readings: ${getStats.data?.statistics?.totalReadings || 0}`,
      );
    } else {
  ...
  async testGetReadingEndpoint(): Promise<void> {
    ...
    const getUserReadings = await this.makeRequest(
      `/api/tarot/get-reading?userId=${this.testUserId}`,
      "GET",
    );
    ...
    if (getUserReadings.status === "PASS") {
      ...
      console.log(
        `   Readings found: ${getUserReadings.data?.readings?.length || 0}`,
      );
      console.log(
        `   Total count: ${getUserReadings.data?.pagination?.total || 0}`,
      );
    } else {
    ...
    if (getUserReadings.data?.readings?.length > 0) {
      ...
      const getSpecificReading = await this.makeRequest(
        `/api/tarot/get-reading?id=${readingId}`,
        "GET",
      );
      ...
      if (getSpecificReading.status === "PASS") {
        ...
        console.log(
          `   Reading spread: ${getSpecificReading.data?.reading?.spreadType}`,
        );
      } else {
    ...
    if (filterBySpread.status === "PASS") {
      ...
      console.log(
        `   Three-card readings: ${filterBySpread.data?.readings?.length || 0}`,
      );
    } else {
    ...
    const invalidId = await this.makeRequest(
      "/api/tarot/get-reading?id=invalid-id",
      "GET",
      undefined,
      404,
    );
  ...
  private async createTestUser(): Promise<void> {
    try {
      ...
      if (!response.ok) {
        console.log(
          "⚠️ Could not create test user - using existing user flows",
        );
      }
    } catch (error) {
  ...
  async testCompleteWorkflow(): Promise<void> {
    ...
    try {
      ...
      if (draw.status === "PASS" && draw.data?.cards?.length === 3) {
        saveResult = await this.makeRequest(
          "/api/tarot/save-reading",
          "POST",
          {
            userId: this.testUserId,
            ...
            interpretation:
              "Complete workflow test reading with shuffled cards.",
      ...
      if (
        shuffle.status === "PASS" &&
        draw.status === "PASS" &&
        saveResult?.status === "PASS" &&
        retrieveResult?.status === "PASS"
      ) {
  ...
  generateReport(): void {
    ...
    console.log("\n⚡ PERFORMANCE ANALYSIS:");
    const endpointPerformance = this.results.reduce(
      (acc, result) => {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  tester
    .runAllTests()
    .then(() => {
      const failedTests = tester["results"].filter(
        (r) => r.status === "FAIL",
      ).length;
