...
async function testDailyHoroscopes() {
  ...
  try {
    ...
    console.log(
      "🔮 Key Transits:",
      horoscopeData.metadata.keyTransits.join(", "),
    );
    ...
    horoscopeData.horoscopes.slice(0, 3).forEach((horoscope) => {
      ...
      console.log(
        "⭐ Ratings:",
        `Overall: ${horoscope.rating.overall}, Love: ${horoscope.rating.love}, Career: ${horoscope.rating.career}, Health: ${horoscope.rating.health}`,
      );
    ...
    console.log(
      `✅ Successfully generated ${testResult.horoscopes.length} horoscopes for ${specificDate}`,
    );
    ...
    if (validationResults.isValid) {
      ...
      console.log(
        `   - All required fields present: ${validationResults.requiredFieldsPresent}`,
      );
      console.log(
        `   - Rating system functional: ${validationResults.ratingsValid}`,
      );
      console.log(
        `   - Keywords populated: ${validationResults.keywordsPopulated}`,
      );
    } else {
...
function validateN8nStructure(horoscopeData) {
  ...
  try {
    ...
    if (
      !horoscopeData.date ||
      !horoscopeData.horoscopes ||
      !Array.isArray(horoscopeData.horoscopes)
    ) {
    ...
    horoscopeData.horoscopes.forEach((horoscope, index) => {
      ...
      requiredFields.forEach((field) => {
        if (!horoscope.hasOwnProperty(field)) {
          validation.errors.push(
            `Missing field '${field}' in ${horoscope.sign || "unknown sign"}`,
          );
      ...
      if (horoscope.rating) {
        ...
        ratingFields.forEach((ratingField) => {
          if (
            typeof horoscope.rating[ratingField] !== "number" ||
            horoscope.rating[ratingField] < 1 ||
            horoscope.rating[ratingField] > 5
          ) {
            validation.errors.push(
              `Invalid rating '${ratingField}' in ${horoscope.sign}`,
            );
      ...
      if (
        !Array.isArray(horoscope.keywords) ||
        horoscope.keywords.length === 0
      ) {
        validation.errors.push(
          `Empty or invalid keywords array in ${horoscope.sign}`,
        );
  ...
  testDailyHoroscopes()
    .then((result) => {
      if (result.success) {
        ...
        console.log(
          `✅ Validation Status: ${result.validationResults.isValid ? "PASSED" : "FAILED"}`,
        );
