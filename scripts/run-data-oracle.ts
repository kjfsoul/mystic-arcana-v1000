...
async function main() {
  ...
  try {
    ...
    if (!isHealthy) {
      throw new Error('DataOracle health check failed - database connection issues');
    }
    ...
    console.log(`   Interpretations: ${initialStatus.knowledgeGraph.interpretations}`);
    console.log(`   Relationships: ${initialStatus.knowledgeGraph.relationships}\n`);
    ...
    console.log(`📝 Interpretations created: ${ingestionMetrics.interpretations_created}`);
    console.log(`🔗 Relationships created: ${ingestionMetrics.relationships_created}`);
    ...
    console.log(`🎯 Data quality score: ${ingestionMetrics.data_quality_score.toFixed(2)}/10`);
    console.log(`⚠️ Errors encountered: ${ingestionMetrics.errors_encountered}`);
    ...
    console.log(`   Interpretations: ${finalStatus.knowledgeGraph.interpretations}`);
    console.log(`   Relationships: ${finalStatus.knowledgeGraph.relationships}\n`);
    ...
    if (searchResults.length > 0) {
      console.log(`   Example: ${searchResults[0].name} - ${searchResults[0].archetypal_energy}`);
    }
    ...
    if (foolConcept) {
      ...
      if (conceptDetails.length > 0) {
        console.log(`   Sample interpretation: "${conceptDetails[0].primary_meaning}"`);
      }
    }
