...
async function testTarotAPI() {
  ...
  try {
    ...
    console.log(`✅ API Success: Loaded ${data.cards.length} cards from "${data.deck.name}"`);
    console.log(`📊 Stats: ${data.stats.majorArcana} Major + ${data.stats.minorArcana} Minor Arcana`);
    
    if (data.cards.length === 78) {
      console.log('🎉 Complete deck loaded successfully!');
      return true;
    } else {
      console.warn(`⚠️ Incomplete deck: Expected 78 cards, got ${data.cards.length}`);
      return false;
    }
