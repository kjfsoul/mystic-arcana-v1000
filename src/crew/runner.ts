...
class CrewRunner {
  ...
  private async executeTask(task: CrewTask): Promise<CrewResult> {
    ...
    try {
      ...
      switch (task.agent) {
        ...
        case "UIEnchanter":
          if (task.name === "generatePrompts") {
            result = await uiEnchanter.generateCardPrompts(
        ...
        case "CardWeaver":
          if (task.name === "generateCards") {
            result = await cardWeaver.generateCardData(
              task.params.blueprint,
              task.params.prompts,
            );
          }
        ...
        case "QualityGuardian":
          if (task.name === "validateDeck") {
            ...
            result = await qualityGuardian.storeValidatedDeck(
              task.params.deckId,
              task.params.cards,
            );
          }
      ...
      if (!result) {
        throw new Error(
          `Agent ${task.agent} returned no result for task ${task.name}`,
        );
      }
    ...
    try {
      ...
      switch (taskName) {
        case "generateCrewTarotDeck":
          results.push(
            ...(await this.executeCrewTarotDeckGeneration(operationData)),
          );
          break;
   ...
   */
  private async executeCrewTarotDeckGeneration(
    _params: any,
  ): Promise<CrewResult[]> {
    ...
    for (const task of tasks) {
      ...
      if (task.dependencies) {
        const missingDeps = task.dependencies.filter(
          (dep) => !results.find((r) => r.taskId === dep && r.success),
        );
      ...
      if (
        task.id === "store_metadata" ||
        task.id === "generate_prompts" ||
        task.id === "generate_cards"
      ) {
        const blueprintResult = results.find(
          (r) => r.taskId === "blueprint_generation",
        );
      ...
      if (task.id === "generate_cards") {
        const promptsResult = results.find(
          (r) => r.taskId === "generate_prompts",
        );
      ...
      if (
        !result.success &&
        ["blueprint_generation", "generate_cards", "validate_deck"].includes(
          task.id,
        )
      ) {
        await this.logToMemory({
   ...
   */
  private async validateExistingDeck(params: {
    cards: any[];
  }): Promise<CrewResult[]> {
    const task: CrewTask = {
  ...
  private async performHealthCheck(): Promise<CrewResult> {
    ...
    try {
      ...
      const agentTests = [
        {
          name: "DataOracle",
          test: () => dataOracle.generateThematicBlueprint(),
        },
        {
          name: "UIEnchanter",
          test: () => uiEnchanter.generateCardPrompts({}),
        },
        { name: "CardWeaver", test: () => cardWeaver.generateCardData({}, {}) },
        {
          name: "QualityGuardian",
          test: () => qualityGuardian.validateDeck([]),
        },
      ];
