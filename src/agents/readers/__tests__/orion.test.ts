...
jest.mock("@/lib/astrology/SwissEphemerisShim", () => ({
  SwissEphemerisCalculator: jest.fn().mockImplementation(() => ({
    ...
    calculateTransits: jest.fn().mockResolvedValue({
      transits: [{ planet: "jupiter", aspect: "trine", target: "mc", orb: 2 }],
    }),
  })),
}));
...
jest.mock("@/lib/astrology/interpretations/career", () => ({
  CareerInterpreter: {
    analyzeCareerPath: jest.fn().mockReturnValue({
      primaryPath:
        "Leadership and creative expression in entertainment or performance",
      strengths: [
      ...
      timing:
        "Jupiter's current influence suggests expansion and new opportunities",
      guidance:
        "Focus on aligning your career with your authentic self and creative expression",
...
describe("OrionReader", () => {
  ...
  describe("Persona Configuration", () => {
    ...
    it("should have mentor tone with tactical honesty", () => {
      ...
      expect(persona.tone.communication).toBe(
        "tactically honest with gentle authority",
      );
    });
    ...
    it("should have three intro messages", () => {
      ...
      expect(ORION_PERSONA.introMessages[2]).toContain(
  ...
  describe("Reading Generation", () => {
    ...
    it("should throw error when birth data is missing", async () => {
      ...
      await expect(
        orionReader.generateReading(requestWithoutBirthData),
  ...
  describe("Conversational Styles", () => {
    it("should have conversation styles for different triggers", () => {
      ...
      const careerUncertaintyStyle = persona.conversationalStyles.find(
        (style) => style.trigger === "career_uncertainty",
      );
    ...
    it("should have timing-specific conversation style", () => {
      ...
      const timingStyle = persona.conversationalStyles.find(
        (style) => style.trigger === "timing_questions",
      );
  ...
  describe("Summary Types", () => {
    it("should have career-focused summary template", () => {
      const persona = orionReader.getPersona();
      const careerSummary = persona.summaryTypes.find(
        (summary) => summary.type === SummaryTypeEnum.CAREER,
      );
