...
jest.mock('@/lib/astrology/SwissEphemerisShim', () => ({
  SwissEphemerisCalculator: jest.fn().mockImplementation(() => ({
    ...
    calculateTransits: jest.fn().mockResolvedValue({
      transits: [
        { planet: 'venus', aspect: 'trine', target: 'moon', orb: 3 }
      ]
    }),
  }))
}));
...
jest.mock('@/lib/astrology/interpretations/love', () => ({
  LoveInterpreter: {
    analyzeLoveProfile: jest.fn().mockReturnValue({
      relationshipStyle: 'Your Taurus Venus seeks love that feels like coming home - steady, sensual, and deeply rooted.',
      emotionalNeeds: ['Emotional security and understanding', 'Nurturing, protective love', 'Harmony and beauty'],
      loveLanguage: 'Physical touch and receiving gifts',
      compatibility: {
        bestMatches: ['Earth signs for stability', 'Water signs for emotional depth'],
        challenges: ['Fire signs may feel too impulsive'],
        growthAreas: ['Learning to appreciate different love languages']
      },
      timing: 'Venus is encouraging you to open your heart to new possibilities.',
      shadowWork: 'Your healing journey may involve exploring emotional walls or prioritizing status over connection.',
...
jest.mock('@/lib/astrology/compatibilityEngine', () => ({
  CompatibilityEngine: {
    analyzeSynastry: jest.fn().mockResolvedValue({
      ...
      advice: ['Trust the timing of your connection', 'Honor both individual paths'],
      connectionType: 'complementary',
      harmonies: [
        { person1Planet: 'venus', person2Planet: 'moon', aspect: 'trine', meaning: 'Beautiful emotional compatibility' }
      ],
      tensions: [],
      summary: 'Your connection reveals a harmonious partnership that brings out your best qualities.'
    })
  }
}));
...
jest.mock('@/lib/mem/memoryLogger', () => ({
  memoryLogger: {
    getUserInsights: jest.fn().mockResolvedValue([
      { trait: 'expressive', context: 'Shows emotional depth in journal entries', timestamp: '2024-01-01' },
      { trait: 'seeking-healing', context: 'Reflects on relationship patterns', timestamp: '2024-01-02' }
    ])
  }
}));
...
describe('LunaReader', () => {
  ...
  describe('Persona Configuration', () => {
    ...
    it('should have intuitive tone with compassionate communication', () => {
      ...
      expect(persona.tone.communication).toBe('metaphorical but clear, nurturing yet truthful');
    });
    ...
    it('should have love-focused expertise', () => {
      ...
      expect(persona.expertise.astrology?.specializations).toContain('compatibility');
    });
  ...
  describe('Individual Love Reading', () => {
    ...
    it('should throw error when birth data is missing', async () => {
      ...
      await expect(lunaReader.generateReading(requestWithoutBirthData))
    ...
    it('should integrate journal insights when available', async () => {
      ...
      expect(memoryLogger.getUserInsights).toHaveBeenCalledWith('user-with-journal', 20);
      
      // Should include guidance about journal patterns
      const hasJournalGuidance = response.interpretation.guidance.some(g => 
  ...
  describe('Compatibility Reading', () => {
    it('should generate synastry analysis when partner data provided', async () => {
      ...
      expect(response.interpretation.secondary).toContain('individual love style');
    });
    ...
    it('should provide relationship-specific follow-up questions', async () => {
      ...
      expect(response.followUpQuestions![0]).toContain('patterns from this relationship');
  ...
  describe('Conversational Styles', () => {
    it('should have healing wisdom for heartbreak', () => {
      ...
      const heartbreakStyle = persona.conversationalStyles
        .find(style => style.trigger === 'heartbreak_healing');
    ...
    it('should have intuitive clarity for relationship confusion', () => {
      ...
      const confusionStyle = persona.conversationalStyles
        .find(style => style.trigger === 'relationship_confusion');
    ...
    it('should have nurturing empowerment for self-love', () => {
      ...
      const selfLoveStyle = persona.conversationalStyles
        .find(style => style.trigger === 'self_love_journey');
  ...
  describe('Summary Types', () => {
    it('should have relationship summary template', () => {
      const persona = lunaReader.getPersona();
      const relationshipSummary = persona.summaryTypes
        .find(summary => summary.type === SummaryTypeEnum.RELATIONSHIP);
    ...
    it('should have spiritual summary template', () => {
      const persona = lunaReader.getPersona();
      const spiritualSummary = persona.summaryTypes
        .find(summary => summary.type === SummaryTypeEnum.SPIRITUAL);
