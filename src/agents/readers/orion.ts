...
import { CareerInterpreter, CareerInterpretation } from '@/lib/astrology/interpretations/career';
...
export const ORION_PERSONA: ReaderPersona = {
  ...
  description: 'Master of celestial timing and career guidance. Orion illuminates your professional path through the wisdom of birth charts, transits, and cosmic cycles.',
  tone: {
    primary: 'mentor',
    secondary: 'honest',
  ...
  summaryTypes: [
    {
      type: 'career',
      template: 'Based on your {MC_SIGN} Midheaven and {TENTH_HOUSE} influences, your career path emphasizes {PRIMARY_THEMES}. The current {MAJOR_TRANSIT} transit suggests {TIMING_GUIDANCE}. Focus on {ACTION_STEPS} over the next {TIME_FRAME}.',
      focusAreas: ['professional identity', 'timing', 'reputation', 'achievement']
    },
    {
      type: 'personal_growth',
      template: 'Your {SUN_HOUSE} Sun placement reveals your core purpose lies in {PURPOSE_THEME}. {SATURN_INFLUENCE} indicates the discipline needed for mastery. Embrace {GROWTH_AREAS} to align with your authentic professional self.',
      focusAreas: ['purpose', 'discipline', 'mastery', 'authentic expression']
    },
    {
      type: 'spiritual',
      template: 'The cosmos calls you to integrate {SPIRITUAL_THEMES} into your professional life. Your {JUPITER_INFLUENCE} shows opportunities for meaningful expansion. Trust the timing of {COSMIC_CYCLES} as you evolve professionally.',
      focusAreas: ['higher purpose', 'expansion', 'trust', 'evolution']
    }
  ],
  expertise: {
    primaryFocus: ['career guidance', 'professional timing', 'life purpose', 'cosmic cycles'],
    techniques: ['birth chart analysis', 'transit timing', 'progressed charts', 'solar returns'],
    strengths: ['career transitions', 'professional development', 'leadership potential', 'business timing'],
    astrology: {
      houses: [2, 6, 10], // Career-focused houses
      planets: [Planet.SUN, Planet.SATURN, Planet.JUPITER, Planet.MARS, 'mc'], // Key career planets
      aspects: [AspectType.CONJUNCTION, AspectType.SQUARE, AspectType.TRINE, AspectType.OPPOSITION], // Corrected to use AspectType enum
      transitTypes: ['saturn returns', 'jupiter cycles', 'solar arcs', 'progressions'],
      specializations: ['career', 'purpose', 'timing', 'leadership', 'business cycles']
    }
  },
...
export class OrionReader {
  ...
  async generateReading(request: ReadingRequest): Promise<ReadingResponse> {
    ...
    return {
      ...
      interpretation: {
        ...
        guidance: [careerAnalysis.guidance, ...this.generateTimingGuidance(careerAnalysis.timing)],
  ...
  private generateCareerInterpretation(
    ...
    const tenthHousePlanets = birthChart.planets.filter((p: any) => p.house === 10);
    ...
    const secondary = `Your core strengths in the professional realm include: ${careerAnalysis.strengths.join(', ')}. ` +
  ...
  private getPlanetaryCareerTheme(planet: string): string {
    ...
    return themes[planet as keyof typeof themes] || 'focused dedication and skill development';
  }
  ...
  private generateFollowUpQuestions(careerAnalysis: CareerInterpretation): string[] {
    const questions = [
    ...
    if (careerAnalysis.challenges.length > 0) {
      questions.push("Which of these professional challenges feels most pressing for you right now?");
    }
  ...
  private generateActionSteps(careerAnalysis: CareerInterpretation): string[] {
    ...
    if (careerAnalysis.timing.includes('pivotal')) {
      baseSteps.push("Prepare for significant career restructuring over the next 1-2 years");
    } else if (careerAnalysis.timing.includes('expansion')) {
      baseSteps.push("Actively pursue growth opportunities over the next 6-12 months");
    }
  ...
  getSpecializationKeywords(): string[] {
    return [
      'career guidance', 'professional timing', 'life purpose', 'cosmic cycles',
      'birth chart', 'midheaven', 'career houses', 'saturn return', 'jupiter cycle'
    ];
  }
}
