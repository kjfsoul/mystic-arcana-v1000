...
import {
  LoveInterpreter,
  LoveInterpretation,
} from "@/lib/astrology/interpretations/love";
import {
  CompatibilityEngine,
  SynastryAnalysis,
} from "@/lib/astrology/compatibilityEngine";
...
export const LUNA_PERSONA: ReaderPersona = {
  ...
  description:
    "Guardian of the heart's mysteries. Luna illuminates the sacred patterns of love, emotional healing, and soul connections through the gentle wisdom of Venus, Moon, and synastry.",
  tone: {
    primary: "intuitive",
  ...
  summaryTypes: [
    ...
    },
    {
      type: "spiritual",
      template:
        "Your soul chose this heart to learn about {SPIRITUAL_LESSON}. The sacred wound of {VENUS_CHALLENGE} is also your greatest gift. Trust the {MOON_WISDOM} guidance that flows through your emotional intuition.",
      focusAreas: [
        "soul lessons",
        "sacred wounds",
        "spiritual love",
...
export class LunaReader {
  ...
  async generateReading(request: ReadingRequest): Promise<ReadingResponse> {
    ...
    if (request.userId) {
      const userInsights = await memoryLogger.getUserInsights(
        request.userId,
        20,
      );
      const emotionalInsights = userInsights
        .map(
          (insight) =>
            EmotionalAnalyzer.analyzeText(insight.context || insight.trait)
              .themes,
        )
        .flat();
    ...
    if (
      request.type === ReadingRequestType.SYNASTRY &&
      request.partnerBirthData
    ) {
      const partnerChart = await this.ephemeris.calculateFullChart(
        request.partnerBirthData,
      );
    ...
    const introMessage = this.selectIntroMessage(
      request.focus,
  ...
  private generateLoveInterpretation(
    ...
    if (compatibilityAnalysis) {
      ...
      primary = this.generateCompatibilityNarrative(
        compatibilityAnalysis,
        loveAnalysis,
      );
      secondary =
        `Your individual love style: ${loveAnalysis.relationshipStyle}. ` +
        `Together, you are learning: ${compatibilityAnalysis.advice[0]}`;
    } else {
      // Individual love reading
      primary = this.generatePersonalLoveNarrative(
        loveAnalysis,
        journalInsights,
      );
      secondary =
        `Your emotional needs whisper: ${loveAnalysis.emotionalNeeds[0]}. ` +
  ...
  private generateCompatibilityNarrative(
    ...
    const connectionPoetry = {
      ...
      complementary:
        "You are two notes that create harmony when played together",
      growth:
        "This connection is a sacred mirror, showing each of you what is ready to evolve",
      karmic:
        "Your hearts carry ancient memories of love and learning together",
      challenging:
        "This relationship is a tender teacher, asking for patience and conscious love",
    };

    const poetry = connectionPoetry[compatibility.connectionType];

    return (
      `${poetry}. With ${compatibility.overallCompatibility}% overall harmony, your connection reveals ` +
  ...
  private generateEmotionalGuidance(
    ...
    if (journalInsights && journalInsights.length > 0) {
      guidance.push(
        "Your recent emotional patterns suggest readiness for deeper self-compassion",
      );
    }
    ...
    if (loveAnalysis.compatibility.growthAreas.length > 0) {
      guidance.push(
        `Growth invitation: ${loveAnalysis.compatibility.growthAreas[0]}`,
      );
    }
  ...
  private generateFollowUpQuestions(
    ...
    questions.push(
      "What aspect of love feels most tender or protected in your heart right now?",
    );
