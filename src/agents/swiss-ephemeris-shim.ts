...
export class SwissEphemerisShimAgent extends Agent {
  ...
  async getPlanetaryPosition(request: CoordinateRequest): Promise<EphemerisResponse> {
    try {
      // Generate cache key
      ...
      try {
        ...
        console.warn(`Primary source ${this.config.primarySource} failed:`, primaryError);
