...
export class LunarTransitNarratorAgent extends Agent {
  ...
  private async loadAstrologyKnowledge(): Promise<void> {
    try {
      ...
      try {
        ...
        console.log(
          "LunarTransitNarrator: Loaded AstrologyGuru knowledge base successfully",
        );
      } catch (error) {
        console.warn(
          "LunarTransitNarrator: Using fallback knowledge base:",
  ...
  private loadFallbackKnowledge(): void {
    const signs = {
      ...
      taurus: {
        keywords: ["stability", "beauty", "persistence"],
        element: "earth",
        quality: "fixed",
      },
      gemini: {
      ...
      cancer: {
        keywords: ["emotion", "nurturing", "intuition"],
        element: "water",
        quality: "cardinal",
      },
      leo: {
      ...
      capricorn: {
        keywords: ["ambition", "structure", "responsibility"],
        element: "earth",
        quality: "cardinal",
      },
      aquarius: {
  ...
  async generateDailyHoroscopes(
    ...
    try {
      ...
      console.error(
        "LunarTransitNarratorAgent: Daily horoscope generation failed:",
        error,
      );

      // Return fallback horoscopes for all signs
      const fallbackHoroscopes = await Promise.all(
        this.zodiacSigns.map((sign) =>
          this.generateFallbackHoroscope(sign, date),
        ),
      );
  ...
  private async generateSignHoroscope(
    ...
    try {
      ...
      const horoscope = await this.synthesizeHoroscopeText(
        sign,
        influences,
        moonPhase,
        signData,
      ...
      const careerInsight = this.generateCareerInsight(
        sign,
        influences,
        signData,
      );
      const loveInsight = this.generateLoveInsight(
        sign,
        influences,
        moonPhase,
        signData,
  ...
  async generatePersonalizedReading(
    ...
    try {
      ...
      const personalThemes = this.generatePersonalThemes(
        natalMoon,
        currentTransits,
      );

      // Create personalized guidance
      const emotionalForecast = await this.generateEmotionalForecast(
        natalMoon,
        currentTransits,
      );
      const relationshipInsights = await this.generateRelationshipInsights(
        natalMoon,
        currentTransits,
      );
      ...
      const spiritualMessage = await this.generateSpiritualMessage(
        natalMoon,
        currentTransits,
      );
      ...
      console.error(
        "LunarTransitNarratorAgent: Personalized reading failed:",
        error,
      );
  ...
  async analyzeMoonPhasePatterns(
    startDate: string,
    endDate: string,
  ): Promise<any> {
    ...
    if (!startDate || !endDate) {
      throw new Error(
        "Start date and end date are required for moon phase pattern analysis",
      );
    }
    ...
    try {
      ...
      for (let i = 0; i < planetNames.length; i++) {
        for (let j = i + 1; j < planetNames.length; j++) {
          ...
          if (aspect) {
            transits.push(
              `${planet1.charAt(0).toUpperCase() + planet1.slice(1)} ${aspect} ${planet2.charAt(0).toUpperCase() + planet2.slice(1)}`,
            );
          }
        }
      }
      ...
      planetNames.forEach((planetName) => {
        if (
          planets[planetName].retrograde &&
          planetName !== "sun" &&
          planetName !== "moon"
        ) {
          transits.push(
            `${planetName.charAt(0).toUpperCase() + planetName.slice(1)} Retrograde`,
          );
        }
      });
      ...
      Object.entries(planets).forEach(
        ([planetName, planetData]: [string, any]) => {
          ...
          if (influence.type === "ruling") influences.ruling.push(influence);
            ...
            influences.supportive.push(influence);
  ...
  private calculatePlanetSignInfluence(
    planetName: string,
    planetData: any,
    sign: string,
    _signData: any,
    ...
    try {
      ...
      if (
        planetInfo.rulerOf &&
        planetInfo.rulerOf.includes(sign.toLowerCase())
      ) {
  ...
  private checkSignCompatibility(sign1: string, sign2: string): number {
    ...
    if (
      compatibilityMatrix[sign1Lower] &&
      compatibilityMatrix[sign1Lower][sign2Lower]
    ) {
  ...
  private async synthesizeHoroscopeText(
    ...
    try {
      ...
      text +=
        phaseData.guidance ||
        "Embrace the cosmic flow and stay open to new possibilities.";
   ...
   */
  private async generateFallbackHoroscope(
    sign: string,
    date: string,
  ): Promise<DailyHoroscopeJSON> {
    const signData =
  ...
  private analyzeEmotionalWeather(
    _lunarPhase: LunarPhaseData,
    _transits: TransitEvent[],
  ): any {
  ...
  private async generateAreaGuidance(
    _lunarPhase: LunarPhaseData,
    _transits: TransitEvent[],
  ): Promise<any> {
  ...
  private async findPersonalTransits(
    _natalMoon: any,
    _date: string,
  ...
  private generatePersonalThemes(
    _natalMoon: any,
    _transits: TransitEvent[],
  ): string[] {
  ...
  private async generateEmotionalForecast(
    _natalMoon: any,
    _transits: TransitEvent[],
  ): Promise<string> {
  ...
  private async generateSpiritualMessage(
    _natalMoon: any,
    _transits: TransitEvent[],
  ): Promise<string> {
  ...
  getStatus(): any {
    return {
      ...
      knowledgeBase: {
        ...
        planetsLoaded: Object.keys(this.knowledgeBase.get("planets") || {})
          .length,
        aspectsLoaded: Object.keys(this.knowledgeBase.get("aspects") || {})
          .length,
        housesLoaded: Object.keys(this.knowledgeBase.get("houses") || {})
          .length,
      },
      lunarData: {
        phaseTemplates: this.lunarCycles.get("phases")
          ? Object.keys(this.lunarCycles.get("phases")).length
          : 0,
