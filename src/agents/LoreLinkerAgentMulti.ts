...
const defaultLogger: Logger = {
  info: (message: string, meta?: any) =>
    console.log(`[LoreLinker INFO] ${message}`, meta || ""),
  warn: (message: string, meta?: any) =>
    console.warn(`[LoreLinker WARN] ${message}`, meta || ""),
  error: (message: string, meta?: any) =>
    console.error(`[LoreLinker ERROR] ${message}`, meta || ""),
};
...
export class LoreLinkerAgentMulti {
  ...
  constructor(options?: {
    ...
    if (FIRECRAWL_API_KEY || process.env.NODE_ENV === "test") {
      try {
        this.crawler = new Firecrawl({
          apiKey: FIRECRAWL_API_KEY || "test-key",
        });
      } catch (error) {
      ...
      this.logger.warn(
        "Firecrawl API key not found. Will return stub entries.",
      );
    }
  ...
  public async processMultipleUrls(
    urls: string[],
  ): Promise<LoreLinkerMultiResult[]> {
    if (urls.length > MAX_URLS_BATCH) {
      this.logger.warn(
        `Processing ${urls.length} URLs in batches of ${MAX_URLS_BATCH}`,
      );
    }
   ...
   */
  private async processSingleUrlWithFallback(
    url: string,
  ): Promise<LoreLinkerMultiResult> {
    try {
      // Check cache first
      ...
      const result: LoreLinkerResult = {
        ...
        sourceTitle:
          ...
          this.extractTitleFromContent(cleanedText),
   ...
   */
  private enhanceResultWithFrequencies(
    result: LoreLinkerResult,
  ): LoreLinkerMultiResult {
   ...
   */
  public reduceSummaries(
    results: LoreLinkerMultiResult[],
  ): SummarizedLoreProfile {
    const successfulResults = results.filter(
      (r) => r.processingStatus === "success",
    );
    ...
    for (const result of successfulResults) {
      for (const [archetype, freq] of Object.entries(
        result.archetypeFrequencies,
      )) {
    ...
    for (const result of successfulResults) {
      for (const [category, tokens] of Object.entries(result.semanticTokens)) {
        for (const token of tokens) {
          tokenFrequency[category][token] =
            (tokenFrequency[category][token] || 0) + 1;
        }
      }
    }
    ...
    return {
      metaSummary,
      topArchetypes:
        topArchetypes.length > 0 ? topArchetypes : ["General Lore"],
      allKeywords,
  ...
  private summarize(doc: NlpDoc): string {
    ...
    const scored = sentences.map((s: string) => {
      ...
      if (
        s.length >= OPTIMAL_SENTENCE_MIN_LENGTH &&
    ...
    for (
      let i = 0;
      i < scored.length && selected.length < SUMMARY_SENTENCE_LIMIT;
      i++
    ) {
  ...
  private extractSemanticTokens(doc: NlpDoc): Record<string, string[]> {
      ...
      Array.from(
        new Set(
          tokens
            .filter(
              (w) =>
                w.length > MIN_TOKEN_LENGTH && !stopWords.has(w.toLowerCase()),
            )
            .map((w) => {
              const base = nlp(w);
              return (
    ...
    return {
      ...
      adjectives: cleanTokens(doc.adjectives().out("array")).slice(
        0,
        MAX_ADJECTIVES,
      ),
    };
  ...
  private async getCachedResult(url: string): Promise<LoreLinkerResult | null> {
    ...
    if (
      localEntry &&
      Date.now() - localEntry.timestamp < CACHE_TTL_SECONDS * 1000
    ) {
  ...
  private async cacheResult(
    ...
    if (this.localCache.size > 100) {
      const sortedEntries = Array.from(this.localCache.entries()).sort(
