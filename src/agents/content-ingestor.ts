...
export interface StructuredContent {
  ...
  title: string;
  content: string;
  contentType: 'article' | 'definition' | 'interpretation' | 'tutorial' | 'reference';
  category: 'planets' | 'signs' | 'houses' | 'aspects' | 'techniques' | 'history' | 'modern';
  tags: string[];
  author?: string;
  publishDate?: string;
  lastUpdated: string;
...
export class ContentIngestorAgent extends Agent {
  ...
  async extractStructuredContent(
    ...
    try {
      ...
      const category = this.categorizeContent(contentData.title, contentData.text, url);
      const contentType = this.determineContentType(contentData.text, url);
      
      // Calculate credibility score
      const credibilityScore = this.calculateCredibilityScore(sourceId, contentData);
  ...
  getStatus(): any {
    return {
      ...
      knowledgePool: {
        ...
        categories: Array.from(new Set(Array.from(this.knowledgePool.values()).map(c => c.category)))
      }
    };
  }
}
