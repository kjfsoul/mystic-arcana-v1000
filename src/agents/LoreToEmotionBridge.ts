...
export class LoreToEmotionBridge {
   ...
   */
  public transformToEmotionalFormat(profile: SummarizedLoreProfile): EmotionalSnapshot {
   ...
   */
  public generateVisualizationMetadata(profile: SummarizedLoreProfile): CosmicViewMetadata {
    // Group archetypes into constellation patterns
    const topArchetypeConstellations = this.groupArchetypesIntoConstellations(profile.topArchetypes);
    ...
    const archetypeToColorMap = this.createArchetypeColorMap(profile.topArchetypes);
  ...
  private generateToneDescriptors(profile: SummarizedLoreProfile): string[] {
    ...
    profile.topArchetypes.forEach(archetype => {
      ...
      if (mapping) {
        mapping.toneDescriptors.forEach(descriptor => descriptors.add(descriptor));
      }
    });
  ...
  private inferNeeds(profile: SummarizedLoreProfile): string[] {
    ...
    const keywords = profile.allKeywords.nouns.concat(profile.allKeywords.verbs);
    
    // Spiritual/mystical content suggests need for transcendence
    if (keywords.some(k => ['spiritual', 'mystic', 'sacred', 'divine'].includes(k.toLowerCase()))) {
      needs.add('transcendence');
      needs.add('meaning');
    }
    
    // Adventure/journey content suggests need for growth
    if (keywords.some(k => ['journey', 'adventure', 'quest', 'exploration'].includes(k.toLowerCase()))) {
      needs.add('growth');
      needs.add('discovery');
    }
    
    // Relationship content suggests need for connection
    if (keywords.some(k => ['love', 'relationship', 'connection', 'community'].includes(k.toLowerCase()))) {
  ...
  private createPlanetMoodMap(archetypes: string[]): Record<string, string> {
    ...
    archetypes.forEach(archetype => {
      ...
      if (mapping && mapping.planetAssociation) {
        planetMap[mapping.planetAssociation] = this.convertColorToMoodDescription(mapping.moodColor);
      }
    });
   ...
   */
  private createArchetypeColorMap(archetypes: string[]): Record<string, string> {
   ...
   */
  public static getArchetypeMapping(archetype: string): ArchetypeEmotionMapping | null {
