...
export const AGENT_REGISTRY = {
  ...
  'sophia': {
    ...
    capabilities: ['spiritual_synthesis', 'narrative_weaving', 'compassionate_guidance'],
    status: 'active'
  },
  'persona-learner': {
    name: 'PersonaLearner',
    ...
    capabilities: ['user_memory_storage', 'personalization_learning', 'pattern_recognition'],
    status: 'active'
  },
  'data-oracle': {
    name: 'DataOracle',
    ...
    capabilities: ['automated_data_harvesting', 'knowledge_graph_construction', 'semantic_relationship_mapping', 'multi_source_synthesis', 'explainable_ai_lineage', 'quality_validation'],
    status: 'active'
  },
  ...
  'content-ingestor': {
    ...
    capabilities: ['source_crawling', 'content_extraction', 'knowledge_structuring'],
    status: 'development'
  },
  'astrology-guru': {
    name: 'AstrologyGuru',
    ...
    capabilities: ['ephemeris_calculation', 'chart_generation', 'transit_analysis'],
    status: 'active'
  },
  'lunar-narrator': {
...
// Agent Status Types
export type AgentStatus = 'active' | 'development' | 'maintenance' | 'deprecated';
...
export function getActiveAgents(): Record<string, AgentInfo> {
  return Object.fromEntries(
    Object.entries(AGENT_REGISTRY).filter(([, info]) => info.status === 'active')
  );
}

export function getAgentsByCapability(capability: string): Record<string, AgentInfo> {
  return Object.fromEntries(
