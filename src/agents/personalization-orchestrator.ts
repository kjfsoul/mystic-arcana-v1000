...
export class PersonalizationOrchestratorAgent extends Agent {
  ...
  private initializeAdaptationRules(): void {
    ...
    this.adaptationRules.set("readerMatching", {
      analytical: {
        bestFor: ["logical", "detail-oriented", "skeptical"],
        readers: ["orion"],
      },
      intuitive: {
        bestFor: ["spiritual", "empathic", "creative"],
        readers: ["sophia", "luna"],
      },
      transformational: {
        bestFor: ["growth-seeking", "challenge-ready"],
        readers: ["sol"],
      },
      nurturing: {
        bestFor: ["healing", "emotional-support"],
        readers: ["luna"],
      },
    });
    ...
    this.adaptationRules.set("complexityProgression", {
      beginner: {
        spreads: ["single-card", "three-card"],
        guidance: "foundational",
      },
      intermediate: {
        spreads: ["three-card", "cross", "relationship"],
        guidance: "analytical",
      },
      advanced: {
        spreads: ["celtic-cross", "year-ahead", "soul-purpose"],
  ...
  async buildUserProfile(userId: string): Promise<UserProfile> {
    try {
      ...
      const learningProfile = this.assessLearningProfile(
        interactions,
        feedback,
      );
      ...
      console.error(
        "PersonalizationOrchestratorAgent: Profile building failed:",
        error,
      );
  ...
  async generateRecommendation(
    ...
    try {
      ...
      const communicationAdjustments = this.determineCommunicationAdjustments(
        profile,
        context,
      );
      return {
      ...
      console.error(
        "PersonalizationOrchestratorAgent: Recommendation failed:",
        error,
      );
  ...
  async learnFromFeedback(
    ...
    try {
      ...
      // Store learning data
      await this.supabase.from("personalization_learning").insert({
        user_id: userId,
        session_id: sessionId,
        feedback_data: feedback,
        adjustment_type:
          feedback.satisfactionScore > 4 ? "positive" : "negative",
      ...
      console.error(
        "PersonalizationOrchestratorAgent: Feedback learning failed:",
        error,
      );
    }
  }
  ...
  private determineCommunicationAdjustments(
    profile: UserProfile,
    context: AdaptationContext,
  ): string[] {
  ...
  private adjustPreferenceWeights(
    profile: UserProfile,
    feedback: any,
