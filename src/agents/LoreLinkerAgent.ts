...
export class LoreLinkerAgent {
  ...
  constructor() {
    if (!FIRECRAWL_API_KEY) {
      throw new Error('Firecrawl API key not found. Set FIRECRAWL_API_KEY environment variable.');
    }
  ...
  private summarize(doc: NlpDoc): string {
    ...
    const keywords = [
      ...
      'celestial', 'cosmos', 'spiritual', 'divination', 'horoscope', 'chart', 'sign', 'house', 'aspect',
    ];
  ...
  private extractSemanticTokens(doc: NlpDoc): Record<string, string[]> {
    const stopWords = new Set([
      ...
      'to', 'of', 'in', 'on', 'at', 'for', 'with', 'as', 'by', 'from', 'up', 'down', 'etc',
      'about', 'above', 'after', 'below', 'between', 'more', 'most', 'some', 'such',
      'not', 'own', 'same', 'so', 'than', 'too', 'very', 'can', 'will', 'just', 'should', 'now',
    ]);
