...
export class APIAuthHelper {
  private static sessionCache: { token: string; expiresAt: number } | null = null;
  ...
  static async getValidAccessToken(): Promise<string | null> {
    try {
      // Check cache first
      if (this.sessionCache && Date.now() < this.sessionCache.expiresAt - (this.REFRESH_BUFFER_SECONDS * 1000)) {
      ...
      this.sessionCache = {
        ...
        expiresAt: session.expires_at ? session.expires_at * 1000 : Date.now() + (60 * 60 * 1000) // Default 1 hour
      };
   ...
   */
  static async createAuthHeaders(additionalHeaders: Record<string, string> = {}): Promise<Headers> {
  ...
  static async authenticatedFetch(
    ...
    const headers = await this.createAuthHeaders(
      options.headers ? Object.fromEntries(new Headers(options.headers as HeadersInit)) : {}
    );
