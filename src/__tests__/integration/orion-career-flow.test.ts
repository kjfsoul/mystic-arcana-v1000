...
describe('Orion Career Flow Integration', () => {
  ...
  describe('Complete Career Reading Flow', () => {
    it('should provide comprehensive career guidance for Leo MC with Sun in 10th', async () => {
      ...
      const mockCalculator = require('@/lib/astrology/SwissEphemerisShim').SwissEphemerisCalculator;
      mockCalculator.prototype.calculateBirthChart.mockResolvedValue(mockBirthChart);
      mockCalculator.prototype.calculateTransits.mockResolvedValue(mockTransits);
    ...
    it('should adapt guidance based on Saturn return timing', async () => {
      ...
      const mockCalculator = require('@/lib/astrology/SwissEphemerisShim').SwissEphemerisCalculator;
      mockCalculator.prototype.calculateBirthChart.mockResolvedValue(saturnReturnChart);
      mockCalculator.prototype.calculateTransits.mockResolvedValue(saturnReturnTransits);
    ...
    it('should provide service-oriented guidance for Sun in 6th house', async () => {
      ...
      const mockCalculator = require('@/lib/astrology/SwissEphemerisShim').SwissEphemerisCalculator;
      mockCalculator.prototype.calculateBirthChart.mockResolvedValue(serviceOrientedChart);
      mockCalculator.prototype.calculateTransits.mockResolvedValue({ transits: [] });
      ...
      expect(response.interpretation.insights.some(insight => 
        insight.toLowerCase().includes('service') || insight.toLowerCase().includes('detail')
      )).toBe(true);
    });
  });
  ...
  describe('Career Prompt Integration', () => {
    ...
    it('should provide prompts for different MC signs', () => {
      const ariesPrompts = CareerPromptSelector.selectPromptsForChart('Aries', [], [], undefined);
      const taurusPrompts = CareerPromptSelector.selectPromptsForChart('Taurus', [], [], undefined);

      expect(ariesPrompts.some(p => p.prompt.includes('pioneer'))).toBe(true);
      expect(taurusPrompts.some(p => p.prompt.includes('stability'))).toBe(true);
    });
  });
  ...
  describe('Career Interpreter Integration', () => {
    ...
    it('should identify career strengths from planetary positions', () => {
      ...
      expect(interpretation.strengths.some(strength => 
  ...
  describe('Error Handling', () => {
    it('should handle missing birth data gracefully', async () => {
      ...
      await expect(orionReader.generateReading(invalidRequest))
        .rejects.toThrow('Birth data required');
    });

    it('should handle ephemeris calculation errors', async () => {
      const mockCalculator = require('@/lib/astrology/SwissEphemerisShim').SwissEphemerisCalculator;
      mockCalculator.prototype.calculateBirthChart.mockRejectedValue(new Error('Calculation error'));
      ...
      await expect(orionReader.generateReading(readingRequest))
