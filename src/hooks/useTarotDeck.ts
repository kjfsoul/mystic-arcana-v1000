...
export const useTarotDeck = (
  deckId: string = RIDER_WAITE_DECK_ID,
): UseTarotDeckState => {
  ...
  const fetchDeck = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || `Failed to fetch deck: ${response.status}`,
        );
      }
      ...
      console.log(
        `✨ Loaded ${data.cards.length} cards from ${data.deck.name}`,
      );
    } catch (err) {
      const errorMessage =
  ...
  const drawCards = useCallback(
    (count: number): TarotCard[] => {
      ...
      const currentOrder =
        shuffledOrder.length === cards.length
          ? shuffledOrder
...
export const useFilteredCards = (deckId?: string) => {
  ...
  const getCardsBySuit = useCallback(
    (suit: "wands" | "cups" | "swords" | "pentacles"): TarotCardData[] => {
  ...
  const getCardById = useCallback(
    (cardId: string): TarotCardData | undefined => {
