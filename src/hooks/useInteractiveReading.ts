...
export const useInteractiveReading = ({
  ...
  const [currentPrompt, setCurrentPrompt] = useState<InteractionPrompt | null>(
    null,
  );
    ...
    async (response: UserResponse) => {
      ...
      try {
        ...
        const { insights, traits } =
          await personaLearner.current.processInteractiveResponse(
            userId,
            sessionId,
            promptType,
            response.response,
  ...
  const generateContextualPrompts = useCallback(
    (context: {
      ...
      if (
        context.cardName &&
        readingType === InteractionSessionReadingType.TAROT
      ) {
        ...
        if (cardEnergy.includes("change") || cardEnergy.includes("new")) {
          prompts.push({
            ...
            question:
              "Are you currently going through a significant change in your life?",
            metadata: {
              cardName: context.cardName,
              theme: "change",
            },
      ...
      if (
        context.transitAspect &&
        readingType === InteractionSessionReadingType.ASTROLOGY
      ) {
        prompts.push({
          id: `astro_reflection_${Date.now()}`,
          type: PromptType.SCALE,
          question:
            "How much do you feel this cosmic influence in your life right now?",
