...
export class LoveEngine {
   ...
   */
  static async calculateLoveProfile(birthData: BirthData): Promise<LoveProfileResult> {
   ...
   */
  private static calculateBasicCompatibility(person1: BirthData, person2: BirthData): number {
  ...
  private static determineIdealPartner(sign: string): string {
    ...
    return partners[sign as keyof typeof partners] || 'Someone who complements your nature';
  }
  ...
  private static determineRomanticNeeds(sign: string): string[] {
    ...
    return needs[sign as keyof typeof needs] || ['Love', 'Understanding', 'Support'];
  }
