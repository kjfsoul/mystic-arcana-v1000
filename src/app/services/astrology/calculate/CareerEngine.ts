...
import type { BirthData, PlanetPosition } from '../../../../lib/astrology/AstronomicalCalculator';
...
export class CareerEngine {
   ...
   */
  static async calculateCareerAnalysis(birthData: BirthData): Promise<CareerAnalysisResult> {
   ...
   */
  static async calculateCareerTransits(birthData: BirthData): Promise<CareerTransitResult> {
  ...
  private static getIdealCareers(sunSign: string, mcSign: string): string[] {
    const careersBySign: Record<string, string[]> = {
      'Aries': ['Entrepreneur', 'Military Officer', 'Sports Coach', 'Emergency Responder', 'Sales Director'],
      'Taurus': ['Banking', 'Real Estate', 'Chef', 'Artist', 'Financial Advisor'],
      'Gemini': ['Journalist', 'Teacher', 'Marketing', 'Social Media Manager', 'Translator'],
      'Cancer': ['Nurse', 'Social Worker', 'Interior Designer', 'Chef', 'Child Care'],
      'Leo': ['Actor', 'CEO', 'Event Planner', 'Fashion Designer', 'Entertainment Industry'],
      'Virgo': ['Accountant', 'Editor', 'Health Professional', 'Analyst', 'Quality Control'],
      'Libra': ['Lawyer', 'Diplomat', 'Interior Designer', 'HR Manager', 'Counselor'],
      'Scorpio': ['Psychologist', 'Detective', 'Surgeon', 'Researcher', 'Investment Banker'],
      'Sagittarius': ['Professor', 'Travel Guide', 'Publisher', 'International Business', 'Philosopher'],
      'Capricorn': ['Executive', 'Architect', 'Government Official', 'Project Manager', 'Engineer'],
      'Aquarius': ['Scientist', 'Tech Innovator', 'Social Activist', 'Astrologer', 'Inventor'],
      'Pisces': ['Artist', 'Therapist', 'Musician', 'Marine Biologist', 'Spiritual Counselor']
    };
  ...
  private static getCareerStrengths(sign: string): string[] {
    const strengths: Record<string, string[]> = {
      'Aries': ['Natural leadership', 'High energy', 'Quick decision-making', 'Pioneering spirit'],
      'Taurus': ['Reliability', 'Persistence', 'Practical skills', 'Financial acumen'],
      'Gemini': ['Communication skills', 'Adaptability', 'Quick learning', 'Networking'],
      'Cancer': ['Empathy', 'Team care', 'Intuition', 'Protective instincts'],
      'Leo': ['Charisma', 'Creativity', 'Confidence', 'Inspirational leadership'],
      'Virgo': ['Attention to detail', 'Analytical thinking', 'Efficiency', 'Problem-solving'],
      'Libra': ['Diplomacy', 'Teamwork', 'Aesthetic sense', 'Balance'],
      'Scorpio': ['Determination', 'Research skills', 'Transformation ability', 'Depth'],
      'Sagittarius': ['Vision', 'Teaching ability', 'Cultural awareness', 'Optimism'],
      'Capricorn': ['Discipline', 'Long-term planning', 'Authority', 'Structure'],
      'Aquarius': ['Innovation', 'Technology skills', 'Group dynamics', 'Originality'],
      'Pisces': ['Creativity', 'Compassion', 'Intuition', 'Healing abilities']
    };

    return strengths[sign] || ['Adaptability', 'Dedication', 'Communication', 'Problem-solving'];
  }

  private static getCareerChallenges(sign: string): string[] {
    const challenges: Record<string, string[]> = {
      'Aries': ['Impatience with details', 'May rush decisions', 'Difficulty with routine'],
      'Taurus': ['Resistance to change', 'Can be too cautious', 'Stubborn approach'],
      'Gemini': ['May lack focus', 'Difficulty with routine', 'Can spread too thin'],
      'Cancer': ['May take things personally', 'Mood fluctuations', 'Overly protective'],
      ...
      'Sagittarius': ['Lack of attention to details', 'Overcommitting', 'Tactless communication'],
      'Capricorn': ['Workaholic tendencies', 'Pessimism', 'Inflexibility'],
      'Aquarius': ['Detachment from emotions', 'Rebellious nature', 'Unconventional methods'],
      'Pisces': ['Lack of boundaries', 'Overwhelmed by emotions', 'Impractical approach']
    };

    return challenges[sign] || ['Work-life balance', 'Communication clarity', 'Time management'];
  }
  ...
  private static getCareerAdvice(sign: string): string[] {
    const advice: Record<string, string[]> = {
      'Aries': ['Trust your instincts', 'Take calculated risks', 'Lead by example'],
      'Taurus': ['Build long-term relationships', 'Focus on quality', 'Be patient with change'],
      'Gemini': ['Network actively', 'Stay curious', 'Communicate clearly'],
      'Cancer': ['Trust your intuition', 'Build supportive teams', 'Create emotional safety'],
      'Leo': ['Showcase your talents', 'Inspire others', 'Seek meaningful recognition'],
      ...
      'Sagittarius': ['Share your knowledge', 'Explore opportunities', 'Think globally'],
      'Capricorn': ['Set clear goals', 'Build your reputation', 'Be patient with success'],
      'Aquarius': ['Innovate constantly', 'Build communities', 'Stay true to your values'],
      'Pisces': ['Follow your intuition', 'Help others heal', 'Create beauty']
    };

    return advice[sign] || ['Stay authentic', 'Build relationships', 'Keep learning'];
  }
