...
const oracleCache = new Map<
  string,
  { data: DailyOracleData; timestamp: number }
>();
...
async function getDailyTarotSpread(): Promise<TarotCardOracleData[]> {
  try {
    ...
    logger.error(
      ...
      {},
      error as Error,
      "Failed to generate daily tarot spread",
    );
    throw error;
  }
}
...
async function getDailyHoroscope(
  birthData: BirthData,
): Promise<HoroscopeOracleData> {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/astrology/horoscope`,
      {
        method: "POST",
    ...
    if (!result.success || result.data.isUnavailable) {
      return {
        ...
        degrees: 0,
        daily:
          "Astrological guidance is temporarily unavailable. Trust your intuition today.",
        element: "Unknown",
        quality: "Unknown",
    ...
    logger.error(
      ...
      {},
      error as Error,
      "Failed to get daily horoscope",
    );
    return {
      sign: "Unavailable",
      degrees: 0,
      daily:
        "The stars are whispering secrets today. Listen to your inner wisdom and trust the path before you.",
      element: "Unknown",
      quality: "Unknown",
      rulingPlanet: "Unknown",
    };
...
function getCompatibilityInsight(sign: string): CompatibilityInsight {
  const compatibilityData: Record<string, CompatibilityInsight> = {
    aries: {
      ...
      relationshipAdvice:
        "Your natural leadership shines in partnerships today. Lead with passion but remember to listen.",
      communicationTips:
        "Speak directly but temper your intensity with warmth and understanding.",
    },
    taurus: {
      bestMatchSign: "Virgo",
      challengingSign: "Aquarius",
      relationshipAdvice:
        "Stability and consistency in relationships bring you peace today. Show love through actions.",
      communicationTips:
        "Take your time to express thoughts clearly. Your steady presence is comforting to others.",
    },
    gemini: {
    ...
    cancer: {
      ...
      relationshipAdvice:
        "Emotional depth and nurturing create strong bonds today. Trust your intuitive feelings.",
      communicationTips:
        "Share your feelings openly but create safe spaces for others to do the same.",
    },
    leo: {
      bestMatchSign: "Sagittarius",
      challengingSign: "Scorpio",
      relationshipAdvice:
        "Your generous heart and warm spirit attract loving connections today. Shine authentically.",
      communicationTips:
        "Express appreciation generously while ensuring others feel heard and valued.",
    },
    virgo: {
      bestMatchSign: "Capricorn",
      challengingSign: "Gemini",
      relationshipAdvice:
        "Practical support and attention to details strengthen relationships today. Serve with love.",
      communicationTips:
        "Offer helpful insights while avoiding criticism. Your practical wisdom is valued.",
    },
    libra: {
      bestMatchSign: "Gemini",
      challengingSign: "Capricorn",
      relationshipAdvice:
        "Harmony and balance in partnerships bring joy today. Seek win-win solutions.",
      communicationTips:
        "Your diplomatic skills smooth over conflicts. Create beauty in your conversations.",
    },
    scorpio: {
      bestMatchSign: "Pisces",
      challengingSign: "Leo",
      relationshipAdvice:
        "Deep, transformative connections fulfill your soul today. Embrace vulnerability.",
      communicationTips:
        "Share your insights with intensity balanced by compassion and trust.",
    },
    sagittarius: {
      bestMatchSign: "Aries",
      challengingSign: "Pisces",
      relationshipAdvice:
        "Adventure and growth shared with others expand your world today. Explore together.",
      communicationTips:
        "Your enthusiasm inspires others. Share your vision while respecting different perspectives.",
    },
    capricorn: {
      bestMatchSign: "Taurus",
      challengingSign: "Libra",
      relationshipAdvice:
        "Building long-term security in relationships brings satisfaction today. Invest wisely.",
      communicationTips:
        "Your reliability speaks volumes. Communicate goals clearly and honor commitments.",
    },
    aquarius: {
      bestMatchSign: "Libra",
      challengingSign: "Taurus",
      relationshipAdvice:
        "Unique connections and intellectual freedom energize partnerships today. Be authentically you.",
      communicationTips:
        "Share innovative ideas while staying grounded in genuine care for others.",
    },
    pisces: {
      bestMatchSign: "Cancer",
      challengingSign: "Sagittarius",
      relationshipAdvice:
        "Compassionate, spiritual connections touch your heart today. Love unconditionally.",
      communicationTips:
        "Express your dreams and emotions artistically. Your empathy heals and inspires.",
    },
  };
 ...
 */
function getSignMetadata(sign: string): {
  element: string;
  quality: string;
  rulingPlanet: string;
} {
  const signData: Record<
    string,
    { element: string; quality: string; rulingPlanet: string }
  > = {
...
function generatePersonalizedInterpretation(
  ...
  const positionInterpretations =
    interpretations[position] || interpretations["Morning Energy"];
  const randomIndex = Math.floor(
    Math.random() * positionInterpretations.length,
  );
...
function generateOverallTheme(
  cards: TarotCardOracleData[],
  horoscope: HoroscopeOracleData,
  cosmic: CosmicFocusData,
): string {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.info(
      "oracle_generation_start",
      body.userId,
      { birthData: body.birthData.city },
      "Starting daily oracle generation",
    );
    ...
    const dailySpread: DailySpread = {
      ...
      cards: {
        morning: {
          card: tarotCards[0],
          interpretation: tarotCards[0].isReversed
            ? tarotCards[0].meaning_reversed
            : tarotCards[0].meaning_upright,
          personalizedMessage: generatePersonalizedInterpretation(
            tarotCards[0],
            "Morning Energy",
            horoscope.sign,
          ),
        ...
        afternoon: {
          card: tarotCards[1],
          interpretation: tarotCards[1].isReversed
            ? tarotCards[1].meaning_reversed
            : tarotCards[1].meaning_upright,
          personalizedMessage: generatePersonalizedInterpretation(
            tarotCards[1],
            "Afternoon Focus",
            horoscope.sign,
        ...
        evening: {
          card: tarotCards[2],
          interpretation: tarotCards[2].isReversed
            ? tarotCards[2].meaning_reversed
            : tarotCards[2].meaning_upright,
          personalizedMessage: generatePersonalizedInterpretation(
            tarotCards[2],
            "Evening Reflection",
            horoscope.sign,
    ...
    const oracleData: DailyOracleData = {
      ...
      actionableAdvice: generateActionableAdvice(
        tarotCards,
        horoscope,
        cosmicFocus,
      ),
    ...
    await Bash(
      `npx claude-flow@alpha hooks post-edit --file "api/oracle/daily/route.ts" --memory-key "agent/backend/daily_oracle_implementation"`,
    );
    ...
    logger.error(
      "oracle_generation_error",
      body?.userId,
      {},
      error as Error,
      "Failed to generate daily oracle",
    );
    ...
    const fallbackData: DailyOracleData = {
      ...
      dailySpread: {
        ...
        overallGuidance:
          "Today is filled with potential for growth and discovery. Trust your intuition.",
        practicalAdvice:
      ...
      horoscope: {
        ...
        degrees: 0,
        daily:
          "The universe supports your journey today. Trust your path and remain open to guidance.",
        element: "Spirit",
        quality: "Universal",
