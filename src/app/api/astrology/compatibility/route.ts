...
function getDefaultScores() {
  return {
    ...
    overall: {
      ...
      keyAspects: ["Service unavailable"],
    },
  };
}
function calculateCompatibilityScores(synastryData: SynastryResult["data"]): {
  ...
  const harmonicAspects = aspects.filter(
    (a: SynastryAspect) =>
      a.aspect === "trine" ||
      a.aspect === "sextile" ||
  ...
  const challengingAspects = aspects.filter(
    (a: SynastryAspect) => a.aspect === "square" || a.aspect === "opposition",
  ).length;
  ...
  friendshipScore = Math.max(
    1,
    Math.min(5, (friendshipScore + aspectModifier * 0.8) / 20),
  );
  teamworkScore = Math.max(
    1,
    Math.min(5, (teamworkScore + aspectModifier * 0.9) / 20),
  );
...
function getLoveDescription(score: number): string {
  if (score >= 4.5)
    return "Incredible romantic chemistry! Your hearts beat in cosmic harmony, creating a passionate and deeply fulfilling connection.";
  if (score >= 3.5)
    return "Strong romantic potential with natural attraction and emotional understanding. Your love story has beautiful cosmic support.";
  if (score >= 2.5)
    return "Good romantic compatibility with some areas of growth. Communication and patience will strengthen your bond over time.";
  if (score >= 1.5)
    return "Moderate romantic connection that requires effort and understanding. Different approaches to love can create both challenges and growth.";
...
function getFriendshipDescription(score: number): string {
  if (score >= 4.5)
    return "Exceptional friendship potential! You understand each other intuitively and bring out the best in one another.";
  if (score >= 3.5)
    return "Strong friendship compatibility with shared values and mutual respect. You'll create lasting memories together.";
  if (score >= 2.5)
    return "Good friendship foundation with complementary qualities. Your differences can create interesting and enriching exchanges.";
  if (score >= 1.5)
    return "Moderate friendship compatibility. Building trust and finding common ground will strengthen your connection over time.";
...
function getTeamworkDescription(score: number): string {
  if (score >= 4.5)
    return "Outstanding teamwork potential! You complement each other's strengths and work toward goals with unified energy.";
  if (score >= 3.5)
    return "Strong collaborative compatibility with shared work ethics and complementary skills. Projects together will flourish.";
  if (score >= 2.5)
    return "Good teamwork potential with some areas to navigate. Clear communication about roles and expectations will help.";
  if (score >= 1.5)
    return "Moderate teamwork compatibility. Success will come through patience, compromise, and leveraging individual strengths.";
...
function extractKeyAspects(aspects: SynastryAspect[]): string[] {
  ...
  aspects.forEach((aspect: SynastryAspect) => {
    if (aspect.aspect === "conjunction") {
      keyAspects.push(
        `${aspect.planet1}-${aspect.planet2} conjunction creating powerful unified energy`,
      );
    } else if (aspect.aspect === "trine") {
      keyAspects.push(
        `${aspect.planet1}-${aspect.planet2} trine bringing natural harmony and flow`,
      );
    } else if (aspect.aspect === "sextile") {
      keyAspects.push(
        `${aspect.planet1}-${aspect.planet2} sextile offering opportunities for growth and cooperation`,
      );
    } else if (aspect.aspect === "square") {
      keyAspects.push(
        `${aspect.planet1}-${aspect.planet2} square providing dynamic tension that catalyzes growth`,
      );
    } else if (aspect.aspect === "opposition") {
      keyAspects.push(
        `${aspect.planet1}-${aspect.planet2} opposition creating complementary yet challenging dynamics`,
      );
    }
  });
  ...
  if (keyAspects.length < 3) {
    keyAspects.push(
      "Unique cosmic patterns creating opportunities for mutual growth and discovery",
    );
    keyAspects.push(
      "Planetary positions suggesting the importance of patience and open communication",
    );
    keyAspects.push(
      "Astrological indicators pointing toward building trust through shared experiences",
    );
  }
      ...
      return NextResponse.json({
        ...
        data: {
          love: {
            rating: 0,
            description:
              "Compatibility data temporarily unavailable. Please try again later.",
          },
          friendship: {
            rating: 0,
            description:
          ...
          overall: {
            summary:
              "We're currently unable to access the astronomical calculation service. Please try again in a few moments.",
