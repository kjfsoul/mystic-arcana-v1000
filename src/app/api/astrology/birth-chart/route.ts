...
function generateEnhancedSignSummary(chartData: any): string {
  try {
    ...
    const ascendantSign = houses.angles?.ascendant?.sign || houses.ascendant?.sign || 'Unknown';
    ...
    if (houses.success) {
      summary += "your house cusps have been precisely calculated using astronomical ephemeris data. ";
    } else {
      summary += "your chart uses reliable fallback calculations. ";
    }
    
    summary += "This comprehensive analysis reveals the cosmic influences active at your birth moment.";
...
function generateEnhancedHouseBreakdown(chartData: any): string[] {
  try {
    ...
    if (houses.angles) {
      ...
      if (asc) {
        breakdown.push(`Ascendant (1st House): ${asc.degree_in_sign?.toFixed(1) || asc.degree?.toFixed(1)}° ${asc.sign}`);
      }
      
      if (mc) {
        breakdown.push(`Midheaven (10th House): ${mc.degree_in_sign?.toFixed(1) || mc.degree?.toFixed(1)}° ${mc.sign}`);
      }
      
      if (houses.angles.descendant) {
        const desc = houses.angles.descendant;
        breakdown.push(`Descendant (7th House): ${desc.degree_in_sign?.toFixed(1)}° ${desc.sign}`);
      }
      
      if (houses.angles.imum_coeli) {
        const ic = houses.angles.imum_coeli;
        breakdown.push(`IC (4th House): ${ic.degree_in_sign?.toFixed(1)}° ${ic.sign}`);
      }
    } else if (houses.ascendant && houses.midheaven) {
      // Fallback to Kerykeion data
      breakdown.push(`Ascendant (1st House): ${houses.ascendant.degree?.toFixed(1)}° ${houses.ascendant.sign}`);
      breakdown.push(`Midheaven (10th House): ${houses.midheaven.degree?.toFixed(1)}° ${houses.midheaven.sign}`);
    }
    ...
    if (houses.metadata?.fallback_mode) {
      breakdown.push("Calculation: Mathematical fallback (Swiss Ephemeris unavailable)");
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const fallbackMode = chartResult.data.chart_data?.houses?.metadata?.fallback_mode || false;
