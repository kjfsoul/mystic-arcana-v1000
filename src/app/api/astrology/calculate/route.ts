...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      console.log(
        "🐍 Attempting Python calculation with Placidus integration...",
      );
      ...
      console.log(
        "⚠️ Python calculation failed, trying fallback methods:",
        (pythonError as Error).message,
      );
    }
    ...
    if (!chart) {
      try {
        ...
        const shimChart =
          await SwissEphemerisShim.calculateFullChart(birthData);
        chart = {
          ...
          ascendant: {
            degree: shimChart.ascendant,
            sign: SwissEphemerisShim.getZodiacSign(shimChart.ascendant),
          },
          midheaven: {
            degree: shimChart.midheaven,
            sign: SwissEphemerisShim.getZodiacSign(shimChart.midheaven),
          },
    ...
    if (!chart) {
      throw new Error(
        "All calculation methods failed - unable to generate astrological chart",
      );
    }
    ...
    return NextResponse.json(
      {
        ...
        metadata: {
          ...
          birthData: {
            name: birthData.name,
            date: (
              birthData.date || new Date(birthData.birthDate)
            ).toISOString(),
...
function callPythonCalculation(birthData: ValidatedBirthData): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    const scriptPath = path.join(
      process.cwd(),
      "src/services/astrology-python/cached_astrology.py",
    );
    ...
    pythonProcess.on("close", (code) => {
      if (code !== 0) {
        reject(
          new Error(`Python calculation failed with code ${code}: ${stderr}`),
        );
        return;
      }
...
export async function GET() {
  return NextResponse.json({
    ...
    description:
      "Calculate comprehensive astrological chart with planets, houses, and aspects",
    requiredFields: {
