    ...
    if (includePreview && shuffledCards.length >= 3) {
      preview = {
        ...
        middleCard: {
          ...
          arcana: shuffledCards[Math.floor(shuffledCards.length / 2)].arcana_type,
 ...
 */
function shuffleDeck(cards: any[], algorithm: string): any[] {
  const deck = [...cards];
 ...
 */
function calculateEntropy(shuffledCards: any[]): number {
  let entropy = 0;
