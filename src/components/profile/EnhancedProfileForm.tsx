...
export const EnhancedProfileForm: React.FC<EnhancedProfileFormProps> = ({
  ...
  const validateForm = () => {
    ...
    if (formData.birthTime && !/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(formData.birthTime)) {
            ...
            <div className={styles.readerGrid}>
              {tarotReaders.map((reader) => (
                <button
                  key={reader.id}
                  type="button"
                  onClick={() => setFormData(prev => ({ ...prev, preferredTarotReader: reader.id }))}
                  className={`${styles.readerOption} ${
                    formData.preferredTarotReader === reader.id ? styles.readerSelected : ''
                  }`}
                >
