  ...
  useEffect(() => {
    async function fetchCompatibility() {
      try {
        ...
        if (
          result.love.rating === 0 &&
          result.love.description.includes("temporarily unavailable")
        ) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 p-4 flex items-center justify-center">
        ...
        >
          ...
          <p className="text-purple-200">
            Analyzing planetary positions and synastry aspects...
          </p>
        </motion.div>
      </div>
    );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {isUnavailable && (
          ...
          >
            ...
            <p className="text-yellow-100 mb-4">
              Our astronomical calculation service (using Swiss Ephemeris and
              Kerykeion) is currently offline. We believe in providing authentic
              astrological insights based on real planetary positions, not mock
              data.
            </p>
            <button
              onClick={handleRetry}
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          ...
          >
            ...
            <div
              className={`text-3xl font-bold mb-2 ${getScoreColor(compatibility.love.rating)}`}
            >
              {getScoreStars(compatibility.love.rating)}
            </div>
            <p className="text-white/80 text-sm leading-relaxed">
              {compatibility.love.description}
            </p>
          </motion.div>
          ...
          >
            ...
            <div
              className={`text-3xl font-bold mb-2 ${getScoreColor(compatibility.friendship.rating)}`}
            >
              {getScoreStars(compatibility.friendship.rating)}
            </div>
            <p className="text-white/80 text-sm leading-relaxed">
              {compatibility.friendship.description}
            </p>
          </motion.div>
          ...
          >
            ...
            <div
              className={`text-3xl font-bold mb-2 ${getScoreColor(compatibility.teamwork.rating)}`}
            >
              {getScoreStars(compatibility.teamwork.rating)}
            </div>
            <p className="text-white/80 text-sm leading-relaxed">
              {compatibility.teamwork.description}
            </p>
          </motion.div>
        </div>
        {/* Overall Summary */}
        ...
        {!isUnavailable && (
          ...
          >
            <p>
              ✨ Analysis based on authentic astronomical calculations using
