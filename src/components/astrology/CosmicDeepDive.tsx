...
export const CosmicDeepDive: React.FC<CosmicDeepDiveProps> = ({
  className,
}) => {
  ...
  useEffect(() => {
    ...
    const drawGravitationalWaves = () => {
      gravitationalWaves.forEach((wave) => {
        ...
        ctx.globalAlpha =
          wave.opacity *
          (0.5 + 0.5 * Math.sin(time * 0.02 + wave.radius * 0.01));
        ...
        ctx.arc(
          wave.x,
          wave.y,
          wave.radius + Math.sin(time * 0.01 + wave.radius * 0.005) * 10,
          0,
          Math.PI * 2,
        );
        ctx.stroke();
        ...
        ctx.arc(
          wave.x,
          wave.y,
          wave.radius * 0.7 + Math.cos(time * 0.015 + wave.radius * 0.008) * 8,
          0,
          Math.PI * 2,
        );
        ctx.stroke();
