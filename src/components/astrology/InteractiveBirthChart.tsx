...
import {
  ...
  PlanetaryPosition,
  TransitAspect,
} from "@/lib/ephemeris/transitEngine";
    ...
    if (showTransits) {
      const loadTransits = async () => {
        try {
          ...
          const aspects = await transitEngine.calculateTransits(
            birthData,
            transitDate,
          );
      ...
      >
        ...
        >
          ...
          {planetPositions.map((planet, index) => {
            ...
            const planetInfo =
              PLANETS[planet.name.toLowerCase() as keyof typeof PLANETS];
          ...
          {/* Transit Planets */}
          {showTransits &&
            transitData.positions.map((transit, index) => {
              const coords = degreeToCoords(
                transit.longitude,
                transitRadius,
                center,
              ...
              const planetInfo =
                PLANETS[transit.planet as keyof typeof PLANETS];
          ...
          {showTransits &&
            transitData.aspects.map((aspect, index) => {
              const natalPlanet = planetPositions.find(
                (p) => p.name.toLowerCase() === aspect.natalPlanet,
              );
              const transitPlanet = transitData.positions.find(
                (p) => p.planet === aspect.transitPlanet,
              );

              if (!natalPlanet || !transitPlanet) return null;

              const natalCoords = degreeToCoords(
                natalPlanet.longitude,
                planetRadius,
                center,
                center,
              );
              const transitCoords = degreeToCoords(
              ...
              <div className={styles.modalHeader}>
                ...
                {selectedObject.type === "transit" && (
                  <>
                    <span className={styles.modalSymbol}>
                      {PLANETS[
                        (selectedObject.data as PlanetaryPosition)
                          .planet as keyof typeof PLANETS
                      ]?.symbol || "●"}
                    </span>
                    <h3>
                      Transit{" "}
                      {(selectedObject.data as PlanetaryPosition).planet}
                    </h3>
                  </>
                )}
                {selectedObject.type === "house" && (
                  <>
                    ...
                    <h3>
                      House {(selectedObject.data as HousePosition).number}
                    </h3>
                  </>
                )}
                {selectedObject.type === "sign" && (
                  <>
                    ...
                    <h3>
                      {(selectedObject.data as (typeof ZODIAC_SIGNS)[0]).name}
                    </h3>
                  </>
                )}
              </div>
              ...
              <div className={styles.modalBody}>
                {selectedObject.type === "planet" && (
                  <div>
                    <p>
                      ...
                      {(selectedObject.data as PlanetPosition).longitude}°{" "}
                      {(selectedObject.data as PlanetPosition).longitude}&apos;{" "}
                      {(selectedObject.data as PlanetPosition).sign}
                    </p>
                    <p>
                      <strong>House:</strong>{" "}
                      {(selectedObject.data as PlanetPosition).house}
                    </p>
                    {(selectedObject.data as PlanetPosition).isRetrograde && (
                      <p className={styles.retrogradeText}>
                        <strong>Retrograde:</strong> This planet appears to move
                        backward, intensifying its introspective energy.
                      </p>
                    )}
                    <p className={styles.interpretation}>
                      {getPlantInterpretation(
                        selectedObject.data as PlanetPosition,
                      )}
                    </p>
                  </div>
                )}
                {selectedObject.type === "transit" && (
                  <div>
                    <p>
                      ...
                      {Math.floor(
                        (selectedObject.data as PlanetaryPosition).longitude,
                      )}
                      ° {(selectedObject.data as PlanetaryPosition).sign}
                    </p>
                    <p>
                      <strong>Daily Motion:</strong>{" "}
                      {Math.abs(
                        (selectedObject.data as PlanetaryPosition).speed,
                      ).toFixed(2)}
                      ° per day
                    </p>
                    {(selectedObject.data as PlanetaryPosition).retrograde && (
                      <p className={styles.retrogradeText}>
                        <strong>Retrograde:</strong> This planet is currently
                        moving backward, creating reflective energy.
                      </p>
                    )}
                    <p className={styles.interpretation}>
                      This transit planet is currently influencing your natal
                      chart. Its position relative to your birth planets creates
                      specific energetic patterns and opportunities for growth.
                    </p>
                          ...
                          {transitData.aspects
                            ...
                            .map((aspect, idx) => (
                              <li key={idx} className="text-sm">
                                {aspect.aspect} {aspect.natalPlanet} (orb:{" "}
                ...
                {selectedObject.type === "house" && (
                  <div>
                    <p>
                      <strong>Cusp:</strong>{" "}
                      {Math.floor((selectedObject.data as HousePosition).cusp)}°{" "}
                      {(selectedObject.data as HousePosition).sign}
                    </p>
                    <p>
                      <strong>Ruling Planet:</strong>{" "}
                      {(selectedObject.data as HousePosition).ruler}
                    </p>
                    <p className={styles.interpretation}>
                      {getHouseInterpretation(
                        (selectedObject.data as HousePosition).number,
                      )}
                    </p>
                  </div>
                )}
                {selectedObject.type === "sign" && (
                  <div>
                    <p>
                      ...
                      {
                        (selectedObject.data as (typeof ZODIAC_SIGNS)[0])
                          .element
                      }
                    </p>
                    <p className={styles.interpretation}>
...
function getPlantInterpretation(planet: PlanetPosition): string {
  const interpretations: Record<string, string> = {
    ...
    Mercury:
      "Mercury rules communication, thinking, and information processing. It shows how you learn, express ideas, and connect with others through words and thoughts. This placement reveals your mental agility and communication style.",
    Venus:
      "Venus represents love, beauty, values, and attraction. It shows what you find beautiful, how you express affection, and what brings you pleasure. This placement reveals your approach to relationships and aesthetic preferences.",
    ...
    Jupiter:
      "Jupiter represents expansion, wisdom, and growth. It shows where you seek meaning, truth, and abundance. This placement reveals your philosophical nature, optimism, and potential for personal development.",
    Saturn:
      "Saturn represents structure, discipline, and life lessons. It shows where you face challenges that lead to wisdom and mastery. This placement reveals your approach to responsibility and long-term achievement.",
    Uranus:
      "Uranus represents innovation, rebellion, and sudden change. It shows where you seek freedom and express your uniqueness. This placement reveals your progressive nature and capacity for breakthrough insights.",
    Neptune:
      "Neptune represents dreams, spirituality, and transcendence. It shows where you connect with the divine and express your imagination. This placement reveals your intuitive gifts and spiritual aspirations.",
    Pluto:
      "Pluto represents transformation, power, and regeneration. It shows where you experience deep change and uncover hidden truths. This placement reveals your capacity for renewal and psychological insight.",
  };

  return (
    interpretations[planet.name] ||
    "This celestial body influences your personal journey in profound ways."
  );
}
function getHouseInterpretation(house: number): string {
  ...
  return (
    ...
    "This house area of life holds special significance in your personal journey."
  );
}
function getSignInterpretation(sign: {
  ...
  const interpretations: Record<string, string> = {
    Aries:
      "Aries energy is pioneering, courageous, and initiating. This fire sign brings enthusiasm, leadership, and a desire to be first. It represents new beginnings and the warrior spirit.",
    Taurus:
      "Taurus energy is stable, practical, and sensual. This earth sign brings patience, determination, and appreciation for beauty. It represents material security and enjoying life's pleasures.",
    Gemini:
      "Gemini energy is curious, communicative, and adaptable. This air sign brings versatility, wit, and love of learning. It represents mental agility and social connection.",
    Cancer:
      "Cancer energy is nurturing, intuitive, and protective. This water sign brings emotional depth, empathy, and strong family ties. It represents home, security, and maternal instincts.",
    ...
    Virgo:
      "Virgo energy is analytical, helpful, and perfectionistic. This earth sign brings attention to detail, service orientation, and practical wisdom. It represents healing and improvement.",
    Libra:
      "Libra energy is harmonious, diplomatic, and aesthetic. This air sign brings balance, fairness, and appreciation for beauty. It represents partnership and social grace.",
    Scorpio:
      "Scorpio energy is intense, transformative, and mysterious. This water sign brings emotional depth, psychological insight, and powerful regeneration. It represents death and rebirth.",
    Sagittarius:
      "Sagittarius energy is adventurous, philosophical, and optimistic. This fire sign brings wisdom-seeking, cultural exploration, and jovial spirit. It represents higher learning and freedom.",
    Capricorn:
      "Capricorn energy is ambitious, disciplined, and traditional. This earth sign brings structure, responsibility, and long-term vision. It represents achievement and authority.",
    Aquarius:
      "Aquarius energy is innovative, humanitarian, and independent. This air sign brings progressive thinking, social consciousness, and unique perspectives. It represents the future and collective ideals.",
    Pisces:
      "Pisces energy is compassionate, intuitive, and spiritual. This water sign brings imagination, empathy, and connection to the divine. It represents unity and transcendence.",
  };

  return (
    interpretations[sign.name] ||
    "This zodiac sign brings its unique energy and perspective to your chart."
  );
}
