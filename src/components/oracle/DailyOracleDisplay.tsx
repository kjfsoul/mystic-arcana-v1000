...
export const DailyOracleDisplay: React.FC<DailyOracleDisplayProps> = ({
  className,
}) => {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "tarot" | "cosmic" | "compatibility"
  >("overview");
    ...
    const loadOracleData = async () => {
      ...
      try {
        ...
        if (!profile?.birth_date) {
          setError(
            "Complete your birth profile to receive your Daily Oracle reading",
          );
          setIsLoading(false);
          return;
        }
        ...
        if (!result.success || !result.data) {
          throw new Error(
            result.error || "Failed to generate Daily Oracle reading",
          );
        }
        ...
        <div className={styles.shimmerGrid}>
          {[...Array(4)].map((_, i) => (
            <div key={i} className={styles.shimmerCard}>
              <div
                className={styles.shimmerLine}
                style={{ width: "70%", height: "20px", marginBottom: "12px" }}
              ></div>
              <div
                className={styles.shimmerLine}
      ...
      <div className={styles.tarotTab}>
        <div className={styles.spreadHeader}>
          <h4 className={styles.spreadTitle}>
            🃏 {oracleData.dailySpread.theme}
          </h4>
          <p className={styles.spreadDescription}>
            {oracleData.dailySpread.overallGuidance}
          </p>
        </div>

        <div className={styles.tarotCards}>
          {Object.entries(oracleData.dailySpread.cards).map(
            ([timeOfDay, cardData], index) => (
              <motion.div
                key={timeOfDay}
                ...
                <div className={styles.cardContent}>
                  <h5 className={styles.cardName}>
                    ...
                    {cardData.card.isReversed && (
                      <span className={styles.reversed}> (Reversed)</span>
                    )}
                  </h5>
        ...
        >
          ...
          <p className={styles.practicalText}>
            {oracleData.dailySpread.practicalAdvice}
          </p>
        </motion.div>
      </div>
    ),
      ...
      <div className={styles.cosmicTab}>
        ...
        >
          <h4 className={styles.horoscopeTitle}>
            ♌{" "}
            {oracleData.horoscope.sign.charAt(0).toUpperCase() +
              oracleData.horoscope.sign.slice(1)}{" "}
            Reading
          </h4>
        ...
        >
          ...
          <div className={styles.cosmicDetails}>
            <div className={styles.cosmicItem}>
              ...
              <span className={styles.cosmicValue}>
                {oracleData.cosmicFocus.moonPhase}
              </span>
            </div>
            <div className={styles.cosmicItem}>
              <span className={styles.cosmicLabel}>Moon Sign:</span>
              <span className={styles.cosmicValue}>
                {oracleData.cosmicFocus.moonSign}
              </span>
            </div>
            <div className={styles.cosmicItem}>
              <span className={styles.cosmicLabel}>Dominant Planet:</span>
              <span className={styles.cosmicValue}>
                {oracleData.cosmicFocus.dominantPlanet}
              </span>
            </div>
            <div className={styles.cosmicItem}>
              <span className={styles.cosmicLabel}>Energy Theme:</span>
              <span className={styles.cosmicValue}>
                {oracleData.cosmicFocus.energyTheme}
              </span>
            </div>
          </div>
          <div className={styles.keyAspects}>
            ...
            <div className={styles.aspectsList}>
              {oracleData.cosmicFocus.keyAspects.map((aspect, index) => (
                <span key={index} className={styles.aspect}>
                  {aspect}
                </span>
              ))}
            </div>
          ...
          <div className={styles.cosmicRecommendation}>
            ...
            <p className={styles.recommendationText}>
              {oracleData.cosmicFocus.recommendation}
            </p>
          </div>
        </motion.div>
      </div>
    ...
    compatibility: (
      <div className={styles.compatibilityTab}>
        <motion.div
          ...
          transition={{ delay: 0.1 }}
        >
          <h4 className={styles.compatibilityTitle}>
          ...
          <div className={styles.compatibilityDetails}>
            <div className={styles.matchSection}>
              <div className={styles.bestMatch}>
                ...
                <span className={styles.matchSign}>
                  {oracleData.compatibility.bestMatchSign}
                </span>
              </div>
              <div className={styles.challengingMatch}>
                <span className={styles.matchLabel}>
                  Growth Challenge With:
                </span>
                <span className={styles.challengeSign}>
                  {oracleData.compatibility.challengingSign}
                </span>
              </div>
            </div>

            <div className={styles.relationshipAdvice}>
              <h5 className={styles.relationshipTitle}>
                Relationship Guidance
              </h5>
              <p className={styles.relationshipText}>
                {oracleData.compatibility.relationshipAdvice}
              </p>
            </div>
            ...
            <div className={styles.communicationTips}>
              ...
              <p className={styles.communicationText}>
                {oracleData.compatibility.communicationTips}
              </p>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className={styles.previewHint}>
            <span className={styles.hintText}>
              Click + to explore your full reading
            </span>
