...
import {
  ...
  HoroscopeData,
  isHoroscopeAvailable,
  formatZodiacSign,
  getZodiacEmoji,
} from "@/lib/astrology/HoroscopeService";
...
export const DailyHoroscopeWidget: React.FC<DailyHoroscopeWidgetProps> = ({
  className,
}) => {
  const [horoscope, setHoroscope] = useState<DailyHoroscope | null>(null);
  const [realHoroscope, setRealHoroscope] = useState<HoroscopeData | null>(
    null,
  );
          ...
          try {
            ...
            if (profile?.birth_date && profile?.birth_city) {
              ...
              try {
                ...
                const realHoroscopeData =
                  await getPersonalizedHoroscope(birthData);
              ...
              const personalizedHoroscope = service.getPersonalizedHoroscope(
                profile.birth_date,
              );
  ...
  const ShimmerContent = () => (
    ...
    >
      ...
      <div className={styles.content}>
        ...
        <div
          className={styles.shimmerLine}
          style={{ width: "80%", height: "16px", marginBottom: "12px" }}
        ></div>
        <div
          className={styles.shimmerLine}
      ...
      <div className={styles.header}>
        <h3 className={styles.title}>
          <span className={styles.titleIcon}>
            {hasRealData && realHoroscope
              ? getZodiacEmoji(realHoroscope.sign)
              : "✨"}
          </span>
      ...
      <div className={styles.content}>
        ...
        {realHoroscope && !isHoroscopeAvailable(realHoroscope) && (
          <div className={styles.unavailableNotice}>
            ...
            <p className={styles.unavailableText}>{realHoroscope.daily}</p>
          </div>
        )}
        ...
        <div className={styles.details}>
          <div className={styles.detailItem}>
            ...
            <span className={styles.detailValue}>
              {horoscope && energyIcons[horoscope.general.energy]}{" "}
              {horoscope?.general.energy}
            </span>
          </div>
            ...
            <div className={styles.personalizedSection}>
              <div className={styles.zodiacHeader}>
                ...
                <span className={styles.signName}>
                  {horoscope.personalized.sign.charAt(0).toUpperCase() +
                    horoscope.personalized.sign.slice(1)}
                </span>
              </div>

              <div className={styles.personalizedInsight}>
                <p className={styles.insight}>
                  {horoscope.personalized.insight}
                </p>
              </div>
