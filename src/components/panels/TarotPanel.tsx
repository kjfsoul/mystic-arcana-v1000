...
export const TarotPanel: React.FC<TarotPanelProps> = ({ className = '' }) => {
  ...
  const spreads = [
    ...
    { id: 'celtic-cross', name: 'Celtic Cross', description: 'Comprehensive life reading' },
    { id: 'zodiac', name: 'Zodiac Spread', description: '12-card astrological layout' },
    ...
    <div className={`${styles.container} ${className}`}>
          ...
          {decks.map(deck => (
            ...
            >
              ...
              <div className={styles.deckInfo}>
                ...
                <p id={`deck-${deck.id}-desc`} className={styles.deckDescription}>
                  {deck.description}
                </p>
              </div>
            </button>
          ...
          {spreads.map(spread => (
            ...
            >
              ...
              <div className={styles.spreadInfo}>
                ...
                <p id={`spread-${spread.id}-desc`} className={styles.spreadDescription}>
                  {spread.description}
                </p>
              </div>
            </button>
      ...
      <section className={styles.cardSection} aria-labelledby="card-interaction-title">
        ...
        <div className={styles.deckArea}>
          <div className={`${styles.deck} ${isShuffling ? styles.shuffling : ''}`}>
            <div className={styles.cardStack}>
              {[...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className={styles.card}
                  style={{ transform: `translateY(${-i * 2}px) rotate(${i * 0.5}deg)` }}
                />
              ))}
            </div>
          </div>
