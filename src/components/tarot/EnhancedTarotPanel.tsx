...
export const EnhancedTarotPanel: React.FC<EnhancedTarotPanelProps> = ({
  ...
  const [selectedSpreadType, setSelectedSpreadType] =
    useState<SpreadType>(initialSpreadType);
  const [drawnCards, setDrawnCards] = useState<TarotCard[]>([]);
  const [isShuffling, setIsShuffling] = useState(false);
  const [showSpread, setShowSpread] = useState(false);
  const [isRevealing, setIsRevealing] = useState(false);
  const [selectedDeckId, _setSelectedDeckId] = useState(
    "00000000-0000-0000-0000-000000000001",
  );
  const [showSpreadSelector, setShowSpreadSelector] =
    useState(!socialMediaMode);
  ...
  const spreadConfigs = {
    single: {
      ...
      title: "Single Card",
      icon: Star,
      description: "Quick cosmic guidance",
    },
    "three-card": {
      cardCount: 3,
    ...
    custom: {
      ...
      title: "Custom Spread",
      icon: Users,
      description: "Personalized layout",
    },
  };
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div className="text-purple-300 text-sm mb-6">{error}</div>
          <motion.button
                  ...
                  {Object.entries(spreadConfigs).map(([type, config]) => {
                    ...
                    return (
                      <motion.button
                        ...
                        onClick={() =>
                          setSelectedSpreadType(type as SpreadType)
                        }
                      ...
                      >
                        <div className="flex items-center space-x-3 mb-2">
                          ...
                          />
                          <span
                            className={`font-medium ${isSelected ? "text-white" : "text-purple-300"}`}
                          >
                            {config.title}
                          </span>
                        </div>
                        <div
                          className={`text-sm ${isSelected ? "text-purple-200" : "text-purple-400"}`}
                        >
                          {config.description}
                        </div>
                        <div
                          className={`text-xs mt-2 ${isSelected ? "text-purple-300" : "text-purple-500"}`}
                        >
                          {config.cardCount}{" "}
                ...
                <div className="space-y-4">
                  ...
                  {!isShuffling && (
                    <p className="text-purple-400 text-lg max-w-md mx-auto">
                      Focus your intention and click to shuffle the{" "}
                      {spreadConfigs[selectedSpreadType].title.toLowerCase()}{" "}
                      spread
                    </p>
                  )}
                </div>
