...
export class NasaFallbackHandler {
  ...
  public async getFallbackData(): Promise<FallbackAstrologyData> {
    ...
    try {
      ...
      console.error('Could not read from NASA cache, fetching live data...', error);
  ...
  private generateThemeFromTitle(title: string): string {
    if (title.toLowerCase().includes('nebula')) return 'creation and cosmic mystery';
    if (title.toLowerCase().includes('galaxy')) return 'expansion and interconnectedness';
    if (title.toLowerCase().includes('cluster')) return 'community and shared brilliance';
    if (title.toLowerCase().includes('moon')) return 'intuition and inner cycles';
    if (title.toLowerCase().includes('mars')) return 'action and assertive energy';
  ...
  private findClosestNeo(near_earth_objects: Record<string, any[]>): any | null {
    ...
    return todayNeos.reduce((closest, current) => {
      ...
      const currentDist = parseFloat(current.close_approach_data[0].miss_distance.kilometers);
