...
function getFallbackHoroscope(): HoroscopeData {
  return {
    ...
    degrees: 0,
    daily:
      "Our astrological calculation service is currently offline. We use authentic astronomical calculations with Swiss Ephemeris data to determine your real sun sign and provide personalized insights. Please try again in a few moments.",
    isUnavailable: true,
  };
}
...
export function isHoroscopeAvailable(horoscope: HoroscopeData): boolean {
  return (
    !horoscope.isUnavailable &&
    horoscope.sign !== "Horoscope temporarily unavailable"
  );
}
