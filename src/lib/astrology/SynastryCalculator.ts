...
async function calculateRealCompatibility(
  person1Data: BirthData,
  person2Data: BirthData,
): Promise<CompatibilityAPIResponse> {
  try {
...
function getFallbackCompatibility(): CompatibilityResult {
  return {
    love: {
      rating: 0,
      description:
        "Compatibility data temporarily unavailable. Our astrological calculation service is currently offline. Please try again later.",
    },
    friendship: {
      rating: 0,
      description:
    ...
    overall: {
      summary:
        "We're currently unable to access our astronomical calculation service. This feature requires real-time planetary position calculations using Swiss Ephemeris data. Please try again in a few moments.",
      keyAspects: [
...
function analyzeSynastryAspects(
  chart1: any,
  chart2: any,
): {
  love: CompatibilityRating;
  ...
  const loveScore = calculateLoveCompatibility(
    aspects,
    chart1.planets,
    chart2.planets,
  );
  const friendshipScore = calculateFriendshipCompatibility(
    aspects,
    chart1.planets,
    chart2.planets,
  );
  const teamworkScore = calculateTeamworkCompatibility(
    aspects,
    chart1.planets,
    chart2.planets,
  );

  const keyAspects = generateKeyAspects(
    aspects,
    chart1.planets,
    chart2.planets,
  );
...
function calculateLoveCompatibility(
  aspects: any[],
  _planets1: PlanetPosition[],
  _planets2: PlanetPosition[],
): number {
  let score = 0;
  let maxScore = 0;
  ...
  const venusVenusAspects = aspects.filter(
...
function calculateFriendshipCompatibility(
  ...
  const sunSunAspects = aspects.filter(
...
function calculateTeamworkCompatibility(
  aspects: any[],
  _planets1: PlanetPosition[],
  _planets2: PlanetPosition[],
): number {
...
function getAspectDescription(aspect: any): string {
  ...
  return (
    descriptions[aspect.planet1]?.[aspect.planet2] ||
...
function analyzeElementCompatibility(
  ...
  const person1Dominant = Object.entries(elements1).reduce((a, b) =>
    elements1[a[0]] > elements1[b[0]] ? a : b,
  )[0];
  const person2Dominant = Object.entries(elements2).reduce((a, b) =>
  ...
  return (
    compatibility[person1Dominant]?.[person2Dominant] ||
    "Unique elemental blend creates interesting dynamics"
  );
}
...
export async function calculateCompatibility(
  ...
  try {
    ...
    const apiResponse = await calculateRealCompatibility(
      person1Data,
      person2Data,
    );
    ...
    if (!apiResponse.success || !apiResponse.data) {
      console.warn(
        "Compatibility calculation failed, using fallback:",
        apiResponse.error,
      );
