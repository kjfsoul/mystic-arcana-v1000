...
export interface SynastryAnalysis {
  connectionType: 'soulmate' | 'growth' | 'karmic' | 'challenging' | 'complementary';
...
export class CompatibilityEngine {
  static analyzeSynastry(chart1: ChartData, chart2: ChartData): SynastryAnalysis {
  ...
  private static getSynastryAspects(planets1: PlanetPosition[], planets2: PlanetPosition[]): any[] {
    ...
    for (const p1 of planets1) {
      ...
      for (const p2 of planets2) {
        ...
        else if (Math.abs(angle - 90) < 10) aspects.push({ type: 'square', p1, p2 });
        else if (Math.abs(angle - 120) < 10) aspects.push({ type: 'trine', p1, p2 });
      }
    }
    return aspects;
  }

  private static getConnectionType(aspects: any[]): SynastryAnalysis['connectionType'] {
  ...
  private static getStrengths(aspects: any[]): string[] {
    return aspects
      ...
      .map(a => `A harmonious ${a.type} between ${a.p1.name} and ${a.p2.name}`)
      .slice(0, 2);
  }
  ...
  private static getChallenges(aspects: any[]): string[] {
    return aspects
      ...
      .map(a => `A challenging ${a.type} between ${a.p1.name} and ${a.p2.name}`)
      .slice(0, 2);
  }

  private static getAdvice(connectionType: SynastryAnalysis['connectionType']): string[] {
    const advice: Record<SynastryAnalysis['connectionType'], string[]> = {
      soulmate: ["Lean into the natural harmony and celebrate your connection."],
      growth: ["Embrace the challenges as opportunities for mutual growth."],
      karmic: ["Be mindful of repeating past patterns and focus on conscious relating."],
      challenging: ["Practice patience and clear communication to navigate friction."],
      complementary: ["Appreciate your differences and learn from each other's perspectives."]
    };
