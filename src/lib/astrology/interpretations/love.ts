...
export class LoveInterpreter {
  static analyzeLoveProfile(
    ...
    return {
      ...
      shadowWork,
      compatibility,
      guidance: "Embrace your unique love signature and trust the wisdom of your heart.",
      timing: "Cosmic tides are currently favorable for heart-centered connections.",
      confidence: 0.8
    };
  }

  private static getRelationshipStyle(venus?: PlanetPosition, seventhHouse?: HousePosition): string {
    if (!venus || !seventhHouse) return "A seeker of deep and meaningful connections";
  ...
  private static getLoveLanguage(venus?: PlanetPosition): string {
    ...
    if (['taurus', 'virgo', 'capricorn'].includes(sign)) return "Acts of Service";
    if (['gemini', 'libra', 'aquarius'].includes(sign)) return "Words of Affirmation";
  ...
  private static getEmotionalNeeds(moon?: PlanetPosition): string[] {
    ...
    return [`To feel emotionally secure through ${this.getMoonSignNeeds(moon.sign)}`, `A need for nurturing and to be nurtured in return`];
  }

  private static getShadowWork(venus?: PlanetPosition, moon?: PlanetPosition): string {
    if (!venus || !moon) return "Learning to love and accept all parts of yourself";
  ...
  private static getCompatibility(venus?: PlanetPosition, moon?: PlanetPosition): { bestMatches: string[]; growthAreas: string[] } {
    if (!venus || !moon) return { bestMatches: ["earth"], growthAreas: ["Expressing emotional needs clearly"] };
    return {
