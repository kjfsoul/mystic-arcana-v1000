...
export const MIDHEAVEN_SIGNS = {
  ...
  taurus:
    "Banking, agriculture, beauty, luxury goods, stability-focused careers",
  gemini: "Communication, media, writing, teaching, versatile careers",
  cancer:
    "Nurturing professions, food service, real estate, family-oriented work",
  ...
  capricorn:
    "Business, government, traditional institutions, authoritative roles",
  aquarius: "Technology, humanitarian work, innovation, group-oriented careers",
  pisces:
    "Healing arts, spirituality, creativity, service-oriented professions",
} as const;

export class CareerInterpreter {
  static analyzeCareerPath(
    birthData: BirthData,
    ...
    const interpretation: CareerInterpretation = {
      primaryPath: this.determinePrimaryPath(
        midheaven,
        tenthHouse,
        sunPosition,
      ),
  ...
  static findMidheaven(houses: HousePosition[]): {
    sign: string;
    degree: number;
  } {
  ...
  static determinePrimaryPath(
    ...
    const mcInterpretation =
      MIDHEAVEN_SIGNS[mcSign as keyof typeof MIDHEAVEN_SIGNS] ||
    ...
    if (tenthHouse.planets.length > 0) {
      ...
      const planetKeywords =
        CAREER_PLANETS[dominantPlanet.planet as keyof typeof CAREER_PLANETS]
          ?.keywords || [];
  ...
  static identifyStrengths(
    ...
    tenthHousePlanets.forEach((planet) => {
      const planetData =
        CAREER_PLANETS[planet.planet as keyof typeof CAREER_PLANETS];
      if (planetData) {
  ...
  static identifyChallenges(
    ...
    if (saturn) {
      if (saturn.house === 10) {
        challenges.push(
          "May face delays or obstacles in achieving recognition",
        );
      }
      if (saturn.house === 6) {
    ...
    // Empty career houses
    const careerHousePlanets = planets.filter((p) =>
      [2, 6, 10].includes(p.house),
    );
    if (careerHousePlanets.length < 2) {
      challenges.push(
        "May need to actively develop career focus and direction",
      );
    }
  ...
  static analyzeCareerTiming(
    ...
    const nearSaturnReturn = saturnReturnAges.some(
      (age) => Math.abs(approximateAge - age) <= 2,
    );
  ...
  static generateCareerGuidance(
    ...
    if (tenthHouse.length > 0) {
      guidance +=
        "Your public reputation and professional image are especially important. ";
    }

    if (sun?.house === 6) {
      guidance +=
        "Find meaning in service and attention to detail in your work. ";
    } else if (sun?.house === 10) {
      guidance +=
        "Leadership and recognition are key themes in your career path. ";
    }

    guidance +=
      "Trust your instincts and take measured risks toward your professional goals.";

    return guidance;
  }

  static extractCareerKeywords(
    planets: PlanetPosition[],
    houses: HousePosition[],
  ): string[] {
