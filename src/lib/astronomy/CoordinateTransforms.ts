...
import { GeoLocation, EquatorialCoordinates, HorizontalCoordinates, ScreenCoordinates } from './types';
...
export class CoordinateTransforms {
  ...
  static equatorialToHorizontal(
    ...
    // Convert to radians
    const ra = rightAscension * 15 * Math.PI / 180; // Hours to radians
  ...
  static applyPrecession(
    ...
    const ra0 = coords.rightAscension * 15 * Math.PI / 180;
    const dec0 = coords.declination * Math.PI / 180;
    ...
    return {
      rightAscension: (ra * 180 / Math.PI) / 15, // Convert back to hours
