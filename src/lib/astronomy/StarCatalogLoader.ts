 ...
 */
import type { Star, EquatorialCoordinates } from "../../types/astronomical";
export class StarCatalogLoader {
  ...
  async loadConstellationLines(): Promise<{
    ...
    try {
      const response = await fetch(
        StarCatalogLoader.CATALOG_URLS.constellations,
      );
  ...
  private generateHipparcosCatalog(options: {
    maxMagnitude?: number;
    minDeclination?: number;
    maxDeclination?: number;
    constellations?: string[];
  ...
  private getRealBrightStars(): Star[] {
    ...
    const brightStarsData = [
      ...
      },
      {
        ...
        spectralType: "F5IV",
        distance: 11.5,
        properMotion: { ra: -714.59, dec: -1036.8 },
        constellation: "CMi",
        colorIndex: 0.42,
      },
      {
      ...
      },
      {
        ...
        spectralType: "B1III",
        distance: 250,
        properMotion: { ra: -42.5, dec: -31.73 },
        constellation: "Vir",
        colorIndex: -0.24,
        variableType: "ELL",
      },
      {
        ...
        spectralType: "M1.5I",
        distance: 600,
        properMotion: { ra: -12.11, dec: -23.3 },
        constellation: "Sco",
        colorIndex: 1.83,
        variableType: "LC",
      },
  ...
  private galacticToEquatorial(l: number, b: number): EquatorialCoordinates {
    ...
    const x =
      Math.sin(bRad) * Math.cos(deltaNGP) -
  ...
  private generateSpectralType(magnitude: number): string {
    ...
    if (magnitude < 2) {
      ...
      else if (rand < 0.3) type = "A";
      else if (rand < 0.45) type = "F";
      else if (rand < 0.6) type = "G";
      else if (rand < 0.8) type = "K";
      else type = "M";
    } else {
      ...
      else if (rand < 0.3) type = "G";
      else if (rand < 0.6) type = "K";
      else type = "M";
    }
    // Add subclass
    ...
    const luminosityClass =
      magnitude < 3
        ? ["I", "II", "III", "IV", "V"][Math.floor(Math.random() * 5)]
        : "V";
  ...
  private generateYaleBrightStars(options: {
    maxMagnitude?: number;
    minDeclination?: number;
    maxDeclination?: number;
    constellations?: string[];
   ...
   */
  static getStarColor(spectralType?: string): {
    r: number;
    g: number;
    b: number;
  } {
    ...
    return (
      this.SPECTRAL_COLORS[
        spectralClass as keyof typeof this.SPECTRAL_COLORS
      ] || { r: 1, g: 1, b: 1 }
    );
  }
  /**
