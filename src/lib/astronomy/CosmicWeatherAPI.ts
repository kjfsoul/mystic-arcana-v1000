...
export class CosmicWeatherAPI {
  ...
  private cache: Map<string, { data: CosmicInfluenceData; timestamp: number }> =
    new Map();
  ...
  async calculateCosmicWeather(
    ...
    try {
      ...
      const planets =
        await this.ephemerisBridge.calculatePlanetaryPositions(request);
      // Calculate aspects
      const aspects = await this.ephemerisBridge.calculateAspects(
        planetNames,
        time,
        8.0,
      );
  ...
  private async detectCurrentRetrogrades(
    _planets: PlanetaryData[],
  ): Promise<Planet[]> {
  ...
  private calculatePlanetaryHour(time: Date): Planet {
    ...
    const planetaryHourIndex =
      (dayRulerIndex + hour) % planetarySequence.length;
  ...
  private determineCosmicWeatherType(
    ...
    const phase =
      typeof moonPhase.phase === "string" ? moonPhase.phase : moonPhase.phase;
    if (
      phase === "New Moon" ||
    ...
    const tensionAspects = aspects.filter(
      (a) =>
    ...
    const harmonicAspects = aspects.filter(
      (a) => a.aspect === AspectType.TRINE || a.aspect === AspectType.SEXTILE,
    );
  ...
  private generateSpiritualInfluences(
    ...
    retrogrades.forEach((planet) => {
      switch (planet) {
        case Planet.MERCURY:
          influences.push(
            "Communication and technology may require extra attention",
          );
          break;
        case Planet.VENUS:
        ...
        case Planet.JUPITER:
          influences.push(
            "Philosophical beliefs and growth patterns are being reviewed",
          );
          break;
        case Planet.SATURN:
          influences.push(
            "Structures and responsibilities are being restructured",
          );
          break;
      }
    });
    ...
    aspects.slice(0, 3).forEach((aspect) => {
      ...
      influences.push(
        `${planetNames} brings ${this.getAspectInfluence(aspect)}`,
      );
    });
    return influences;
  }
  ...
  private getAspectInfluence(aspect: AspectData): string {
    const influences = {
      ...
      [AspectType.SESQUIQUADRATE]:
        "creative tension and breakthrough potential",
    };
   ...
   */
  private generateTarotCorrelations(
    aspects: AspectData[],
    moonPhase: MoonPhaseData,
  ): string[] {
  ...
  private calculateIntuitionLevel(
    moonPhase: MoonPhaseData,
    aspects: AspectData[],
  ): number {
    ...
    aspects.forEach((aspect) => {
      const planet1 =
        typeof aspect.planet1 === "string"
          ? aspect.planet1.toUpperCase()
          : aspect.planet1;
      const planet2 =
        typeof aspect.planet2 === "string"
          ? aspect.planet2.toUpperCase()
          : aspect.planet2;
  ...
  private calculateTransformationLevel(
    ...
    aspects.forEach((aspect) => {
      if (
        aspect.aspect === AspectType.SQUARE ||
        aspect.aspect === AspectType.OPPOSITION
      ) {
        level += 0.15;
      }
    });
  ...
  private calculateCommunicationLevel(
    retrogrades: Planet[],
    aspects: AspectData[],
  ): number {
    let level = 0.5;
  ...
  private calculateLoveLevel(aspects: AspectData[]): number {
    ...
    aspects.forEach((aspect) => {
      ...
      if (planet1 === Planet.VENUS || planet2 === Planet.VENUS) {
        if (
          aspect.aspect === AspectType.TRINE ||
          aspect.aspect === AspectType.SEXTILE
        ) {
          level += 0.2;
        }
      }
