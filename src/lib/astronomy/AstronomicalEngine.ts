...
import { PrecessionCorrection, VisibleCelestialBodies } from "./types";
...
export class AstronomicalEngine {
  ...
  transformCoordinates(
    ...
    if (
      cached &&
      Date.now() - this.lastUpdateTime < this.config.cacheTimeout!
    ) {
    ...
    const horizontal = this.equatorialToHorizontal(precessed, location, time);
  ...
  async getPlanetaryPositions(
    ...
    const rawPlanetaryData =
      await swissEphemeris.calculatePlanetaryPositions(request);
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      if (previousLongitude !== null) {
        ...
        const adjustedMotion =
          motion > 180 ? motion - 360 : motion < -180 ? motion + 360 : motion;
  ...
  calculatePrecession(epoch: Date, targetDate: Date): PrecessionCorrection {
    ...
    const thetaA = ((2004.3109 - 0.8533 * T - 0.000217 * T * T) * T) / 3600.0;
    return {
      deltaRA: zetaA,
      deltaDec: thetaA,
      epoch: epoch,
  ...
  calculatePrecessionMatrix(
    ...
    return [
      [
        cosZeta * cosZ * cosTheta - sinZeta * sinZ,
        -sinZeta * cosZ * cosTheta - cosZeta * sinZ,
        -sinZ * cosTheta,
      ],
      [
        cosZeta * sinZ * cosTheta + sinZeta * cosZ,
        -sinZeta * sinZ * cosTheta + cosZeta * cosZ,
        cosZ * cosTheta,
      ],
  ...
  private async loadHipparcosSubset(): Promise<Star[]> {
    ...
    const brightStars: Star[] = [
      ...
      // Arcturus (α Bootis)
      {
        ...
        properMotion: { ra: -1093.45, dec: -1999.4 },
  ...
  private equatorialToHorizontal(
    ...
    const cosAz =
      (Math.sin(decRad) -
        Math.sin((altitude * Math.PI) / 180) * Math.sin(latRad)) /
  ...
  private applyRefraction(
    ...
    const r =
      1.02 /
      Math.tan(
        ((coords.altitude + 10.3 / (coords.altitude + 5.11)) * Math.PI) / 180,
      );
    return {
