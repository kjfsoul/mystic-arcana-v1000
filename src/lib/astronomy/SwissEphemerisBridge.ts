...
import type { RetrogradeData } from "./types";
import type { Planet, ZodiacSign, AspectType } from "../../types/astronomical";
...
export class SwissEphemerisBridge {
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      if (!validation.success) {
        console.warn(
          `Swiss Ephemeris API validation failed: ${validation.error}, falling back to mock data`,
        );
  ...
  async calculatePlanetaryPositions(
    ...
    try {
      ...
      const scriptArgs = [
        ...
        "--datetime",
        request.datetime.toISOString(),
        "--latitude",
        request.location.latitude.toString(),
        "--longitude",
        request.location.longitude.toString(),
        "--planets",
        request.planets.join(","),
        "--precision",
        this.config.precision,
      ];
      if (request.options?.includeRetrograde)
        scriptArgs.push("--include-retrograde");
      if (request.options?.calculateAspects)
        scriptArgs.push("--calculate-aspects");
      ...
      const planets = this.parsePlanetaryData(
        result.data as { planets: unknown[] },
      );
      this.setCache(cacheKey, planets);
      return planets;
    } catch (error) {
  ...
  async calculateAspects(
    ...
    const cacheKey = this.generateCacheKey("aspects", {
      planets,
      datetime,
      orbTolerance,
    });
    ...
    try {
      if (this.config.useMockData) {
        ...
        const aspects = this.generateMockAspectData(
          planets,
          datetime,
          orbTolerance,
        );
      ...
      const aspects = this.parseAspectData(
        result.data as { aspects: unknown[] },
      );
      this.setCache(cacheKey, aspects);
      return aspects;
    } catch (error) {
  ...
  async detectRetrogrades(
    ...
    const cacheKey = this.generateCacheKey("retrogrades", {
      planet,
      startDate,
      endDate,
    });
    ...
    try {
      if (this.config.useMockData) {
        ...
        const retrogrades = this.generateMockRetrogradeData(
          planet,
          startDate,
          endDate,
        );
      ...
      const retrogrades = this.parseRetrogradeData(
        result.data as { retrogrades: unknown[] },
      );
      this.setCache(cacheKey, retrogrades);
      return retrogrades;
    } catch (error) {
  ...
  async calculateCosmicWeather(
    ...
    const cacheKey = this.generateCacheKey("cosmic_weather", {
      datetime,
      location,
    });
    ...
    try {
      ...
      const cosmicWeather = this.parseCosmicWeatherData(
  ...
  async calculateMoonPhase(datetime: Date): Promise<{
    ...
    try {
      ...
      const moonData = this.parseMoonData(
        result.data as Record<string, unknown>,
      );
      this.setCache(cacheKey, moonData);
      return moonData;
  ...
  private async executePythonScript(
    ...
    try {
      ...
      const response = await fetch(
        `${this.config.apiEndpoint}/${args[0].replace(".py", "")}`,
        {
          method: "POST",
          headers: {
  ...
  private generateMockPlanetaryData(
    ...
    return request.planets.map((planetName, index) => {
      ...
      return {
        ...
        phase:
          planetName.toLowerCase() === "moon"
            ? (Math.sin(now / 2500000) + 1) / 2
            : 1,
  ...
  private generateMockAspectData(
    planets: string[],
    datetime: Date,
    orbTolerance: number,
  ): AspectData[] {
      ...
      for (let j = i + 1; j < planets.length; j++) {
        if (Math.random() < 0.3) {
          ...
          aspects.push({
            ...
            type: aspectType,
            angle:
              this.getAspectAngle(aspectType) +
              (Math.random() - 0.5) * orbTolerance,
  ...
  private getAspectInfluence(
    type: AspectType,
  ): "harmonious" | "challenging" | "neutral" {
    const harmonious: AspectType[] = [
      AspectType.CONJUNCTION,
      AspectType.SEXTILE,
      AspectType.TRINE,
    ];
    const challenging: AspectType[] = [
      AspectType.SQUARE,
      AspectType.OPPOSITION,
    ];
  ...
  private generateMockRetrogradeData(
    planet: string,
    startDate: Date,
    endDate: Date,
  ): RetrogradeData[] {
  ...
  private generateMockCosmicWeather(datetime: Date): CosmicInfluenceData {
    const moonPhase: MoonPhaseData = {
      ...
      nextPhase: {
        type: "Full Moon",
        date: new Date(datetime.getTime() + 3 * 24 * 60 * 60 * 1000),
      },
    };
    return {
      timestamp: datetime,
      ...
      planetaryHours: {
        ...
        next: {
          planet: "Mercury",
          startTime: new Date(datetime.getTime() + 60 * 60 * 1000),
        },
        ruler: "Sun",
  ...
  private parseRetrogradeData(rawData: {
    retrogrades: unknown[];
  }): RetrogradeData[] {
  ...
  private parseCosmicWeatherData(
    ...
    return {
      ...
      planetaryHours:
        rawData.planetary_hours as CosmicInfluenceData["planetaryHours"],
      aspects: {
      ...
      cosmicIntensity:
        rawData.cosmic_intensity as CosmicInfluenceData["cosmicIntensity"],
      spiritualInfluences:
        rawData.spiritual_influences as CosmicInfluenceData["spiritualInfluences"],
      optimalActivities:
        rawData.optimal_activities as CosmicInfluenceData["optimalActivities"],
