...
export class WishlistService {
  ...
  static async addToWishlist(
    ...
    try {
      ...
      if (!user) {
        return {
          success: false,
          error: "User must be authenticated to add to wishlist",
        };
      }
   ...
   */
  static async getWishlist(): Promise<{
    success: boolean;
    data?: WishlistItem[];
    error?: string;
  }> {
   ...
   */
  static async getWishlistCount(): Promise<{
    success: boolean;
    count?: number;
    error?: string;
  }> {
    ...
    try {
      ...
      if (filters.celestialEvents && filters.celestialEvents.length > 0) {
        filteredItems = filteredItems.filter((item) => {
          ...
          const alignmentArray = Array.isArray(cosmicAlignment)
            ? cosmicAlignment
            : [cosmicAlignment];
          return filters.celestialEvents!.some((event) =>
            alignmentArray.includes(event),
          );
        });
      }
