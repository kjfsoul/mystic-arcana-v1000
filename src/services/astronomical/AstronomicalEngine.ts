...
export class ProductionAstronomicalEngine implements AstronomicalEngine {
   ...
   */
  private eclipticToZodiac(longitude: number): {
    sign: ZodiacSign;
    degree: number;
  } {
    const signs = [
  ...
  async initialize(config: CalculationConfig): Promise<void> {
    ...
    if (!initialized) {
      console.warn(
        "⚠️ Swiss Ephemeris bridge failed to initialize, falling back to placeholder calculations",
      );
    }
    console.log(
      "🌟 Production Astronomical Engine initialized with Swiss Ephemeris",
    );
  }
  setObserverLocation(location: GeoLocation): void {
  ...
  async loadStarCatalog(
    ...
    try {
      ...
      const stars: Star[] = catalogStars.map((catalogStar) => ({
        ...
        parallax: catalogStar.distance
          ? 1000 / catalogStar.distance
          : undefined,
  ...
  async getPlanetaryPositions(
    ...
    try {
      ...
      const ephemerisPlanets =
        await this.swissEphemeris.calculatePlanetaryPositions({
          planets: [
            "sun",
            "moon",
            "mercury",
      ...
      console.log(
        `🪐 Calculated ${planets.length} planetary positions for ${time.toISOString()}`,
      );
      return planets;
    } catch (error) {
  ...
  async preloadEphemerisData(startDate: Date, endDate: Date): Promise<void> {
    console.log(
      `📊 Preloading ephemeris data from ${startDate.toISOString()} to ${endDate.toISOString()}`,
    );
  }
  clearCache(): void {
