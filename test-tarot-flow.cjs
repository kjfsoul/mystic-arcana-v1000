...
async function testTarotFlow() {
  ...
  try {
    const response = await fetch('http://localhost:3002/api/tarot/deck/00000000-0000-0000-0000-000000000001');
    ...
    console.log(`   Meaning: ${sampleCard.meaning.upright.substring(0, 50)}...`);
    ...
    if (error) {
      ...
      console.log(`✅ Database accessible, sample cards: ${tableData.map(c => c.name).join(', ')}`);
    }
