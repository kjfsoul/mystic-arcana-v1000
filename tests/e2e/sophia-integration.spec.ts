...
test.describe('Sophia Virtual Reader Integration Audit', () => {
  ...
  test('CRITICAL: Sophia virtual reader is present and visible', async ({ page }) => {
    ...
    const tarotRealmButton = page.locator('text="Enter the Tarot Realm"').first();
    ...
    console.log(`🤖 Sophia Detection Result: ${sophiaFound ? 'FOUND' : 'NOT FOUND'}`);
    if (sophiaFound) {
  ...
  test('VERIFICATION: Sophia displays greeting message on load', async ({ page }) => {
    await page.goto('/', { waitUntil: 'networkidle' });
    ...
    for (const selector of messageSelectors) {
      try {
        ...
        if (await element.isVisible({ timeout: 1000 })) {
          ...
          if (messageText.length > 10) { // Substantial message
            messageFound = true;
            break;
          }
        }
    ...
    console.log(`💬 Sophia Message Detection: ${messageFound ? 'FOUND' : 'NOT FOUND'}`);
    if (messageFound) {
  ...
  test('VERIFICATION: Sophia responds to tarot card interactions', async ({ page }) => {
    await page.goto('/', { waitUntil: 'networkidle' });
    ...
    if (await drawButton.isVisible({ timeout: 5000 })) {
      ...
      try {
        ...
        const cards = page.locator('[data-testid*="card"], .tarot-card, [class*="card"]');
    ...
    if (cardInteractionPossible) {
      ...
      const messageAfterInteraction = await page.locator('[class*="message"], [class*="tooltip"]').first().textContent();
      
      console.log(`💬 Sophia message after card interaction: "${messageAfterInteraction?.slice(0, 100) || 'None detected'}"`);
  ...
  test('VERIFICATION: Sophia integration performance impact', async ({ page }) => {
    ...
    const performanceMetrics = await page.evaluate(() => ({
      ...
      eventListeners: document.body.querySelectorAll('[onclick], [onmouseover], [onhover]').length
    }));
    ...
    if (performanceMetrics.memoryInfo) {
      console.log(`- Memory usage: ${Math.round(performanceMetrics.memoryInfo.usedJSHeapSize / 1024 / 1024)}MB`);
    }
  ...
  test('VERIFICATION: Sophia visual positioning and styling', async ({ page }) => {
    await page.goto('/', { waitUntil: 'networkidle' });
    ...
    const sophiaElement = page.locator('.fixed, [class*="virtual-reader"], [class*="sophia"]').first();
    ...
    if (await sophiaElement.isVisible({ timeout: 5000 })) {
      ...
      console.log(`- Location: bottom: ${computedStyle.bottom}, right: ${computedStyle.right}`);
      console.log(`- Size: ${computedStyle.width} x ${computedStyle.height}`);
      
      if (boundingBox) {
        console.log(`- Bounding box: ${boundingBox.width}x${boundingBox.height} at (${boundingBox.x}, ${boundingBox.y})`);
      }
  ...
  test('DIAGNOSTIC: Sophia Integration Comprehensive Assessment', async ({ page }) => {
    ...
    const assessment = await page.evaluate(() => {
      const results = {
        domElements: {
          virtualReader: !!document.querySelector('[class*="virtual-reader"], [class*="VirtualReader"]'),
          sophiaSpecific: !!document.querySelector('[class*="sophia"], [data-testid*="sophia"]'),
          messageElements: document.querySelectorAll('[class*="message"], [class*="tooltip"], [class*="dialogue"]').length,
          fixedPositioned: document.querySelectorAll('.fixed').length
        },
        scripts: {
          sophiaAgent: !!(window as any).SophiaAgent || !!(window as any).SophiaAgentClient,
          messageState: !!(window as any).sophiaMessage,
          agentClasses: Object.keys(window).filter(key => key.toLowerCase().includes('sophia')).length
        },
        interactions: {
          clickableElements: document.querySelectorAll('[onclick], button, [role="button"]').length,
