...
test.describe("Performance & Stability Audit", () => {
        ...
        for (const entry of entryList.getEntries()) {
          if (
            ...
            (window as any).layoutShifts.push({
              ...
              startTime: entry.startTime,
              sources:
                (entry as any).sources?.map((source: any) => ({
      ...
      const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
          ...
          if (target.className && typeof target.className === "string") {
            ...
            (window as any).renderCounts[componentName] =
              ((window as any).renderCounts[componentName] || 0) + 1;
          }
        });
      });
  ...
  test("CRITICAL: Homepage loads with LCP under 5 seconds", async ({
    page,
  }) => {
    ...
    await page.waitForSelector(
      '[data-testid="cosmic-hub"], .cosmic-hub, main',
      { timeout: 30000 },
    );
    ...
    if (lcpEntries.length === 0) {
      console.warn(
        "⚠️ No LCP entries found, checking navigation timing as fallback",
      );
  ...
  test("CRITICAL: No layout shifts or wobbling detected over 10 seconds", async ({
    page,
  }) => {
    await page.goto("/", { waitUntil: "networkidle" });
    await page.waitForSelector(
    ...
    while (!monitoringComplete) {
      ...
      const currentShifts = await page.evaluate(
        () => (window as any).layoutShifts || [],
      );

      // Calculate cumulative layout shift
      totalCLS = currentShifts.reduce(
  ...
  test("VERIFICATION: CosmicBackground component renders efficiently", async ({
    page,
  }) => {
    await page.goto("/", { waitUntil: "networkidle" });
    ...
    const renderData = await page.evaluate(() => ({
      ...
      totalMutations: Object.values((window as any).renderCounts || {}).reduce(
    ...
    const cosmicBackgroundRenders =
      renderData.renderCounts["CosmicBackground"] || 0;
  ...
  test("VERIFICATION: Save Reading button remains stable", async ({ page }) => {
    ...
    const tarotButton = page
      .locator('text="Draw Card"')
      .or(page.locator('text="Draw 3 Cards"'))
      .or(page.locator('[data-testid="draw-cards-button"]'))
      .first();
    ...
    if (await tarotButton.isVisible()) {
      ...
      const cards = page
        .locator('[data-testid="tarot-card"], .tarot-card')
        .first();
      if (await cards.isVisible()) {
        ...
        if (await saveButton.isVisible()) {
          ...
          const shiftsBeforeButton = await page.evaluate(
            () => (window as any).layoutShifts?.length || 0,
          );

          await page.waitForTimeout(2000);

          const shiftsAfterButton = await page.evaluate(
            () => (window as any).layoutShifts?.length || 0,
          );
      ...
      console.log(
        "ℹ️ Tarot interface not immediately available, skipping save button test",
      );
    }
  });

  test("VERIFICATION: Sophia virtual reader integration is stable", async ({
    page,
  }) => {
    await page.goto("/", { waitUntil: "networkidle" });

    // Look for Sophia virtual reader
    const sophiaReader = page
      .locator('[data-testid="virtual-reader"], .virtual-reader')
      .first();
    ...
    console.log(
      `🤖 Sophia Virtual Reader Status: ${sophiaExists ? "Present" : "Not detected"}`,
    );

    if (sophiaExists) {
      // Check that Sophia doesn't cause layout shifts
      const shiftsBeforeSophia = await page.evaluate(
        () => (window as any).layoutShifts?.length || 0,
      );
      ...
      const shiftsAfterSophia = await page.evaluate(
        () => (window as any).layoutShifts?.length || 0,
      );
      ...
      console.log(
        "ℹ️ Sophia virtual reader not detected - may need implementation verification",
      );
    }
  });

  test("DIAGNOSTIC: Capture comprehensive performance metrics", async ({
    page,
  }) => {
    const startTime = Date.now();
    ...
    if (metrics.memoryInfo) {
      console.log(
        `Memory Usage: ${Math.round(metrics.memoryInfo.usedJSHeapSize / 1024 / 1024)}MB`,
      );
    }

    // Log LCP specifically
    const lcpEntries = metrics.performanceEntries.filter(
