...
test.describe("Conversational Reading Flow", () => {
  ...
  test("Complete conversational reading flow for guest user", async ({
    page,
  }) => {
    ...
    await test.step("Wait for card drawing to complete", async () => {
      ...
      await expect(
        page.locator("text=Shuffling Cosmic Energies"),
      ).toBeVisible();
      ...
      await expect(
        page.locator("text=In conversation with Sophia"),
      ).toBeVisible();
    });
    ...
    await test.step("Interact with conversation options", async () => {
      ...
      await page.waitForSelector("button.bg-purple-700\\/30", {
        timeout: 5000,
      });
    ...
    await test.step("Complete multiple conversation turns", async () => {
      ...
      while (turnCount < maxTurns) {
        ...
        const hasOptions =
          (await page.locator("button.bg-purple-700\\/30").count()) > 0;
        ...
        // Wait for processing
        await page
          .waitForSelector("text=Sophia is reflecting", { timeout: 2000 })
    ...
    await test.step("Verify reading completion", async () => {
      ...
      const completeHeader = page.locator(
        'h2:has-text("Your Complete Reading")',
      );

      if (await completeHeader.isVisible()) {
        // Verify complete reading display
        await expect(
          page.locator("text=Sophia's personalized guidance"),
        ).toBeVisible();

        // Check for final reading sections
        await expect(page.locator('h4:has-text("Your Reading")')).toBeVisible();
        await expect(
          page.locator('h4:has-text("Guidance & Wisdom")'),
        ).toBeVisible();
        await expect(
          page.locator('h4:has-text("Spiritual Insight")'),
        ).toBeVisible();
    ...
    await test.step("Test save reading functionality", async () => {
      ...
      if (await saveButton.isVisible()) {
        ...
        await expect(
          page.locator('h3:has-text("Save Your Reading")'),
        ).toBeVisible();
  ...
  test("Conversational flow error handling", async ({ page }) => {
    ...
    await test.step("Test network error handling", async () => {
      ...
      if ((await optionButtons.count()) > 0) {
        ...
        await page
          .waitForSelector("text*=Unable to process", { timeout: 5000 })
          .catch(() => {
            console.log(
              "No error message found - this might be expected if using mock data",
            );
          });
      }
    });
...
async function waitForConversationState(
  ...
  await page.waitForFunction(
    (expectedState) => {
      ...
      return (
        stateIndicator && stateIndicator.textContent?.includes(expectedState)
      );
    },
    state,
    { timeout },
