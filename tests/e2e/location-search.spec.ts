...
test.describe("Location Search", () => {
  test("should allow a user to search for and select a location", async ({
    page,
  }) => {
    await page.goto("/");

    // Click the "Birth Chart Reading" heading to reveal the location search
    const birthChartHeading = page.locator(
      'h3:has-text("Birth Chart Reading")',
    );
    ...
    const locationSearchInput = page.locator(
      'input[placeholder="Enter city, state, country, or ZIP code"]',
    );
    ...
    // Type a location
    await page
      .locator('input[placeholder="Enter city, state, country, or ZIP code"]')
      .type("New York");
    ...
    await expect(
      page.locator('div:has-text("New York, NY, USA")'),
    ).toBeVisible();
  });
});
