...
test.describe("API Validation Audit", () => {
  test("CRITICAL: Geocode API accepts both q and input parameters", async ({
    request,
  }) => {
    const testQueries = [
    ...
    for (const query of testQueries) {
      const response = await request.get(
        `${BASE_URL}/api/geocode?${query.param}=${encodeURIComponent(query.value)}`,
      );
      ...
      if (data && Array.isArray(data)) {
        ...
        console.log(
          `✅ Geocode returned ${data.length} results for ${query.value}`,
        );
      }
    }
  });

  test("VERIFICATION: Geocode API handles edge cases gracefully", async ({
    request,
  }) => {
    const edgeCases = [
    ...
    for (const testCase of edgeCases) {
      const response = await request.get(
        `${BASE_URL}/api/geocode?${testCase.param}=${encodeURIComponent(testCase.value)}`,
      );

      console.log(
        `🧪 Edge case test (${testCase.param}="${testCase.value}"):`,
        {
          status: response.status(),
        },
      );
  ...
  test("CRITICAL: Save Reading API authentication flow", async ({
    request,
  }) => {
    const mockReading = {
      userId: "test-user",
    ...
    console.log(
      "✅ Save Reading API properly handles unauthenticated requests",
    );
  });

  test("VERIFICATION: Save Reading API input validation", async ({
    request,
  }) => {
    const invalidInputs = [
      {
        ...
        data: {
          userId: "test",
          cards: [
            { id: "test", name: "Test", position: "center", isReversed: false },
          ],
          interpretation: "test",
        },
      },
    ...
    for (const invalidInput of invalidInputs) {
      const response = await request.post(
        `${BASE_URL}/api/tarot/save-reading`,
        {
      ...
      if (response.status() !== 401) {
        // Might hit auth error first
        expect([400, 401]).toContain(response.status());
      }
    }
  });

  test("VERIFICATION: API response times are reasonable", async ({
    request,
  }) => {
    const apiTests = [
    ...
    for (const apiTest of apiTests) {
      ...
      if (apiTest.name !== "Tarot Draw") {
        // Tarot draw might need auth
        expect(response.status()).not.toBe(500);
      }
    }
  });
  ...
  test("DIAGNOSTIC: API Error Handling Assessment", async ({ request }) => {
    ...
    for (const errorTest of errorTests) {
      try {
        ...
        console.log(
          `❌ ${errorTest.name}: Test execution failed -`,
