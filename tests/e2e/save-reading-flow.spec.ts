...
test.describe('Save Reading Flow - Complete User Journey', () => {
  ...
  test('CRITICAL: Complete authenticated save reading flow', async ({ page }) => {
    ...
    console.log('📍 Page contains tarot-related text:', bodyText?.includes('tarot') || bodyText?.includes('Tarot') || false);
    ...
    if (isCardVisible) {
      ...
      const alternativeCards = page.locator('.card, [data-card], [role="button"]:has-text("card")');
    ...
    if (saveButtonFound && saveButton) {
      ...
      // Look for save modal
      const saveModal = page.locator('[data-testid="save-modal"], .save-modal, [class*="modal"]').first();
      if (await saveModal.isVisible({ timeout: 3000 })) {
        ...
        const interpretationField = page.locator('textarea[placeholder*="interpretation"], [name="interpretation"], #interpretation').first();
        if (await interpretationField.isVisible({ timeout: 2000 })) {
          await interpretationField.fill('Test interpretation for automated test');
        }

        // Fill question field if present
        const questionField = page.locator('input[placeholder*="question"], [name="question"], #question').first();
        ...
        const submitButton = page.locator('button[type="submit"], text="Save", .save-btn, [data-testid="submit-save"]').first();
  ...
  test('VERIFICATION: Save reading flow for guest users', async ({ page }) => {
    console.log('👤 Testing save reading flow for guest (unauthenticated) users...');
    ...
    const tarotCards = page.locator('.card-container, .tarot-card-display, [data-card-id]');
    ...
    const saveButton = page.locator('text="Save Reading", [data-testid="save-reading-button"]').first();
    ...
    if (await saveButton.isVisible({ timeout: 5000 })) {
      ...
      const authPrompt = page.locator('text="Sign in", text="Log in", text="Login", .auth-modal, [class*="login"]').first();
  ...
  test('DIAGNOSTIC: Save reading UI state analysis', async ({ page }) => {
    ...
    const initialSaveButtons = await page.locator('text="Save", [class*="save"]').count();
    ...
    if (cardCount > 0) {
      ...
      const postFlipSaveButtons = await page.locator('text="Save", [class*="save"]').count();
