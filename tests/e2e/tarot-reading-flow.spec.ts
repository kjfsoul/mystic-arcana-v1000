...
test.describe('Tarot Reading Flow E2E Tests', () => {
  ...
  test('Complete guest user single card reading flow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="spread-single"]')).toHaveClass(/border-purple-400/);
    ...
    await page.waitForSelector('[data-testid="tarot-card"]', { timeout: 10000 });
    ...
    await expect(page.locator('[data-testid="interpretation-panel"]')).toBeVisible();
    ...
    await page.locator('button:has-text("Save Reading")').click();
    await expect(page.locator('text=Reading saved successfully')).toBeVisible({ timeout: 5000 });
  ...
  test('Three-card reading requires authentication', async ({ page }) => {
    ...
    await expect(page.locator('text=Sign in to unlock advanced spreads')).toBeVisible();
  });
  ...
  test('Celtic Cross spread with authenticated user', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="spread-celtic-cross"]')).toHaveClass(/border-purple-400/);
    ...
    await page.waitForSelector('[data-testid="tarot-card"]', { timeout: 15000 });
  ...
  test('Interpretation panel functionality', async ({ page }) => {
    ...
    await expect(page.locator('text=This card invites you to explore')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="audio-toggle"]')).toHaveClass(/bg-purple-600/);
    ...
    for (const tab of tabs) {
      ...
      await expect(page.locator(`button:has-text("${tab}")`)).toHaveClass(/bg-purple-600/);
    }
  ...
  test('Error handling and recovery', async ({ page }) => {
    ...
    // Verify error state
    await expect(page.locator('text=Cosmic Connection Disrupted')).toBeVisible();
  ...
  test('Keyboard navigation accessibility', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="spread-three-card"]')).toHaveClass(/border-purple-400/);
  ...
  test('Mobile responsive behavior', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="spread-selector"]')).toHaveCSS('grid-template-columns', 'repeat(1, minmax(0, 1fr))');
