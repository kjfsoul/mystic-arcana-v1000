...
const EINSTEIN_EXPECTED_HOUSES: EinsteinHouseTest[] = [
  {
    ...
    description:
      "Ascendant - Rising sign reflecting Einstein's communicative, curious nature",
  },
  {
    house: 2,
    expectedSign: "Cancer",
...
function validateHouseCusp(
  houseData: any,
  expectedHouse: EinsteinHouseTest,
): boolean {
  ...
  if (sign !== expectedHouse.expectedSign) {
    console.error(
      `❌ House ${expectedHouse.house}: Expected ${expectedHouse.expectedSign}, got ${sign}`,
    );
    return false;
  }
  ...
  if (degree_in_sign < minDegree || degree_in_sign > maxDegree) {
    console.error(
      `❌ House ${expectedHouse.house}: Degree ${degree_in_sign.toFixed(1)}° out of range [${minDegree}-${maxDegree}]`,
    );
    return false;
  }

  console.log(
    `✅ House ${expectedHouse.house}: ${degree_in_sign.toFixed(1)}° ${sign} (within expected range)`,
  );
  return true;
}
...
function validateSpecialAngles(houseData: any): boolean {
  ...
  if (
    ...
    ascendant?.degree_in_sign >= 10 &&
    ascendant?.degree_in_sign <= 25
  ) {
    console.log(
      `✅ Ascendant: ${ascendant.degree_in_sign.toFixed(1)}° ${ascendant.sign}`,
    );
  } else {
    console.error(
      `❌ Ascendant: Expected 10-25° Gemini, got ${ascendant?.degree_in_sign?.toFixed(1)}° ${ascendant?.sign}`,
    );
    allValid = false;
  }
  ...
  if (
    ...
    midheaven?.degree_in_sign >= 5 &&
    midheaven?.degree_in_sign <= 25
  ) {
    console.log(
      `✅ Midheaven: ${midheaven.degree_in_sign.toFixed(1)}° ${midheaven.sign}`,
    );
  } else {
    console.error(
      `❌ Midheaven: Expected 5-25° Pisces, got ${midheaven?.degree_in_sign?.toFixed(1)}° ${midheaven?.sign}`,
    );
    allValid = false;
  }
...
async function runEinsteinHouseTest(): Promise<void> {
  ...
  console.log(
    "Testing: Placidus house system with Swiss Ephemeris integration\n",
  );
  ...
  try {
    ...
    console.log("================");
    console.log(
      `✅ Validations Passed: ${validationsPassed}/${totalValidations}`,
    );
    console.log(
      `📊 Success Rate: ${((validationsPassed / totalValidations) * 100).toFixed(1)}%`,
    );

    if (validationsPassed === totalValidations) {
      console.log(
        "🎉 ALL TESTS PASSED - Einstein house calculations are astronomically accurate!",
      );
    } else if (validationsPassed >= totalValidations * 0.8) {
      console.log(
        "⚠️  MOST TESTS PASSED - House calculations are within acceptable tolerance",
      );
    } else {
    ...
    console.log(
      `Coordinate: ${houseData.metadata?.latitude}°N, ${houseData.metadata?.longitude}°E`,
    );
    console.log(`House System: ${houseData.metadata?.house_system}`);
    console.log(`Fallback Mode: ${houseData.metadata?.fallback_mode || false}`);
    console.log(
      `Swiss Ephemeris Available: ${houseData.method?.includes("Swiss Ephemeris") || false}`,
    );
  } catch (error) {
