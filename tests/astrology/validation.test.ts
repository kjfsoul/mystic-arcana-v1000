...
describe("Astrology Validation Suite - Historical Figures", () => {
  ...
  describe("Solar Position Validation", () => {
    test.each(historicalFigures)(
      ...
      async ({ name, birthData, expectedSunSign, notes, source }) => {
        ...
        console.log(
          `✓ ${name}: Sun in ${sun?.sign} at ${sun?.longitude.toFixed(2)}° (${notes})`,
        );
      },
    );
  });
    ...
    test.each(historicalFigures.filter((f) => f.expectedMoonSign))(
      ...
      async ({ name, birthData, expectedMoonSign, notes }) => {
        ...
        if (expectedMoonSign) {
          ...
          console.log(
            `✓ ${name}: Moon in ${moon?.sign} at ${moon?.longitude.toFixed(2)}° (${notes})`,
          );
        }
      },
    );
  ...
  describe("Planetary Position Accuracy", () => {
    it("should calculate all major planets for complete birth chart", async () => {
      ...
      const chart = await SwissEphemerisShim.calculateFullChart(
        testCase.birthData,
      );
      ...
      for (const planetName of expectedPlanets) {
        ...
        console.log(
          `${planetName}: ${planet?.longitude.toFixed(2)}° ${planet?.sign} in House ${planet?.house}`,
        );
      }
    });
    ...
    it("should handle retrograde motion correctly", async () => {
      ...
      chart.planets.forEach((planet) => {
        ...
        if (
          ...
          console.log(
            `${planet.name}: ${planet.isRetrograde ? "Retrograde" : "Direct"} at ${planet.speed.toFixed(4)}°/day`,
          );
        }
      });
    });
  ...
  describe("House System Validation", () => {
    it("should calculate 12 houses with proper cusps", async () => {
      ...
      for (let i = 0; i < 12; i++) {
        ...
        expect(house.sign).toMatch(
          /^(Aries|Taurus|Gemini|Cancer|Leo|Virgo|Libra|Scorpio|Sagittarius|Capricorn|Aquarius|Pisces)$/,
        );
        expect(house.ruler).toMatch(
      ...
      console.log(
        `Ascendant: ${chart.ascendant.toFixed(2)}° ${chart.houses[0].sign}`,
      );
      console.log(
        `Midheaven: ${chart.midheaven.toFixed(2)}° ${chart.houses[9].sign}`,
      );
    });
    ...
    it("should assign planets to correct houses", async () => {
      ...
      chart.planets.forEach((planet) => {
        ...
        if (nextHouse.cusp < house.cusp) {
          expect(
            planet.longitude >= house.cusp || planet.longitude < nextHouse.cusp,
          ).toBe(true);
        } else {
        ...
        console.log(
          `${planet.name}: House ${planet.house} (${planet.longitude.toFixed(2)}° between ${house.cusp.toFixed(2)}° and ${nextHouse.cusp.toFixed(2)}°)`,
        );
      });
    });
  });
  ...
  describe("Zodiac Boundary Accuracy", () => {
    it("should handle zodiac sign boundaries correctly", async () => {
      ...
      const boundaryTests = [
        {
          ...
          expectedSigns: ["Pisces", "Aries"],
          description: "Pisces/Aries boundary",
        },
        {
          date: "2023-06-21T12:00:00Z",
          expectedSigns: ["Gemini", "Cancer"],
          description: "Gemini/Cancer boundary (Summer Solstice)",
        },
        {
          date: "2023-09-23T12:00:00Z",
          expectedSigns: ["Virgo", "Libra"],
          description: "Virgo/Libra boundary (Autumn Equinox)",
        },
        {
          date: "2023-12-22T12:00:00Z",
          expectedSigns: ["Sagittarius", "Capricorn"],
          description: "Sagittarius/Capricorn boundary (Winter Solstice)",
        },
      ];
      ...
      for (const test of boundaryTests) {
        ...
        console.log(
          `${test.description}: Sun in ${sun?.sign} at ${sun?.longitude.toFixed(4)}°`,
        );
      }
    });
  });
  ...
  describe("Time Period Accuracy", () => {
    it("should handle different historical eras correctly", async () => {
      ...
      const eras = [
        {
          name: "Ancient Era",
          figure: historicalFigures.find(
            (f) => f.name === "Leonardo da Vinci",
          )!,
        },
        {
          name: "Industrial Era",
          figure: historicalFigures.find(
            (f) => f.name === "Napoleon Bonaparte",
          )!,
        },
        {
          name: "Modern Era",
          figure: historicalFigures.find((f) => f.name === "Albert Einstein")!,
        },
        {
          name: "Contemporary Era",
          figure: historicalFigures.find((f) => f.name === "John F. Kennedy")!,
        },
      ];

      for (const era of eras) {
        const chart = await SwissEphemerisShim.calculateFullChart(
          era.figure.birthData,
        );
  ...
  describe("Statistical Accuracy Tests", () => {
    it("should maintain consistent calculations across multiple runs", async () => {
      ...
      console.log(
        `Consistency test: ${results.length} identical results for Sun position: ${firstResult?.toFixed(6)}°`,
      );
    });
    ...
    it("should show reasonable planetary speeds", async () => {
      ...
      chart.planets.forEach((planet) => {
        const expected =
          expectedSpeeds[planet.name as keyof typeof expectedSpeeds];
        if (expected) {
          expect(planet.speed).toBeGreaterThanOrEqual(expected.min);
          expect(planet.speed).toBeLessThanOrEqual(expected.max);

          console.log(
            `${planet.name}: ${planet.speed.toFixed(4)}°/day (${planet.isRetrograde ? "R" : "D"})`,
          );
        }
      });
    });
