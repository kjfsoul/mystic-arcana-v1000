...
describe("Astrology Compatibility Integration", () => {
  ...
  it("should handle Swiss Ephemeris failure and return fallback data", async () => {
    ...
    const SwissEphemerisShim =
      require("@/lib/astrology/SwissEphemerisShim").SwissEphemerisShim;
    SwissEphemerisShim.initialize.mockRejectedValueOnce(
      new Error("Swiss Ephemeris error"),
    );
    ...
    expect(result.overall.keyAspects).toContain(
  ...
  it("should handle calculation returning no valid data", async () => {
    ...
    const SwissEphemerisShim =
      require("@/lib/astrology/SwissEphemerisShim").SwissEphemerisShim;
    SwissEphemerisShim.calculateFullChart.mockResolvedValueOnce({
      planets: [],
    });
  ...
  it("should handle chart calculation errors gracefully", async () => {
    ...
    const SwissEphemerisShim =
      require("@/lib/astrology/SwissEphemerisShim").SwissEphemerisShim;
    SwissEphemerisShim.calculateFullChart.mockRejectedValueOnce(
      new Error("Chart calculation failed"),
    );
